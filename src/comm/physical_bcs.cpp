...
//
//  Each BC implementation fills ghost zones at one domain face using only the array
//  and index ranges.  The dispatch layer (ApplyPhysicalBCFace) handles parity sign
//  flips for Reflect and PolarWedge BCs using the parity module.
//
//  Face ordering in ApplyPhysicalBCs / ApplyPhysicalBCsOnCoarseLevel matches the old
//  system exactly: x1 first, then x2 (with extended x1 ghost limits), then x3 (with
//  extended x1 + x2 ghost limits).
...
// Extrapolation order is compile-time configurable via configure.py --nextrapolate.
// Coefficients are the signed binomial coefficients from BiasR<NEXTRAPOLATE>.
//
// NOTE: the x1 inner loops have a loop-carried dependency (each ghost cell reads the
// one filled in the previous iteration). The #pragma omp simd is inherited from the
// old system; compilers typically detect the dependency and ignore the hint. The x2/x3
// cases are safe because the cascading loop is an outer loop, not the simd-annotated one.
...
#pragma omp simd
            for (int i = il-1; i >= il-ngh; --i) {
              Real v = 0.0;
              for (int p = 0; p < NE; ++p) v += c[p] * var(n,k,j,i+p+1);
              var(n,k,j,i) = v;
            }
      break;
    case BoundaryFace::outer_x1:
...
#pragma omp simd
            for (int i = il; i <= iu; ++i) {
              Real v = 0.0;
              for (int p = 0; p < NE; ++p) v += c[p] * var(n,k,j+p+1,i);
              var(n,k,j,i) = v;
            }
      break;
    case BoundaryFace::outer_x2:
...
#pragma omp simd
            for (int i = il; i <= iu; ++i) {
              Real v = 0.0;
              for (int p = 0; p < NE; ++p) v += c[p] * var(n,k+p+1,j,i);
              var(n,k,j,i) = v;
            }
      break;
    case BoundaryFace::outer_x3:
...
// This is the geometric fill only - parity sign flips are applied by the dispatch layer.
// Matches the old polarwedge_cc.cpp pattern but without the hardcoded flip array.
...
// Compute the ghost-zone index range that a BC fill wrote to, for applying signs.
// The BC fills ghost zones outside [il..iu] x [jl..ju] x [kl..ku] by ngh cells
// on the face side.  This returns the range of the filled ghost region.

static void GhostRange(BoundaryFace face,
                       int il, int iu, int jl, int ju, int kl, int ku, int ngh,
                       int &si, int &ei, int &sj, int &ej, int &sk, int &ek) {
  si = il; ei = iu; sj = jl; ej = ju; sk = kl; ek = ku;
  switch (face) {
    case BoundaryFace::inner_x1: si = il - ngh; ei = il - 1; break;
    case BoundaryFace::outer_x1: si = iu + 1;   ei = iu + ngh; break;
    case BoundaryFace::inner_x2: sj = jl - ngh; ej = jl - 1; break;
    case BoundaryFace::outer_x2: sj = ju + 1;   ej = ju + ngh; break;
    case BoundaryFace::inner_x3: sk = kl - ngh; ek = kl - 1; break;
    case BoundaryFace::outer_x3: sk = ku + 1;   ek = ku + ngh; break;
    default: break;
  }
}
...
// GRSommerfeld is not a separate implementation - it maps to Outflow (CC/FC) or
// ExtrapolateOutflow (CX/VC), matching the old system's delegation pattern.
//
// For Reflect and PolarWedge, parity sign flips are applied after the geometric fill.
...
void ApplyPhysicalBCFace(AthenaArray<Real> &var,
  ...
  switch (bc) {
    ...
    case PhysicalBC::PolarWedge: {
      ...
      // Apply polar parity sign flips: theta/phi vector components and mixed tensors.
      if (!spec.component_groups.empty()) {
    ...
    case PhysicalBC::User:
      // User BCs are handled by the caller (they call a per-face enrolled function).
      break;
...
// Uses the same logic as the old apply_bndry_fn_[] array: true for reflect, outflow,
// extrapolate_outflow, gr_sommerfeld, polar_wedge, user.  False for block/periodic/polar.

static bool FaceNeedsBC(MeshBlock *pmb, BoundaryFace face) {
  BoundaryFlag bf = pmb->nc().boundary_flag(face);
  return (bf == BoundaryFlag::reflect
       || bf == BoundaryFlag::outflow
       || bf == BoundaryFlag::extrapolate_outflow
       || bf == BoundaryFlag::gr_sommerfeld
       || bf == BoundaryFlag::polar_wedge
       || bf == BoundaryFlag::user);
}
...
void ApplyPhysicalBCs(MeshBlock *pmb, const CommSpec &spec,
  ...
  if (FaceNeedsBC(pmb, BoundaryFace::inner_x1)) {
    ...
    if (bc == PhysicalBC::User && spec.physical_bc_fn != nullptr) {
      spec.physical_bc_fn(pmb, pmb->pcoord, var, time, dt,
                          is, ie, bjs, bje, bks, bke, ng);
    } else {
      ApplyPhysicalBCFace(var, BoundaryFace::inner_x1,
                          is, ie, bjs, bje, bks, bke, ng,
                          bc, spec);
    }
  }
  if (FaceNeedsBC(pmb, BoundaryFace::outer_x1)) {
    ...
    if (bc == PhysicalBC::User && spec.physical_bc_fn != nullptr) {
      ...
      ApplyPhysicalBCFace(var, BoundaryFace::outer_x1,
                          is, ie, bjs, bje, bks, bke, ng,
                          bc, spec);
    }
  }
  ...
  if (pmb->block_size.nx2 > 1) {
    if (FaceNeedsBC(pmb, BoundaryFace::inner_x2)) {
      PhysicalBC bc = spec.physical_bc[static_cast<int>(BoundaryFace::inner_x2)];
      if (bc == PhysicalBC::User && spec.physical_bc_fn != nullptr) {
        spec.physical_bc_fn(pmb, pmb->pcoord, var, time, dt,
                            bis, bie, js, je, bks, bke, ng);
      } else {
        ApplyPhysicalBCFace(var, BoundaryFace::inner_x2,
                            bis, bie, js, je, bks, bke, ng,
                            bc, spec);
      }
    }
    if (FaceNeedsBC(pmb, BoundaryFace::outer_x2)) {
      PhysicalBC bc = spec.physical_bc[static_cast<int>(BoundaryFace::outer_x2)];
      if (bc == PhysicalBC::User && spec.physical_bc_fn != nullptr) {
        spec.physical_bc_fn(pmb, pmb->pcoord, var, time, dt,
                            bis, bie, js, je, bks, bke, ng);
      } else {
        ApplyPhysicalBCFace(var, BoundaryFace::outer_x2,
                            bis, bie, js, je, bks, bke, ng,
                            bc, spec);
      }
    }
  ...
  if (pmb->block_size.nx3 > 1) {
    ...
    if (FaceNeedsBC(pmb, BoundaryFace::inner_x3)) {
      PhysicalBC bc = spec.physical_bc[static_cast<int>(BoundaryFace::inner_x3)];
      if (bc == PhysicalBC::User && spec.physical_bc_fn != nullptr) {
        spec.physical_bc_fn(pmb, pmb->pcoord, var, time, dt,
                            bis, bie, bjs, bje, ks, ke, ng);
      } else {
        ApplyPhysicalBCFace(var, BoundaryFace::inner_x3,
                            bis, bie, bjs, bje, ks, ke, ng,
                            bc, spec);
      }
    }
    if (FaceNeedsBC(pmb, BoundaryFace::outer_x3)) {
      PhysicalBC bc = spec.physical_bc[static_cast<int>(BoundaryFace::outer_x3)];
      if (bc == PhysicalBC::User && spec.physical_bc_fn != nullptr) {
        spec.physical_bc_fn(pmb, pmb->pcoord, var, time, dt,
                            bis, bie, bjs, bje, ks, ke, ng);
      } else {
        ApplyPhysicalBCFace(var, BoundaryFace::outer_x3,
                            bis, bie, bjs, bje, ks, ke, ng,
                            bc, spec);
      }
    }
...
//
// This replaces ApplyPhysicalBoundariesOnCoarseLevel().  Instead of swapping var/coarse
// pointers (InterchangeFundamentalCoarse), we pass the coarse array directly.
// The coarse coordinates come from MeshRefinement::pcoarsec.
//
// The caller provides the coarse interior bounds (cis/cie/cjs/cje/cks/cke) and the
// coarse ghost width (cng).  These vary per sampling type:
...
void ApplyPhysicalBCsOnCoarseLevel(MeshBlock *pmb, const CommSpec &spec,
  ...
  if (FaceNeedsBC(pmb, BoundaryFace::inner_x1)) {
    PhysicalBC bc = spec.physical_bc[static_cast<int>(BoundaryFace::inner_x1)];
    if (bc == PhysicalBC::User && spec.physical_bc_fn != nullptr) {
      spec.physical_bc_fn(pmb, pco, cvar, time, dt,
                          cis, cie, bjs, bje, bks, bke, cng);
    } else {
      ApplyPhysicalBCFace(cvar, BoundaryFace::inner_x1,
                          cis, cie, bjs, bje, bks, bke, cng,
                          bc, spec);
    }
  }
  if (FaceNeedsBC(pmb, BoundaryFace::outer_x1)) {
    ...
    if (bc == PhysicalBC::User && spec.physical_bc_fn != nullptr) {
      ...
      ApplyPhysicalBCFace(cvar, BoundaryFace::outer_x1,
                          cis, cie, bjs, bje, bks, bke, cng,
                          bc, spec);
    }
  }
  ...
  if (pmb->block_size.nx2 > 1) {
    if (FaceNeedsBC(pmb, BoundaryFace::inner_x2)) {
      ...
      if (bc == PhysicalBC::User && spec.physical_bc_fn != nullptr) {
        spec.physical_bc_fn(pmb, pco, cvar, time, dt,
                            bis, bie, cjs, cje, bks, bke, cng);
      } else {
        ApplyPhysicalBCFace(cvar, BoundaryFace::inner_x2,
                            bis, bie, cjs, cje, bks, bke, cng,
                            bc, spec);
      }
    }
    if (FaceNeedsBC(pmb, BoundaryFace::outer_x2)) {
      ...
      if (bc == PhysicalBC::User && spec.physical_bc_fn != nullptr) {
        ...
        ApplyPhysicalBCFace(cvar, BoundaryFace::outer_x2,
                            bis, bie, cjs, cje, bks, bke, cng,
                            bc, spec);
      }
    }
  ...
  if (pmb->block_size.nx3 > 1) {
    ...
    if (FaceNeedsBC(pmb, BoundaryFace::inner_x3)) {
      ...
      if (bc == PhysicalBC::User && spec.physical_bc_fn != nullptr) {
        spec.physical_bc_fn(pmb, pco, cvar, time, dt,
                            bis, bie, bjs, bje, cks, cke, cng);
      } else {
        ApplyPhysicalBCFace(cvar, BoundaryFace::inner_x3,
                            bis, bie, bjs, bje, cks, cke, cng,
                            bc, spec);
      }
    }
    if (FaceNeedsBC(pmb, BoundaryFace::outer_x3)) {
      ...
      if (bc == PhysicalBC::User && spec.physical_bc_fn != nullptr) {
        ...
        ApplyPhysicalBCFace(cvar, BoundaryFace::outer_x3,
                            bis, bie, bjs, bje, cks, cke, cng,
                            bc, spec);
      }
    }
...
//========================================================================================
//
// FC BCs operate on all 3 components of a FaceField simultaneously.  Each component's
// loop range extends by +1 in its own stagger direction (x1f: i+1, x2f: j+1, x3f: k+1).
// Sign flips are hardcoded here (not via parity module) because the FC field has only 3
// components with known geometric meaning: B1 = normal on x1 faces, etc.

//----------------------------------------------------------------------------------------
// ReflectBC_FC: mirror-image copy with normal component negation.
//
// For x1 faces (boundary in i-direction):
//   x1f (normal): ghost = -interior (sign flip), source offset by +1 vs CC (face stagger)
//   x2f (tangential): ghost = +interior, j-range extended by +1 (x2f stagger)
//   x3f (tangential): ghost = +interior, k-range extended by +1 (x3f stagger)
// Analogous patterns for x2/x3 faces, with the normal component rotating accordingly.
...
// x1f at iu+1 for outer_x1).  Tangential components copy from the boundary cell
// (e.g. x2f at il for inner_x1, x2f at iu for outer_x1).
// Each component extends its own stagger direction by +1 in the loop range.
...
//----------------------------------------------------------------------------------------
// PolarWedgeBC_FC: mirror copy across x2 boundary with per-component sign flips.
//
...
//
// The mirror geometry is identical to ReflectBC_FC for x2, with the normal component
// using face-stagger source offset (jl+j for inner, ju-j+1 for outer).
// Only valid for inner/outer x2 faces.
...
// GRSommerfeld delegates to OutflowBC_FC (confirmed from old gr_sommerfeld_fc.cpp).
// ExtrapolateOutflow and User are not implemented for FC - runtime error.
...
void ApplyPhysicalBCFace_FC(AthenaArray<Real> *fc[3], BoundaryFace face,
  ...
  switch (bc) {
    ...
    case PhysicalBC::Outflow:
      ...
      break;
    case PhysicalBC::GRSommerfeld:
      // FC GRSommerfeld always delegates to Outflow (no extrapolation variant).
      OutflowBC_FC(fc, face, il, iu, jl, ju, kl, ku, ngh);
      break;
    case PhysicalBC::PolarWedge:
    ...
    case PhysicalBC::User: {
      std::stringstream msg;
      msg << "### FATAL ERROR in ApplyPhysicalBCFace_FC\n"
          << "User BCs are not supported for face-centered variables." << std::endl;
      ATHENA_ERROR(msg);
      break;
    }
    case PhysicalBC::None:
...
// Same face ordering and transverse-limit extension logic as CC ApplyPhysicalBCs,
// but operates on FaceField and uses FC-specific dispatch.

void ApplyPhysicalBCs_FC(MeshBlock *pmb, const CommSpec &spec,
                         Real time, Real dt) {
  if (!spec.HasAnyPhysicalBC()) return;
  // FC data stored as three independent AthenaArray pointers; null check on first.
  if (spec.var_fc[0] == nullptr) return;

  AthenaArray<Real> *fc[3] = {spec.var_fc[0], spec.var_fc[1], spec.var_fc[2]};
  const int ng = spec.nghost;
  ...
  if (FaceNeedsBC(pmb, BoundaryFace::inner_x1)) {
    PhysicalBC bc = spec.physical_bc[static_cast<int>(BoundaryFace::inner_x1)];
    ApplyPhysicalBCFace_FC(fc, BoundaryFace::inner_x1,
                           is, ie, bjs, bje, bks, bke, ng, bc);
  }
  if (FaceNeedsBC(pmb, BoundaryFace::outer_x1)) {
    PhysicalBC bc = spec.physical_bc[static_cast<int>(BoundaryFace::outer_x1)];
    ApplyPhysicalBCFace_FC(fc, BoundaryFace::outer_x1,
                           is, ie, bjs, bje, bks, bke, ng, bc);
  }
  ...
  if (pmb->block_size.nx2 > 1) {
    if (FaceNeedsBC(pmb, BoundaryFace::inner_x2)) {
      PhysicalBC bc = spec.physical_bc[static_cast<int>(BoundaryFace::inner_x2)];
      ApplyPhysicalBCFace_FC(fc, BoundaryFace::inner_x2,
                             bis, bie, js, je, bks, bke, ng, bc);
    }
    if (FaceNeedsBC(pmb, BoundaryFace::outer_x2)) {
      PhysicalBC bc = spec.physical_bc[static_cast<int>(BoundaryFace::outer_x2)];
      ApplyPhysicalBCFace_FC(fc, BoundaryFace::outer_x2,
                             bis, bie, js, je, bks, bke, ng, bc);
    }
  }
  ...
  if (pmb->block_size.nx3 > 1) {
    ...
    if (FaceNeedsBC(pmb, BoundaryFace::inner_x3)) {
      PhysicalBC bc = spec.physical_bc[static_cast<int>(BoundaryFace::inner_x3)];
      ApplyPhysicalBCFace_FC(fc, BoundaryFace::inner_x3,
                             bis, bie, bjs, bje, ks, ke, ng, bc);
    }
    if (FaceNeedsBC(pmb, BoundaryFace::outer_x3)) {
      PhysicalBC bc = spec.physical_bc[static_cast<int>(BoundaryFace::outer_x3)];
      ApplyPhysicalBCFace_FC(fc, BoundaryFace::outer_x3,
                             bis, bie, bjs, bje, ks, ke, ng, bc);
    }
  }
}
...
void ApplyPhysicalBCsOnCoarseLevel_FC(MeshBlock *pmb, const CommSpec &spec,
                                      ...
                                      int cks, int cke, int cng) {
  if (!spec.HasAnyPhysicalBC()) return;
  // FC coarse data stored as three independent AthenaArray pointers; null check on first.
  if (spec.coarse_fc[0] == nullptr) return;

  AthenaArray<Real> *cfc[3] = {spec.coarse_fc[0], spec.coarse_fc[1], spec.coarse_fc[2]};
  ...
  if (FaceNeedsBC(pmb, BoundaryFace::inner_x1)) {
    PhysicalBC bc = spec.physical_bc[static_cast<int>(BoundaryFace::inner_x1)];
    ApplyPhysicalBCFace_FC(cfc, BoundaryFace::inner_x1,
                           cis, cie, bjs, bje, bks, bke, cng, bc);
  }
  if (FaceNeedsBC(pmb, BoundaryFace::outer_x1)) {
    PhysicalBC bc = spec.physical_bc[static_cast<int>(BoundaryFace::outer_x1)];
    ApplyPhysicalBCFace_FC(cfc, BoundaryFace::outer_x1,
                           cis, cie, bjs, bje, bks, bke, cng, bc);
  }
  ...
  if (pmb->block_size.nx2 > 1) {
    if (FaceNeedsBC(pmb, BoundaryFace::inner_x2)) {
      PhysicalBC bc = spec.physical_bc[static_cast<int>(BoundaryFace::inner_x2)];
      ApplyPhysicalBCFace_FC(cfc, BoundaryFace::inner_x2,
                             bis, bie, cjs, cje, bks, bke, cng, bc);
    }
    if (FaceNeedsBC(pmb, BoundaryFace::outer_x2)) {
      PhysicalBC bc = spec.physical_bc[static_cast<int>(BoundaryFace::outer_x2)];
      ApplyPhysicalBCFace_FC(cfc, BoundaryFace::outer_x2,
                             bis, bie, cjs, cje, bks, bke, cng, bc);
    }
  }
  ...
  if (pmb->block_size.nx3 > 1) {
    ...
    if (FaceNeedsBC(pmb, BoundaryFace::inner_x3)) {
      PhysicalBC bc = spec.physical_bc[static_cast<int>(BoundaryFace::inner_x3)];
      ApplyPhysicalBCFace_FC(cfc, BoundaryFace::inner_x3,
                             bis, bie, bjs, bje, cks, cke, cng, bc);
    }
    if (FaceNeedsBC(pmb, BoundaryFace::outer_x3)) {
      PhysicalBC bc = spec.physical_bc[static_cast<int>(BoundaryFace::outer_x3)];
      ApplyPhysicalBCFace_FC(cfc, BoundaryFace::outer_x3,
                             bis, bie, bjs, bje, cks, cke, cng, bc);
    }
  }
}
