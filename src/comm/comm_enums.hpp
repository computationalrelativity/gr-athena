...
// Determines index arithmetic, buffer sizes, and which prolong/restrict family
// to use.
...
//----------------------------------------------------------------------------------------
// Communication targets: which neighbor relationships a channel participates
// in. Bitwise-OR combinable.  A channel registered with (SameLevel |
// ToCoarser) will only communicate with same-level and coarser neighbors; the
// call-site filter can only narrow this set, never widen it.

enum CommTarget : std::uint32_t
{
  SameLevel = 1u << 0,
  ToCoarser =
    1u << 1,          // this block sends restricted data to a coarser neighbor
...
//----------------------------------------------------------------------------------------
// Communication group: variables in the same group on the same MeshBlock are
// fused into one MPI message per neighbor.  Each group is a separate fused
// message.
...
enum class CommGroup : int
{
  ...
  M1Rescatter = 8,  // M1 re-scatter: hydro cons + scalars (no B-field)
  NumGroups   = 9   // sentinel - number of groups
};
...
enum class ProlongOp : int
{
  ...
  LagrangeUniform =
    1,  // VC: symmetric Lagrange on uniform grid, inject+interp
...
// These are the conditions that require ghost-zone filling by function
// evaluation (as opposed to block-to-block or periodic, which are handled by
// communication).
...
// Used in component_groups to determine sign flips across coordinate
// boundaries (e.g. polar axis, reflecting walls).  Empty component_groups =
// all even parity (no flips).
...
enum class GeomType : int
{
  ...
  SymTensor =
    2  // 6 independent components, sign flips from tensor transformation
};

//----------------------------------------------------------------------------------------
// Flux correction mode: how received flux correction data is applied on the
// coarse side. Stored per-channel in CommSpec; determines unpack behaviour in
// CommChannel.
...
enum class FluxCorrMode : int
{
  ...
  OverwriteFromFiner =
    1,  // CC: fine-restricted flux overwrites coarse flux at face
  AccumulateAverage =
