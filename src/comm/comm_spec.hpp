...
//========================================================================================
//! \file comm_spec.hpp
//  \brief CommSpec: all metadata needed to register a variable for communication.
//
//  One CommSpec per logical variable (e.g. "hydro cons", "Z4c u").  The registry
//  creates a CommChannel from this spec; the spec itself is a pure data struct with
//  no behaviour.
...
//----------------------------------------------------------------------------------------
// Parity component group: describes how a contiguous run of components transforms under
// coordinate reflections (needed for reflecting BCs and polar boundaries).
//
// Example for Z4c:  component_groups = {{GeomType::SymTensor, 6}, {GeomType::Vector, 3}}
// means components 0-5 are a symmetric 3-tensor, 6-8 are a 3-vector.
// Empty vector = all even parity (scalar-like, no sign flips).
...
using PhysicalBCFn = void (*)(
    MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &u,
    Real time, Real dt,
    int is, int ie, int js, int je, int ks, int ke, int ngh);

//----------------------------------------------------------------------------------------
//! \struct CommSpec
//  \brief Complete specification for registering a variable with the comm system.
...
struct CommSpec {
  ...
  //     (var_fc[0]=x1f, var_fc[1]=x2f, var_fc[2]=x3f).  var/coarse_var ignored.
  AthenaArray<Real> *var;       // fine-level state array (CC/VC/CX only)
  AthenaArray<Real> *coarse_var; // coarse buffer (CC/VC/CX only)
  ...
  PhysicalBCFn physical_bc_fn; // custom function (only used when physical_bc[f] == User)

  // --- parity / sign flips ---
  std::vector<ComponentGroup> component_groups;
  // Empty = all even parity (no sign flips).
  // Non-empty = describes how consecutive component runs transform geometrically.
  ...
  bool HasAnyPhysicalBC() const {
    for (int f = 0; f < 6; ++f)
      if (physical_bc[f] != PhysicalBC::None) return true;
    return false;
  }
};
...
inline PhysicalBC MapBoundaryFlag(BoundaryFlag bf) {
  switch (bf) {
    case BoundaryFlag::reflect:             return PhysicalBC::Reflect;
    case BoundaryFlag::outflow:             return PhysicalBC::Outflow;
    case BoundaryFlag::extrapolate_outflow: return PhysicalBC::ExtrapolateOutflow;
    case BoundaryFlag::gr_sommerfeld:       return PhysicalBC::GRSommerfeld;
    case BoundaryFlag::polar_wedge:         return PhysicalBC::PolarWedge;
    case BoundaryFlag::user:                return PhysicalBC::User;
    default:                                return PhysicalBC::None;
  }
}
