...
//
//  One CommChannel per registered variable per MeshBlock.  Owns its send/recv
//  flat buffers, status flags, and persistent MPI requests.  Does NOT own the
//  state array (that lives in the physics module).
//
...
class CommChannel
{
  ...
  // target_filter narrows the set of neighbors actually communicated (cannot
  // widen).
  void PackAndSend(const NeighborConnectivity& nc, CommTarget target_filter);

  // Poll for received data.  Returns true if all expected receives have
  // arrived.
  ...
  // Test (or wait on) all outstanding sends and reset status flags.
  // When wait=false (LTS non-blocking path), first polls all off-rank sends
  // via MPI_Test; if any send is still pending, returns false WITHOUT
  // resetting any flags.  Only when all sends are confirmed complete does
  // it reset flags.  Safe to call repeatedly until it returns true.
  // When wait=true (GTS blocking path), MPI_Wait guarantees completion and
  // flags are reset in a single pass.
  bool Clear(const NeighborConnectivity& nc,
             CommTarget target_filter,
             bool wait = true);

  // Post persistent receive requests (called before the send/recv cycle).
  void StartReceiving(const NeighborConnectivity& nc,
                      CommTarget target_filter);

  // --- flux correction communication cycle ---
  // These methods operate on flcor_send_buf_/flcor_recv_buf_ and use the flux
  // arrays (spec_.flx_cc or spec_.flx_fc) rather than the variable arrays.
  // Only active when spec_.flcor_mode != FluxCorrMode::None.

  // Pack area-weighted (CC) or edge-length-weighted (FC) restricted fluxes
  // into send buffers, then send to coarser face (CC) or face+edge (FC)
  // neighbors.
  ...
  // Test (or wait on) outstanding flux correction sends and reset flags.
  // When wait=false, first polls all off-rank sends; returns false without
  // resetting flags if any send is still pending.  Flags are only reset
  // after all sends are confirmed complete.
  bool ClearFluxCorr(const NeighborConnectivity& nc, bool wait = true);
  ...
  CommGroup flux_group() const
  {
    return spec_.flux_group;
  }
  FluxCorrMode flcor_mode() const
  {
    return spec_.flcor_mode;
  }
  ...
  // VC node multiplicity (precomputed at Finalize; valid only for
  // Sampling::VC).
  NodeMultiplicity& node_multiplicity()
  {
    return node_mult_;
  }
  const NodeMultiplicity& node_multiplicity() const
  {
    return node_mult_;
  }
  ...
  // Zero-copy helpers: resolve target block's recv buffer pointer for
  // same-rank neighbors so the sender can pack directly into it (eliminating
  // the intermediate send_buf copy + memcpy).
  ...
  // Allocate flcor_send_buf_ / flcor_recv_buf_ for all active flux-correction
  // neighbors.
  ...
  int ComputeFluxCorrBufferSize(const NeighborConnectivity& nc,
                                int nb_idx) const;
  ...
  // FC EMF flux correction pack+send (raw or edge-length-weighted
  // restriction).
  ...
  // FC EMF two-phase poll+unpack state machine (same-level -> clear ->
  // from-finer -> avg).
  bool PollReceiveFluxCorrFC(const NeighborConnectivity& nc);

  // Pack raw EMF values for a same-level face/edge neighbor.
  int LoadFluxBoundaryBufferSameLevel(Real* buf,
                                      const NeighborBlock& nb,
                                      const AthenaArray<Real>& e1,
                                      const AthenaArray<Real>& e2,
                                      const AthenaArray<Real>& e3);

  // Pack edge-length-weighted restricted EMFs for a to-coarser face/edge
  // neighbor.
  int LoadFluxBoundaryBufferToCoarser(Real* buf,
                                      const NeighborBlock& nb,
                                      const AthenaArray<Real>& e1,
                                      const AthenaArray<Real>& e2,
                                      const AthenaArray<Real>& e3,
  ...
  void SetFluxBoundarySameLevel(Real* buf,
                                const NeighborBlock& nb,
                                AthenaArray<Real>& e1,
                                AthenaArray<Real>& e2,
                                AthenaArray<Real>& e3);

  // FC EMF unpack: accumulate from-finer restricted EMFs (quadrant narrowing
  // via fi1/fi2).
  void SetFluxBoundaryFromFiner(Real* buf,
                                const NeighborBlock& nb,
                                AthenaArray<Real>& e1,
                                AthenaArray<Real>& e2,
                                AthenaArray<Real>& e3);
  ...
  // Zero EMFs at fine/coarse interfaces (called between same-level and
  // from-finer unpack phases in the two-phase FC EMF protocol).
  void ClearCoarseFluxBoundary(const NeighborConnectivity& nc);

  // Divide accumulated EMFs by contributor count after both unpack phases
  // complete.
  ...
  // Cached polar sign array: computed once at Finalize() from
  // component_groups. Empty if component_groups is empty (no flips needed).
  std::vector<Real> polar_signs_;

  // VC node multiplicity: precomputed at Finalize() for vertex-centered
  // channels. Unused (empty) for CC/CX/FC.
  ...
  // Per-neighbor flux correction status flags.  Atomic for same-rank
  // concurrency.
  ...
  // exchange because only face and edge neighbors participate).
  int nbmax_flcor_;

  // FC EMF two-phase state: true while receiving same-level contributions,
  // false during from-finer phase.  Only used when flcor_mode ==
  // AccumulateAverage.
  bool recv_flx_same_lvl_;

  // Per-edge flags for FC EMF averaging (matches old bvals
  // edge_flag_[12]/nedge_fine_[12]). edge_flag_[eid]: true if the finest
  // neighbor touching this edge is at the same level
  //   (i.e. no finer neighbor contributes to this edge).
  // nedge_fine_[eid]: number of blocks contributing at the finest level to
  // this edge
  //   (used as the divisor in AverageFluxBoundary).
  bool edge_flag_[12];
  int nedge_fine_[12];

  // Scratch arrays for area weighting during CC flux restriction (x2/x3
  // faces). sarea_[0] and sarea_[1] are 1D arrays of size (nx1 + 2*NGHOST),
  // matching BoundaryBase::sarea_[2] in the old system.
