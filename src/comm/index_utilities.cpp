...
//  The per-dimension helpers (LoadRangeCC, SetRangeCC, etc.) are inlined from
//  the header.
//
...
//      - LoadToCoarser uses nghost as slab width, indices in coarse buffer
//      (cis..cie).
//      - LoadToFiner uses cnghost as slab width, indices in fine array
//      (is..ie).
//    VC:
//      - Fine level: pmb->ng (== NGHOST).
//      - Coarse level: pmb->cng (== NCGHOST).
//      - LoadToCoarser uses pmb->ng as slab width, indices in coarse buffer
//      (civs..cive).
//      - LoadToFiner uses pmb->cng as slab width, indices in fine array
//      (ivs..ive).
//      - Shared vertex included in load/set; unpack is additive (handled by
//      caller).
//    CX:
//      - Fine level: NGHOST.
...
IndexRange3D LoadSameLevel(const MeshBlock* pmb,
  ...
  switch (sampling)
  {
    ...
    case Sampling::VC:
      if (!is_coarse)
      {
        ...
        LoadRangeVC(
          ni.ox1, r.si, r.ei, pmb->ivs, pmb->ive, pmb->igs, pmb->ige);
        LoadRangeVC(
          ni.ox2, r.sj, r.ej, pmb->jvs, pmb->jve, pmb->jgs, pmb->jge);
        LoadRangeVC(
          ni.ox3, r.sk, r.ek, pmb->kvs, pmb->kve, pmb->kgs, pmb->kge);
      }
      else
      {
        // Coarse VC buffer uses coarse ghost-load indices
        LoadRangeVC(
          ni.ox1, r.si, r.ei, pmb->civs, pmb->cive, pmb->cigs, pmb->cige);
        LoadRangeVC(
          ni.ox2, r.sj, r.ej, pmb->cjvs, pmb->cjve, pmb->cjgs, pmb->cjge);
        LoadRangeVC(
          ni.ox3, r.sk, r.ek, pmb->ckvs, pmb->ckve, pmb->ckgs, pmb->ckge);
      }
      break;
    ...
    case Sampling::FC:
      if (!is_coarse)
      {
        ...
        int ng = nghost;
        LoadRangeFC(
          ni.ox1, r.si, r.ei, pmb->is, pmb->ie, ng, stagger_axis == 0, true);
        LoadRangeFC(ni.ox2,
                    r.sj,
                    r.ej,
                    pmb->js,
        ...
        // Edge/corner overlap: re-include the shared boundary face for
        // multilevel non-face neighbors.  Without this the transverse
        // ghost-zone strip of the boundary face (e.g. B1 at i=ie+1, j>je) is
        // never communicated, leaving stale data that corrupts ComputeCornerE
        // and hence the CT update, breaking div(B).
        //
        // In the old code this was disabled because the overlapping values
        // could overwrite EMF-corrected boundary faces and spike div(B) after
        // F2C restriction.  The new comm system re-restricts in
        // ProlongateAndApplyPhysicalBCs (after unpack, before prolongation),
        // so the authoritative fine-level restricted values always win.
        if (pmb->pmy_mesh->multilevel && ni.type != NeighborConnect::face)
        {
          if (stagger_axis == 0)
          {
            if (ni.ox1 > 0)
      ...
      {
        // Coarse same-level payload: uses cnghost with degenerate-dimension
        // scaling
        int cng = pmb->cnghost;
    ...
    default:
    {
      ...
      msg << "### FATAL ERROR in LoadSameLevel: unsupported Sampling type"
          << std::endl;
      ATHENA_ERROR(msg);
    }
  }
...
IndexRange3D SetSameLevel(const MeshBlock* pmb,
                          const NeighborIndexes& ni,
                          int nghost,
                          int type,
                          Sampling sampling,
  ...
  switch (sampling)
  {
    ...
    case Sampling::VC:
      if (type == 1)
      {
        ...
        SetRangeVC(ni.ox1, r.si, r.ei, pmb->ivs, pmb->ive, pmb->ims, pmb->ipe);
        SetRangeVC(ni.ox2, r.sj, r.ej, pmb->jvs, pmb->jve, pmb->jms, pmb->jpe);
        SetRangeVC(ni.ox3, r.sk, r.ek, pmb->kvs, pmb->kve, pmb->kms, pmb->kpe);
      }
      else
      {
        // Coarse VC buffer
        SetRangeVC(
          ni.ox1, r.si, r.ei, pmb->civs, pmb->cive, pmb->cims, pmb->cipe);
        SetRangeVC(
          ni.ox2, r.sj, r.ej, pmb->cjvs, pmb->cjve, pmb->cjms, pmb->cjpe);
        SetRangeVC(
          ni.ox3, r.sk, r.ek, pmb->ckvs, pmb->ckve, pmb->ckms, pmb->ckpe);
      }
      break;
    ...
    case Sampling::FC:
      if (type == 1)
      {
        ...
        SetRangeFC(ni.ox3,
                   ...
                   stagger_axis == 2,
                   pmb->block_size.nx3 > 1);
        // Edge/corner overlap (Set side): re-include the shared boundary
        // face on the ghost-zone side (direction opposite from Load).
        // See the matching comment in LoadSameLevel FC for the full
        // explanation.
        if (pmb->pmy_mesh->multilevel && ni.type != NeighborConnect::face)
        {
          if (stagger_axis == 0)
    ...
    {
      std::stringstream msg;
      msg << "### FATAL ERROR in SetSameLevel: unsupported Sampling type"
          << std::endl;
      ATHENA_ERROR(msg);
    }
  }
...
//========================================================================================
// LoadToCoarser: pack source indices from the coarse buffer for sending to a
// coarser neighbor.
...
IndexRange3D LoadToCoarser(const MeshBlock* pmb,
                           const NeighborIndexes& ni,
                           int nghost,
                           Sampling sampling,
                           int stagger_axis)
{
  ...
  switch (sampling)
  {
    ...
    case Sampling::FC:
    {
      ...
      int cng = nghost;
      LoadRangeFC(
        ni.ox1, r.si, r.ei, pmb->cis, pmb->cie, cng, stagger_axis == 0, true);
      LoadRangeFC(ni.ox2,
                  r.sj,
                  r.ej,
                  pmb->cjs,
    ...
    {
      std::stringstream msg;
      msg << "### FATAL ERROR in LoadToCoarser: unsupported Sampling type"
          << std::endl;
      ATHENA_ERROR(msg);
    }
  }
...
//========================================================================================
// SetFromCoarser: unpack destination indices into the coarse buffer when
// receiving from a coarser neighbor.
//   CC: uses cnghost as ghost extension with lx-parity.
//   VC: uses precomputed coarse ghost ranges (cims/cime/cips/cipe) with
//   lx-parity. CX: uses precomputed coarse CX ghost ranges (cx_cims/etc.) with
//   lx-parity.
//========================================================================================

IndexRange3D SetFromCoarser(const MeshBlock* pmb,
                            const NeighborIndexes& ni,
                            int nghost,
                            Sampling sampling,
                            int stagger_axis)
{
  ...
  switch (sampling)
  {
    ...
    {
      int cng = pmb->cnghost;
      SetFromCoarserRangeCC(
        ni.ox1, r.si, r.ei, pmb->cis, pmb->cie, cng, pmb->loc.lx1, true);
      SetFromCoarserRangeCC(ni.ox2,
                            r.sj,
                            r.ej,
                            pmb->cjs,
                            pmb->cje,
    ...
    case Sampling::FC:
    {
      // FC uses cnghost with stagger-aware per-dimension helper, then parity
      // extend
      int cng = pmb->cnghost;
      SetFromCoarserRangeFC(ni.ox1,
                            r.si,
                            r.ei,
                            pmb->cis,
                            pmb->cie,
    ...
    {
      std::stringstream msg;
      msg << "### FATAL ERROR in SetFromCoarser: unsupported Sampling type"
          << std::endl;
      ATHENA_ERROR(msg);
    }
  }
...
//========================================================================================
// LoadToFiner: pack source indices from the fine array for sending to a finer
// neighbor.
//   CC: slab width = cnghost, indices in fine array (is..ie).
//   VC: slab width = cng, indices in fine VC array (ivs..ive), excludes
//   boundary vertex. CX: uses cx_dng offset, half_size = NCGHOST_CX.
//
...
IndexRange3D LoadToFiner(const MeshBlock* pmb,
                         const NeighborIndexes& ni,
                         int nghost,
                         Sampling sampling,
                         int stagger_axis)
{
  ...
  switch (sampling)
  {
    ...
    case Sampling::FC:
    {
      // FC: slab width = cnghost; stagger dim includes boundary face (span =
      // cng+1).
      int cng = pmb->cnghost;
    ...
    {
      std::stringstream msg;
      msg << "### FATAL ERROR in LoadToFiner: unsupported Sampling type"
          << std::endl;
      ATHENA_ERROR(msg);
    }
  }
...
//========================================================================================
// SetFromFiner: unpack destination indices into the fine array when receiving
// from a finer neighbor.
//   CC: ghost width = nghost, half_size = nx/2.
//   VC: includes shared vertex, half_size = nx/2.
//   CX: gap at boundary, half_size = nx/2.
//========================================================================================

IndexRange3D SetFromFiner(const MeshBlock* pmb,
                          const NeighborIndexes& ni,
                          int nghost,
                          Sampling sampling,
                          int stagger_axis)
{
  ...
  switch (sampling)
  {
    ...
    case Sampling::VC:
    {
      // VC includes shared vertex in set-from-finer (additive unpack done by
      // caller)
      int hs = pmb->block_size.nx1 / 2;

      SetFromFinerRangeVC(ni.ox1,
                          r.si,
                          r.ei,
                          pmb->ivs,
                          pmb->ive,
    ...
    case Sampling::FC:
    {
      ...
      // half_size = nx/2 (SetFromFiner uses fine-level half, not
      // cng-adjusted).
      int ng  = nghost;
      int nx1 = pmb->block_size.nx1;
      int nx2 = pmb->block_size.nx2;
      ...
      SetFromFinerRangeFC(ni.ox1,
                          ...
                          true,
                          stagger_axis == 0);
      // Edge/corner overlap for stagger_axis==0: direction opposite from
      // LoadToCoarser
    ...
    {
      std::stringstream msg;
      msg << "### FATAL ERROR in SetFromFiner: unsupported Sampling type"
          << std::endl;
      ATHENA_ERROR(msg);
    }
  }
...
//========================================================================================
// ComputeBufferSizeFromRanges: compute the maximum buffer size needed for one
// neighbor by computing actual index ranges for all communication patterns.
//
// This replaces the CC-only analytical formula and works for all samplings.
// For multilevel, takes the max over same-level (fine+coarse), to-coarser,
// to-finer, from-coarser, and from-finer buffer sizes, mirroring the old
// per-subclass NeighborVariableBufferSize() logic.
//========================================================================================
...
int ComputeBufferSizeFromRanges(const MeshBlock* pmb,
                                const NeighborIndexes& ni,
                                int nvar,
                                int nghost,
                                Sampling sampling)
{
  ...
  if (multilevel)
  {
    // Same-level: coarse payload (sent alongside fine for cross-level
    // neighbors)
...
//========================================================================================
// ComputeMPIBufferSize: compute the MPI message count for one neighbor,
// potentially smaller than the buffer allocation size when the coarse payload
// is not needed.
//
// When skip_coarse is true and the mesh is multilevel, the same-level size
// omits the coarse payload.  Cross-level sizes (to-coarser, to-finer) are
// unchanged because they are never sent to same-level neighbors.  The final
// result is still the max over all level cases - the persistent MPI request
// must accommodate the largest message this neighbor slot can ever carry
// (same-level, to-coarser, or to-finer after regrid).
//
// Sender calls with skip_coarse = nb.neighbor_all_same_level (receiver's
// flag). Receiver calls with skip_coarse = pmb->NeighborBlocksSameLevel() (own
// flag).
//========================================================================================

int ComputeMPIBufferSize(const MeshBlock* pmb,
                         const NeighborIndexes& ni,
                         int nvar,
                         int nghost,
                         Sampling sampling,
                         bool skip_coarse)
{
  ...
  // If not multilevel, or not skipping coarse, fall back to full allocation
  // size.
...
// Each face neighbor exchanges nvar * (nx_perp1/2) * (nx_perp2/2) restricted
// flux values, where the division is integer (fine-to-coarse 2:1 restriction
// in the transverse plane). Non-face neighbors return 0 - CC flux correction
// is face-only.
//========================================================================================

int ComputeFluxCorrBufferSizeCC(const MeshBlock* pmb,
                                const NeighborIndexes& ni,
                                int nvar)
{
  if (ni.type != NeighborConnect::face)
  ...
  // Transverse sizes after 2:1 restriction.  (nx+1)/2 handles the integer
  // division.
...
// Face neighbors carry two tangential EMF components; edge neighbors carry one
// parallel EMF component.  Corner neighbors return 0.
//
// For same-level exchange the full fine-grid EMF array is packed (no
// restriction). For to-coarser the stride-2 restricted size is half, but we
// allocate the maximum, which is the same-level (unrestricted) size.
//========================================================================================

int ComputeFluxCorrBufferSizeFC(const MeshBlock* pmb,
                                const NeighborIndexes& ni)
{
  int nx1 = pmb->block_size.nx1;
  int nx2 = pmb->block_size.nx2;
  ...
  if (ni.type == NeighborConnect::face)
  {
    // Two tangential EMF components.  Each component has stagger-offset +1 in
    // its own direction on the face (edges span one more than cells in that
    // direction).
    if (ni.ox1 != 0)
    {
      // x1-face: tangential = e2 (size nx3 x (nx2+1)) + e3 (size (nx3+1) x
      // nx2) For 2D (nx3==1): e2 -> 1*(nx2+1), e3 -> 1*nx2;  for 1D: 2 (one
      // per component)
    ...
    {
      // x2-face: tangential = e1 (size (nx1+1) x nx3) + e3 (size nx1 x
      // (nx3+1))
    ...
    {
      // x3-face: tangential = e1 (size (nx1+1) x nx2) + e2 (size nx1 x
      // (nx2+1))
      return (nx1 + 1) * nx2 + nx1 * (nx2 + f2);
    }
  }
