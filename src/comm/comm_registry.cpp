...
#ifdef DBG_FUSED_COMM
  if (fused_[g].active) { StartReceivingFused(group, target_filter); return; }
#endif
...
#ifdef DBG_FUSED_COMM
  if (fused_[g].active) { SendBoundaryBuffersFused(group, target_filter); return; }
#endif
...
bool CommRegistry::ReceiveBoundaryBuffers(CommGroup group, CommTarget target_filter) {
  int g = static_cast<int>(group);
#ifdef DBG_FUSED_COMM
  if (fused_[g].active) { return ReceiveBoundaryBuffersFused(group, target_filter); }
#endif
  bool all_done = true;
  for (int id : group_channels_[g]) {
...
void CommRegistry::SetBoundaries(CommGroup group, CommTarget target_filter) {
  int g = static_cast<int>(group);
#ifdef DBG_FUSED_COMM
  if (fused_[g].active) { SetBoundariesFused(group, target_filter); return; }
#endif
...
void CommRegistry::ClearBoundary(CommGroup group, CommTarget target_filter) {
  int g = static_cast<int>(group);
#ifdef DBG_FUSED_COMM
  if (fused_[g].active) { ClearBoundaryFused(group, target_filter); return; }
#endif
  for (int id : group_channels_[g]) {
...
//
// All channels within a CommGroup are packed sequentially into a single contiguous
// buffer per neighbor.  A single MPI message is sent/received instead of one per
// channel, reducing MPI message count by ~3x for MainInt.
//
// Same-rank neighbors use zero-copy: the sender packs directly into the target's
// fused recv_buf[targetid], then atomically sets recv_flag[targetid] to arrived.
//========================================================================================

//----------------------------------------------------------------------------------------
// Tag construction helpers (duplicated from comm_channel.cpp's file-static functions
// because those are not visible outside that translation unit).
...
static int FusedMakeTag(int lid, int bufid, int channel_id,
...
//----------------------------------------------------------------------------------------
// FreeFusedMPIRequests: release all fused persistent MPI requests for one group.
...
//----------------------------------------------------------------------------------------
// ReinitializeFused: teardown fused state for all active groups before regrid rebuild.
...
//----------------------------------------------------------------------------------------
// FinalizeFused: allocate fused buffers, set up persistent MPI, resolve zero-copy.
//
// Called after all per-channel Finalize() calls are complete.  For each group with
// >= 2 channels, computes fused buffer sizes as the sum of per-channel sizes, then
// allocates buffers and creates persistent MPI requests.
...
void CommRegistry::FinalizeFused() {
  ...
  for (int g = 0; g < static_cast<int>(CommGroup::NumGroups); ++g) {
    ...
    // Buffer allocation size per neighbor = sum of per-channel allocation sizes.
    // Same as per-channel: send_buf is only allocated for off-rank neighbors.
...
#ifdef MPI_PARALLEL
    // Fused messages reuse the first channel's ID as the tag's channel_id field.
    // Safe because per-channel MPI for fused groups is never started when
    // DBG_FUSED_COMM is active.
    ...
    for (int n = 0; n < nc_.num_neighbors(); ++n) {
      ...
      int recv_size = 0;
      for (int id : chids) {
        if (!HasTarget(channels_[id].spec().targets, nbt)) continue;
      ...
      // Send tag: keyed on receiver's lid and targetid (matching per-channel convention).
      int stag = FusedMakeTag(nb.snb.lid, nb.targetid, fused_ch_id,
                              bufid_bits, channel_bits);
      if (fs.req_send[nb.bufid] != MPI_REQUEST_NULL)
        MPI_Request_free(&fs.req_send[nb.bufid]);
      MPI_Send_init(fs.send_buf[nb.bufid], send_size, MPI_ATHENA_REAL,
                     nb.snb.rank, stag, MPI_COMM_WORLD, &fs.req_send[nb.bufid]);

      // Recv tag: keyed on this block's lid and bufid.
      int rtag = FusedMakeTag(pmb->lid, nb.bufid, fused_ch_id,
                              bufid_bits, channel_bits);
      if (fs.req_recv[nb.bufid] != MPI_REQUEST_NULL)
        MPI_Request_free(&fs.req_recv[nb.bufid]);
      MPI_Recv_init(fs.recv_buf[nb.bufid], recv_size, MPI_ATHENA_REAL,
                     nb.snb.rank, rtag, MPI_COMM_WORLD, &fs.req_recv[nb.bufid]);
    }
#endif  // MPI_PARALLEL
...
//
// Restriction has already been called by SendBoundaryBuffers before reaching here.
// For each neighbor, packs all channels sequentially into the fused send buffer
// (or directly into the target's fused recv buffer for same-rank zero-copy).
...
void CommRegistry::SendBoundaryBuffersFused(CommGroup group,
  ...
  // Compute effective target for each channel (intersection of spec targets and filter).
  // All channels in the group must agree on which neighbors to communicate with;
  // the target_filter narrows but cannot widen.

  for (int n = 0; n < nc_.num_neighbors(); ++n) {
    const NeighborBlock &nb = nc_.neighbor(n);
    CommTarget nbt = FusedNeighborTarget(mylevel, nb.snb.level);

    // Check if any channel in the group wants to communicate with this neighbor.
    // (All channels in a fused group should have compatible targets, but be safe.)
    bool any_active = false;
    for (int id : chids) {
      CommTarget eff = channels_[id].spec().targets & target_filter;
      if (HasTarget(eff, nbt)) { any_active = true; break; }
    }
...
bool CommRegistry::ReceiveBoundaryBuffersFused(CommGroup group,
  ...
  for (int n = 0; n < nc_.num_neighbors(); ++n) {
    ...
    // Must match the same filter logic as Send - only poll neighbors that were expected.
    bool any_active = false;
    for (int id : group_channels_[g]) {
      CommTarget eff = channels_[id].spec().targets & target_filter;
      if (HasTarget(eff, nbt)) { any_active = true; break; }
...
// Handles VC zero-ghost / additive-unpack / divide protocol exactly as the unfused path.

void CommRegistry::SetBoundariesFused(CommGroup group, CommTarget target_filter) {
  int g = static_cast<int>(group);
  MeshBlock *pmb = pmy_block_;
  const int mylevel = pmb->loc.level;
  FusedGroupState &fs = fused_[g];
  ...
  // Phase 1: zero VC ghost zones before additive unpack (same as unfused path).
  for (int id : chids) {
    CommChannel &ch = channels_[id];
    if (ch.spec().sampling != Sampling::VC) continue;
  ...
  // Phase 2: for each neighbor, unpack all channels sequentially from the fused buffer.
  for (int n = 0; n < nc_.num_neighbors(); ++n) {
    const NeighborBlock &nb = nc_.neighbor(n);
    CommTarget nbt = FusedNeighborTarget(mylevel, nb.snb.level);
  ...
  // Phase 3: divide VC arrays by accumulated node multiplicity (same as unfused path).
  for (int id : chids) {
    CommChannel &ch = channels_[id];
    if (ch.spec().sampling != Sampling::VC) continue;
...
void CommRegistry::ClearBoundaryFused(CommGroup group, CommTarget target_filter) {
  MeshBlock *pmb = pmy_block_;
  const int mylevel = pmb->loc.level;
  int g = static_cast<int>(group);
  FusedGroupState &fs = fused_[g];
...
void CommRegistry::StartReceivingFused(CommGroup group, CommTarget target_filter) {
#ifdef MPI_PARALLEL
  MeshBlock *pmb = pmy_block_;
  const int mylevel = pmb->loc.level;
  int g = static_cast<int>(group);
