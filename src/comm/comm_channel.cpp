...
//  \brief CommChannel implementation: buffer lifecycle, MPI setup, pack/unpack.
...
//----------------------------------------------------------------------------------------
// Tag construction.
// Layout: (lid << (bufid_bits + channel_bits)) | (bufid << channel_bits) | channel_id
// Bit widths are computed from actual counts; validated against MPI_TAG_UB.

static int MakeTag(int lid, int bufid, int channel_id,
                   int bufid_bits, int channel_bits) {
  return (lid << (bufid_bits + channel_bits))
       | (bufid << channel_bits)
       | channel_id;
}
...
// flux-correction tags occupy disjoint channel_id ranges within the same bit layout.
...
CommChannel::CommChannel(const CommSpec &spec, MeshBlock *pmb, int channel_id)
  ...
  for (int n = 0; n < kMaxNeighbor; ++n) {
    ...
    flcor_recv_flag_[n].store(BoundaryStatus::waiting, std::memory_order_relaxed);
    flcor_send_flag_[n].store(BoundaryStatus::waiting, std::memory_order_relaxed);
#ifdef MPI_PARALLEL
...
void CommChannel::Finalize(const NeighborConnectivity &nc, int max_channel_id) {
  ...
  if (spec_.flcor_mode != FluxCorrMode::None) {
    ...
    // Flux correction needs scratch arrays for area/edge-length weighting (CC and FC).
    if (spec_.flcor_mode != FluxCorrMode::None) {
...
void CommChannel::Reinitialize(const NeighborConnectivity &nc, int max_channel_id) {
  FreeFluxCorrMPIRequests();
  FreeFluxCorrBuffers();
  FreeMPIRequests();
  FreeBuffers();
...
// Delegates to idx::ComputeBufferSizeFromRanges(), which computes actual index ranges
// for all communication patterns (same-level, to-coarser, to-finer) and takes the max.
// This works correctly for all samplings (CC, VC, CX) because the per-dimension helpers
// in index_utilities use sampling-specific base indices from MeshBlock.

int CommChannel::ComputeBufferSize(const NeighborConnectivity &nc, int nb_idx) const {
  const NeighborBlock &nb = nc.neighbor(nb_idx);
  return idx::ComputeBufferSizeFromRanges(pmy_block_, nb.ni,
...
void CommChannel::AllocateBuffers(const NeighborConnectivity &nc) {
  ...
  for (int n = 0; n < nc.num_neighbors(); ++n) {
    ...
    recv_flag_[nb.bufid].store(BoundaryStatus::waiting, std::memory_order_relaxed);
    send_flag_[nb.bufid].store(BoundaryStatus::waiting, std::memory_order_relaxed);
  }
}
...
void CommChannel::SetupPersistentMPI(const NeighborConnectivity &nc,
  ...
  // flux-correction ids [kFluxCorrTagChannelOffset, max_channel_id + kFluxCorrTagChannelOffset]
  // so that all MPI tags share the same bit layout and cannot collide.
  // bufid max is kMaxNeighbor - 1 = 55.
  const int channel_bits = BitsNeeded(max_channel_id + kFluxCorrTagChannelOffset);
  ...
  for (int n = 0; n < nc.num_neighbors(); ++n) {
    const NeighborBlock &nb = nc.neighbor(n);
    if (nb.snb.rank == Globals::my_rank) continue;  // same-rank: no MPI

    // Buffer allocation size is the max over all level cases (same, to-coarser,
    // to-finer).  MPI message sizes may be smaller when the coarse payload can
    // be skipped for same-level neighbors in uniform-resolution regions.
    //
    // Send size: skip coarse payload if the *receiver* has all same-level neighbors
    //   (nb.neighbor_all_same_level) - it will never prolong, never read coarse data.
    // Recv size: skip coarse payload if *this block* has all same-level neighbors
    //   (pmb->NeighborBlocksSameLevel()) - we will never prolong, never read coarse data.
    //
    // Cross-level sizes are unaffected (the flag only suppresses the same-level
    // coarse payload; to-coarser/to-finer paths are unchanged).
    int ssize = idx::ComputeMPIBufferSize(pmb, nb.ni, spec_.nvar, spec_.nghost,
                                          spec_.sampling,
                                          nb.neighbor_all_same_level);
    int rsize = idx::ComputeMPIBufferSize(pmb, nb.ni, spec_.nvar, spec_.nghost,
                                          spec_.sampling,
                                          pmb->NeighborBlocksSameLevel());

    // Send tag: receiver's (lid, targetid, channel_id).
    int tag = MakeTag(nb.snb.lid, nb.targetid, channel_id_,
                      bufid_bits, channel_bits);
    if (req_send_[nb.bufid] != MPI_REQUEST_NULL)
      MPI_Request_free(&req_send_[nb.bufid]);
    MPI_Send_init(send_buf_[nb.bufid], ssize, MPI_ATHENA_REAL,
                  nb.snb.rank, tag, MPI_COMM_WORLD, &req_send_[nb.bufid]);

    // Recv tag: this block's (lid, bufid, channel_id).
    tag = MakeTag(pmb->lid, nb.bufid, channel_id_,
                  bufid_bits, channel_bits);
    if (req_recv_[nb.bufid] != MPI_REQUEST_NULL)
      MPI_Request_free(&req_recv_[nb.bufid]);
    MPI_Recv_init(recv_buf_[nb.bufid], rsize, MPI_ATHENA_REAL,
                  nb.snb.rank, tag, MPI_COMM_WORLD, &req_recv_[nb.bufid]);
  }
#endif
}
...
static CommTarget NeighborTarget(int my_level, int nb_level) {
  if (nb_level == my_level) return CommTarget::SameLevel;
  if (nb_level < my_level)  return CommTarget::ToCoarser;
  return CommTarget::ToFiner;
}

//----------------------------------------------------------------------------------------
// Pack data from state array into send buffers, then initiate sends.
// For each neighbor, select the correct index range based on relative refinement level:
...
void CommChannel::PackAndSend(const NeighborConnectivity &nc,
  ...
  if (samp == Sampling::FC) {
    ...
    for (int n = 0; n < nc.num_neighbors(); ++n) {
      ...
      Real *buf = same_rank ? ResolveTargetRecvBuffer(n)
                            : send_buf_[nb.bufid];
      int p = 0;
      ...
      if (nb.snb.level == mylevel) {
        ...
        for (int sa = 0; sa < 3; ++sa) {
          idx::IndexRange3D r = idx::LoadSameLevel(pmb, nb.ni, ngh, false,
                                                   samp, sa);
        ...
        if (pmb->pmy_mesh->multilevel && spec_.coarse_fc[0] != nullptr
          ...
          AthenaArray<Real> *cface[3] = {spec_.coarse_fc[0], spec_.coarse_fc[1],
                                         spec_.coarse_fc[2]};
          for (int sa = 0; sa < 3; ++sa) {
            idx::IndexRange3D cr = idx::LoadSameLevel(pmb, nb.ni, ngh, true,
                                                      samp, sa);
  ...
  for (int n = 0; n < nc.num_neighbors(); ++n) {
    ...
    send_flag_[nb.bufid].store(BoundaryStatus::completed, std::memory_order_release);
  }
}

//----------------------------------------------------------------------------------------
// Poll for received data.  Returns true when all expected receives have arrived.
...
bool CommChannel::PollReceive(const NeighborConnectivity &nc,
  ...
  for (int n = 0; n < nc.num_neighbors(); ++n) {
    ...
    // Already arrived?
    if (recv_flag_[nb.bufid].load(std::memory_order_acquire) == BoundaryStatus::arrived)
      continue;
    ...
    if (nb.snb.rank != Globals::my_rank) {
      ...
      if (test_flag)
        recv_flag_[nb.bufid].store(BoundaryStatus::arrived, std::memory_order_release);
    }
#endif
...
// For each neighbor, select the correct destination based on relative refinement level:
...
//
// When nb.polar is true, the j-index loop is reversed and per-component parity sign
// flips are applied - matching the old system's polar boundary handling exactly.
//
// VC additive unpack: vertex-centered data uses UnpackDataAdd for same-level (fine +
// coarse) and from-finer cases, because shared vertices accumulate contributions from
// multiple neighbors.  From-coarser uses plain UnpackData (overwrites coarse buffer).
// The caller (CommRegistry::SetBoundaries) handles ZeroGhosts before and ApplyDivision
// after, using the precomputed NodeMultiplicity.

// Helper: unpack buffer into array with reversed j-order and sign flips.
// Mirrors the old SetBoundarySameLevel polar branch.
static void UnpackPolar(Real *buf, AthenaArray<Real> &var, int nvar,
  ...
  for (int nn = 0; nn < nvar; ++nn) {
    ...
    for (int k = sk; k <= ek; ++k) {
      for (int j = ej; j >= sj; --j) {  // reversed j
#pragma omp simd
        for (int i = si; i <= ei; ++i) {
...
static void UnpackPolarFC(Real *buf, AthenaArray<Real> &arr, Real sign,
                          int si, int ei, int sj, int ej, int sk, int ek,
                          int &p) {
  // Buffer index computed from loop variables to avoid loop-carried dependency.
  const int ni = ei - si + 1;
...
void CommChannel::Unpack(const NeighborConnectivity &nc,
  ...
  if (samp == Sampling::FC) {
    ...
    for (int n = 0; n < nc.num_neighbors(); ++n) {
      ...
      if (nb.snb.level == mylevel) {
        ...
        for (int sa = 0; sa < 3; ++sa) {
          idx::IndexRange3D r = idx::SetSameLevel(pmb, nb.ni, ngh, 1,
                                                  samp, sa);
          if (nb.polar) {
            UnpackPolarFC(recv_buf_[nb.bufid], *face[sa], fc_polar_sign[sa],
                          r.si, r.ei, r.sj, r.ej, r.sk, r.ek, p);
          } else {
            BufferUtility::UnpackData(recv_buf_[nb.bufid], *face[sa],
                                      r.si, r.ei, r.sj, r.ej, r.sk, r.ek, p);
          }
        ...
        if (pmb->pmy_mesh->multilevel && spec_.coarse_fc[0] != nullptr
          ...
          AthenaArray<Real> *cface[3] = {spec_.coarse_fc[0], spec_.coarse_fc[1],
                                         spec_.coarse_fc[2]};
          for (int sa = 0; sa < 3; ++sa) {
            idx::IndexRange3D cr = idx::SetSameLevel(pmb, nb.ni, ngh, 2,
                                                     samp, sa);
            BufferUtility::UnpackData(recv_buf_[nb.bufid], *cface[sa],
                                      cr.si, cr.ei, cr.sj, cr.ej,
                                      cr.sk, cr.ek, p);
            if (sa == 1 && nx2_degen)
        ...
        AthenaArray<Real> *cface[3] = {spec_.coarse_fc[0], spec_.coarse_fc[1],
                                       spec_.coarse_fc[2]};
        for (int sa = 0; sa < 3; ++sa) {
          idx::IndexRange3D r = idx::SetFromCoarser(pmb, nb.ni, ngh,
                                                    samp, sa);
          if (nb.polar) {
            UnpackPolarFC(recv_buf_[nb.bufid], *cface[sa], fc_polar_sign[sa],
                          r.si, r.ei, r.sj, r.ej, r.sk, r.ek, p);
          } else {
            BufferUtility::UnpackData(recv_buf_[nb.bufid], *cface[sa],
                                      r.si, r.ei, r.sj, r.ej, r.sk, r.ek, p);
            // Degenerate copies only in non-polar branch (matching old behavior).
            if (sa == 1 && nx2_degen)
        ...
        for (int sa = 0; sa < 3; ++sa) {
          idx::IndexRange3D r = idx::SetFromFiner(pmb, nb.ni, ngh,
                                                  samp, sa);
          if (nb.polar) {
            UnpackPolarFC(recv_buf_[nb.bufid], *face[sa], fc_polar_sign[sa],
                          r.si, r.ei, r.sj, r.ej, r.sk, r.ek, p);
  ...
  for (int n = 0; n < nc.num_neighbors(); ++n) {
    ...
    if (nb.snb.level == mylevel) {
      ...
      if (nb.polar) {
        // TODO: polar additive unpack for VC - not needed yet (Z4c VC has no polar).
        // When needed, add a sign-flipped UnpackDataAdd variant here.
        UnpackPolar(recv_buf_[nb.bufid], var, nvar, polar_signs_,
                    r.si, r.ei, r.sj, r.ej, r.sk, r.ek, p);
      } else if (additive) {
        BufferUtility::UnpackDataAdd(recv_buf_[nb.bufid], var, 0, nvar - 1,
                                     r.si, r.ei, r.sj, r.ej, r.sk, r.ek, p);
      } else {
        BufferUtility::UnpackData(recv_buf_[nb.bufid], var, 0, nvar - 1,
                                  r.si, r.ei, r.sj, r.ej, r.sk, r.ek, p);
      }
      ...
      if (pmb->pmy_mesh->multilevel && spec_.coarse_var != nullptr
        ...
        if (additive) {
          BufferUtility::UnpackDataAdd(recv_buf_[nb.bufid], cvar, 0, nvar - 1,
                                       cr.si, cr.ei, cr.sj, cr.ej, cr.sk, cr.ek, p);
        } else {
          BufferUtility::UnpackData(recv_buf_[nb.bufid], cvar, 0, nvar - 1,
                                    cr.si, cr.ei, cr.sj, cr.ej, cr.sk, cr.ek, p);
        }
      }
    } else if (nb.snb.level < mylevel) {
      ...
      if (nb.polar) {
        UnpackPolar(recv_buf_[nb.bufid], cvar, nvar, polar_signs_,
                    r.si, r.ei, r.sj, r.ej, r.sk, r.ek, p);
      } else {
        BufferUtility::UnpackData(recv_buf_[nb.bufid], cvar, 0, nvar - 1,
                                  r.si, r.ei, r.sj, r.ej, r.sk, r.ek, p);
    ...
    recv_flag_[nb.bufid].store(BoundaryStatus::completed, std::memory_order_release);
  }
}
...
void CommChannel::Clear(const NeighborConnectivity &nc,
                        CommTarget target_filter) {
...
//----------------------------------------------------------------------------------------
// Same-rank direct copy: find the target block's CommChannel and memcpy into its recv buf.
// This avoids MPI overhead for intra-rank communication.  The target channel is found
// via CommRegistry::FindForBlock(), which uses a static map of all registries.
...
//
// Instead of packing into send_buf_ and then memcpy-ing into the target's recv_buf_,
// the sender resolves the target's recv buffer pointer up front and packs directly
// into it.  This eliminates two of the three copies in the old same-rank path
// (pack->send_buf + memcpy->recv_buf), leaving only the single pack->recv_buf copy.
...
//----------------------------------------------------------------------------------------
// Populate edge_flag_[12] and nedge_fine_[12] by scanning the neighbor-level table.
// Determines whether each of the 12 edges has a finer neighbor touching it, and how
// many blocks contribute at the finest level.  Only meaningful for FC EMF (AccumulateAverage).
// Ported from FaceCenteredBoundaryVariable::CountFineEdges (bvals_fc.cpp:790-853).
...
void CommChannel::AllocateFluxCorrBuffers(const NeighborConnectivity &nc) {
  ...
  for (int n = 0; n < nc.num_neighbors(); ++n) {
    ...
    if (size <= 0) continue;  // corner (FC) or non-face (CC) - no buffer

    // Zero-copy: same-rank neighbors don't need flcor_send_buf_ because
    // PackAndSendFluxCorrCC/FC packs directly into the target's flcor_recv_buf_.
    if (nb.snb.rank != Globals::my_rank) {
      flcor_send_buf_[nb.bufid] = new Real[size];
    }
    // flcor_send_buf_[nb.bufid] remains nullptr for same-rank (initialized in ctor).
    flcor_recv_buf_[nb.bufid] = new Real[size];
    flcor_recv_flag_[nb.bufid].store(BoundaryStatus::waiting, std::memory_order_relaxed);
    flcor_send_flag_[nb.bufid].store(BoundaryStatus::waiting, std::memory_order_relaxed);
...
//   CC: send only fine->coarse (nb.level < mylevel), recv only coarse<-finer (nb.level > mylevel)
//   FC: same-level face + same-level edge (with edge_flag_) + fine->coarse + coarse<-finer
// Uses the same tag scheme as ghost exchange, with the flux correction channel_id offset
// by kFluxCorrTagChannelOffset to avoid tag collision with ghost exchange on the same channel.
...
void CommChannel::SetupFluxCorrMPI(const NeighborConnectivity &nc,
  ...
  // ensuring no tag collisions between ghost-exchange and flux-correction messages.
  // Flux correction channel_ids are shifted by kFluxCorrTagChannelOffset.
  ...
  for (int n = 0; n < nc.num_neighbors(); ++n) {
    ...
    // Only set up MPI for neighbors that have an allocated flux correction buffer.
    if (flcor_send_buf_[nb.bufid] == nullptr && flcor_recv_buf_[nb.bufid] == nullptr)
      continue;
    ...
    if (spec_.flcor_mode == FluxCorrMode::OverwriteFromFiner) {
      // CC: fine block sends to coarser neighbor; coarse block receives from finer.
      if (nb.snb.level < mylevel) {
        // This block is finer -> send restricted flux to coarser neighbor.
        int tag = MakeTag(nb.snb.lid, nb.targetid, flcor_ch,
                          bufid_bits, channel_bits);
        if (req_flcor_send_[nb.bufid] != MPI_REQUEST_NULL)
          MPI_Request_free(&req_flcor_send_[nb.bufid]);
        MPI_Send_init(flcor_send_buf_[nb.bufid], buf_size, MPI_ATHENA_REAL,
                      nb.snb.rank, tag, MPI_COMM_WORLD, &req_flcor_send_[nb.bufid]);
      } else if (nb.snb.level > mylevel) {
        // This block is coarser -> receive restricted flux from finer neighbor.
        int tag = MakeTag(pmb->lid, nb.bufid, flcor_ch,
                          bufid_bits, channel_bits);
        if (req_flcor_recv_[nb.bufid] != MPI_REQUEST_NULL)
          MPI_Request_free(&req_flcor_recv_[nb.bufid]);
        MPI_Recv_init(flcor_recv_buf_[nb.bufid], buf_size, MPI_ATHENA_REAL,
                      nb.snb.rank, tag, MPI_COMM_WORLD, &req_flcor_recv_[nb.bufid]);
      }
      // Same-level: CC flux correction has no same-level exchange.
    } else if (spec_.flcor_mode == FluxCorrMode::AccumulateAverage) {
      // FC EMF: same-level (face + qualifying edge) + fine->coarse + coarse<-finer.
      if (nb.snb.level == mylevel) {
        ...
        if (participates) {
          ...
          MPI_Send_init(flcor_send_buf_[nb.bufid], buf_size, MPI_ATHENA_REAL,
                        nb.snb.rank, tag, MPI_COMM_WORLD, &req_flcor_send_[nb.bufid]);
          // Recv
          tag = MakeTag(pmb->lid, nb.bufid, flcor_ch,
                        bufid_bits, channel_bits);
          if (req_flcor_recv_[nb.bufid] != MPI_REQUEST_NULL)
            MPI_Request_free(&req_flcor_recv_[nb.bufid]);
          MPI_Recv_init(flcor_recv_buf_[nb.bufid], buf_size, MPI_ATHENA_REAL,
        ...
        // which is safe for persistent requests (over-allocates for to-coarser).
        int tag = MakeTag(nb.snb.lid, nb.targetid, flcor_ch,
                          bufid_bits, channel_bits);
        if (req_flcor_send_[nb.bufid] != MPI_REQUEST_NULL)
          MPI_Request_free(&req_flcor_send_[nb.bufid]);
...
//----------------------------------------------------------------------------------------
// Same-rank direct copy for flux correction: write into target block's flcor recv buffer.
// Mirrors CopyBufferSameProcess but targets flcor_recv_buf_/flcor_recv_flag_.
...
//
//   CommChannel &target_ch = target_reg->channel(channel_id_);
//   std::memcpy(target_ch.flcor_recv_buf_[nb.targetid], flcor_send_buf_[nb.bufid],
//               send_size * sizeof(Real));
...
//----------------------------------------------------------------------------------------
// Pack area-weighted (CC) or edge-length-weighted (FC) restricted fluxes into send
// buffers, then send to the appropriate neighbors.
// CC (OverwriteFromFiner): only face neighbors with level == mylevel-1.
// FC (AccumulateAverage): same-level face + qualifying edge, and to-coarser face + edge.
// Polar EMF is deferred.

void CommChannel::PackAndSendFluxCorr(const NeighborConnectivity &nc) {
  if (spec_.flcor_mode == FluxCorrMode::None) return;
...
//----------------------------------------------------------------------------------------
// CC flux correction send: area-weighted restriction of fine fluxes to coarser neighbor.
// Ported from CellCenteredBoundaryVariable::SendFluxCorrection (flux_correction_cc.cpp).
...
void CommChannel::PackAndSendFluxCorrCC(const NeighborConnectivity &nc) {
  ...
  for (int n = 0; n < nc.num_neighbors(); ++n) {
    ...
    if (flcor_send_flag_[nb.bufid].load(std::memory_order_relaxed)
        == BoundaryStatus::completed) continue;
    if (nb.snb.level != mylevel - 1) continue;
      ...
      if (pmb->block_size.nx3 > 1) { // 3D
          ...
          for (int k = pmb->ks; k <= pmb->ke; k += 2) {
            ...
            for (int i = pmb->is; i <= pmb->ie; i += 2) {
              ...
              sbuf[p++] = (x2flux(nn, k,   j, i  )*sa0(i)
                         ...
                         + x2flux(nn, k+1, j, i+1)*sa1(i+1)) / tarea;
            }
          }
        }
      } else if (pmb->block_size.nx2 > 1) { // 2D
        ...
        for (int nn = 0; nn < nvar; ++nn) {
          ...
          for (int i = pmb->is; i <= pmb->ie; i += 2) {
            ...
            sbuf[p++] = (x2flux(nn, k, j, i  )*sa0(i)
                       + x2flux(nn, k, j, i+1)*sa0(i+1)) / tarea;
          }
        }
      }
        ...
        for (int j = pmb->js; j <= pmb->je; j += 2) {
          ...
          for (int i = pmb->is; i <= pmb->ie; i += 2) {
            ...
            sbuf[p++] = (x3flux(nn, k, j,   i  )*sa0(i)
                       ...
                       + x3flux(nn, k, j+1, i+1)*sa1(i+1)) / tarea;
          }
        }
      }
    }
...
//----------------------------------------------------------------------------------------
// FC EMF flux correction send: raw EMFs (same-level) or edge-length-weighted restricted
// EMFs (to-coarser) for face + edge neighbors.
// Ported from FaceCenteredBoundaryVariable::SendFluxCorrection (flux_correction_fc.cpp).
...
int CommChannel::LoadFluxBoundaryBufferSameLevel(
...
//----------------------------------------------------------------------------------------
// Pack edge-length-weighted restricted EMFs for a to-coarser face or edge neighbor.
// Returns the number of Reals packed.

int CommChannel::LoadFluxBoundaryBufferToCoarser(
    Real *buf, const NeighborBlock &nb,
    const AthenaArray<Real> &e1, const AthenaArray<Real> &e2,
    const AthenaArray<Real> &e3,
  ...
  if (nb.ni.type == NeighborConnect::face) {
    ...
    if (pmb->block_size.nx3 > 1) { // 3D
      if (nb.eid >= 0 && nb.eid < 4) { // x1x2 edge: restrict e3
        int i = ((nb.eid & 1) == 0) ? pmb->is : pmb->ie + 1;
        int j = ((nb.eid & 2) == 0) ? pmb->js : pmb->je + 1;
        bool pole = pco->IsPole(j);
        for (int k = pmb->ks; k <= pmb->ke; k += 2) {
      ...
      } else if (nb.eid >= 4 && nb.eid < 8) { // x1x3 edge: restrict e2
        int i = ((nb.eid & 1) == 0) ? pmb->is : pmb->ie + 1;
        int k = ((nb.eid & 2) == 0) ? pmb->ks : pmb->ke + 1;
        for (int j = pmb->js; j <= pmb->je; j += 2) {
          Real el1 = pco->GetEdge2Length(k,j,i);
    ...
    } else if (pmb->block_size.nx2 > 1) { // 2D: only x1x2 edge, pack e3
      int i = ((nb.eid & 1) == 0) ? pmb->is : pmb->ie + 1;
      int j = ((nb.eid & 2) == 0) ? pmb->js : pmb->je + 1;
      buf[p++] = e3(pmb->ks,j,i);
    }
...
// FC (AccumulateAverage): two-phase poll+unpack (same-level -> ClearCoarse -> from-finer
//   -> AverageFlux).  Each arrived buffer is unpacked immediately because the two-phase
//   protocol requires same-level unpack to finish before ClearCoarseFluxBoundary.
// Returns true when all expected buffers have been received (and, for FC, fully processed).

bool CommChannel::PollReceiveFluxCorr(const NeighborConnectivity &nc) {
  if (spec_.flcor_mode == FluxCorrMode::None) return true;
...
bool CommChannel::PollReceiveFluxCorrCC(const NeighborConnectivity &nc) {
  ...
  for (int n = 0; n < nc.num_neighbors(); ++n) {
    ...
    if (flcor_recv_flag_[nb.bufid].load(std::memory_order_acquire)
        == BoundaryStatus::arrived) continue;  // ready for unpack
...
// Phase 1 (recv_flx_same_lvl_ == true): poll same-level face + qualifying edge.
//   Unpack each arrived buffer immediately via SetFluxBoundarySameLevel.
//   When all same-level done -> ClearCoarseFluxBoundary -> transition to phase 2.
...
bool CommChannel::PollReceiveFluxCorrFC(const NeighborConnectivity &nc) {
  ...
  if (recv_flx_same_lvl_) {
    ...
    for (int n = 0; n < nc.num_neighbors(); ++n) {
      ...
      if (nb.ni.type == NeighborConnect::edge && !edge_flag_[nb.eid]) continue;
    ...
    // All same-level received and unpacked.  Zero coarse-side EMFs before from-finer.
...
// CC: overwrites coarse flux at the shared face (called after PollReceiveFluxCorr returns true).
// FC: no-op - FC unpack is done inside PollReceiveFluxCorrFC (two-phase protocol requires it).

void CommChannel::UnpackFluxCorr(const NeighborConnectivity &nc) {
  if (spec_.flcor_mode != FluxCorrMode::OverwriteFromFiner) return;
  ...
  for (int n = 0; n < nc.num_neighbors(); ++n) {
    ...
    if (nb.snb.level != mylevel + 1) continue;
    if (flcor_recv_flag_[nb.bufid].load(std::memory_order_acquire)
        != BoundaryStatus::arrived) continue;
...
// Ported from CellCenteredBoundaryVariable::ReceiveFluxCorrection (flux_correction_cc.cpp).
...
void CommChannel::SetFluxBoundarySameLevel(
    Real *buf, const NeighborBlock &nb,
    AthenaArray<Real> &e1, AthenaArray<Real> &e2, AthenaArray<Real> &e3) {
  MeshBlock *pmb = pmy_block_;
  int p = 0;
  ...
  if (nb.ni.type == NeighborConnect::face) {
    ...
    if (nb.eid >= 0 && nb.eid < 4) { // x1x2 edge: e3
      int i = ((nb.eid & 1) == 0) ? pmb->is : pmb->ie + 1;
      int j = ((nb.eid & 2) == 0) ? pmb->js : pmb->je + 1;
      Real sign = (nb.polar && flip_across_pole_field[IB3]) ? -1.0 : 1.0;
      for (int k = pmb->ks; k <= pmb->ke; ++k)
        e3(k,j,i) += sign * buf[p++];
    } else if (nb.eid >= 4 && nb.eid < 8) { // x1x3 edge: e2
      int i = ((nb.eid & 1) == 0) ? pmb->is : pmb->ie + 1;
      int k = ((nb.eid & 2) == 0) ? pmb->ks : pmb->ke + 1;
      for (int j = pmb->js; j <= pmb->je; ++j)
        e2(k,j,i) += buf[p++];
...
//----------------------------------------------------------------------------------------
// FC EMF unpack: accumulate from-finer restricted EMFs with fi1/fi2 quadrant narrowing.
// Ported from FaceCenteredBoundaryVariable::SetFluxBoundaryFromFiner.

void CommChannel::SetFluxBoundaryFromFiner(
    Real *buf, const NeighborBlock &nb,
    AthenaArray<Real> &e1, AthenaArray<Real> &e2, AthenaArray<Real> &e3) {
  MeshBlock *pmb = pmy_block_;
  int p = 0;
  ...
  if (nb.ni.type == NeighborConnect::face) {
    if (pmb->block_size.nx3 > 1) { // 3D
      if (nb.fid == BoundaryFace::inner_x1 || nb.fid == BoundaryFace::outer_x1) {
        int i = (nb.fid == BoundaryFace::inner_x1) ? pmb->is : pmb->ie + 1;
        int jl = pmb->js, ju = pmb->je, kl = pmb->ks, ku = pmb->ke;
        if (nb.ni.fi1 == 0) ju = pmb->js + pmb->block_size.nx2/2 - 1;
        else                 jl = pmb->js + pmb->block_size.nx2/2;
        if (nb.ni.fi2 == 0) ku = pmb->ks + pmb->block_size.nx3/2 - 1;
        ...
        int j = (nb.fid == BoundaryFace::inner_x2) ? pmb->js : pmb->je + 1;
        int il = pmb->is, iu = pmb->ie, kl = pmb->ks, ku = pmb->ke;
        if (nb.ni.fi1 == 0) iu = pmb->is + pmb->block_size.nx1/2 - 1;
        ...
        if (nb.ni.fi2 == 0) ju = pmb->js + pmb->block_size.nx2/2 - 1;
        else                 jl = pmb->js + pmb->block_size.nx2/2;
        for (int j = jl; j <= ju+1; ++j)
          for (int i = il; i <= iu; ++i)
            e1(k,j,i) += buf[p++];
    ...
    if (pmb->block_size.nx3 > 1) { // 3D
      if (nb.eid >= 0 && nb.eid < 4) { // x1x2 edge: e3
        ...
        if (nb.ni.fi1 == 0) ku = pmb->ks + pmb->block_size.nx3/2 - 1;
        else                 kl = pmb->ks + pmb->block_size.nx3/2;
        Real sign = (nb.polar && flip_across_pole_field[IB3]) ? -1.0 : 1.0;
        for (int k = kl; k <= ku; ++k)
          e3(k,j,i) += sign * buf[p++];
    ...
    } else if (pmb->block_size.nx2 > 1) { // 2D: only x1x2 edge, e3
      int i = ((nb.eid & 1) == 0) ? pmb->is : pmb->ie + 1;
      int j = ((nb.eid & 2) == 0) ? pmb->js : pmb->je + 1;
      e3(pmb->ks,j,i) += buf[p++];
    }
...
//----------------------------------------------------------------------------------------
//! \fn void CommChannel::ClearCoarseFluxBoundary(const NeighborConnectivity &nc)
//  \brief Zero EMFs at faces with finer neighbors and edges where finer blocks contribute.
//
//  Called between the same-level and from-finer unpack phases in the two-phase
//  FC EMF protocol.  After same-level EMFs have been accumulated, this zeros the
//  EMF arrays at interfaces where finer blocks will contribute, so the from-finer
//  restricted EMFs start from a clean slate.
...
void CommChannel::ClearCoarseFluxBoundary(const NeighborConnectivity &nc) {
  ...
  const int ndim = 1 + (pmb->block_size.nx2 > 1 ? 1 : 0)
                     + (pmb->block_size.nx3 > 1 ? 1 : 0);
  const int nface = 2 * ndim;

  // Compute nedge (number of edge types: 4 edges per pair of active dimensions)
  int nedge = 0;
  if (pmb->block_size.nx2 > 1) nedge += 4;  // x1x2 edges
  if (pmb->block_size.nx3 > 1) nedge += 8;  // x1x3 + x2x3 edges
  ...
  for (int n = 0; n < nedge; ++n) {
    ...
    if (n >= 0 && n < 4) { // x1x2 edges: e3 component
      int i = ((n & 1) == 0) ? is : ie + 1;
      int j = ((n & 2) == 0) ? js : je + 1;
      for (int k = ks; k <= ke; ++k)
        e3(k,j,i) = 0.0;
    } else if (n >= 4 && n < 8) { // x1x3 edges: e2 component
      int i = ((n & 1) == 0) ? is : ie + 1;
      int k = ((n & 2) == 0) ? ks : ke + 1;
      for (int j = js; j <= je; ++j)
        e2(k,j,i) = 0.0;
    } else if (n >= 8 && n < 12) { // x2x3 edges: e1 component
      int j = ((n & 1) == 0) ? js : je + 1;
      int k = ((n & 2) == 0) ? ks : ke + 1;
      for (int i = is; i <= ie; ++i)
        e1(k,j,i) = 0.0;
...
//  \brief Divide accumulated EMFs by contributor count after both unpack phases complete.
//
//  At faces with same-level neighbors, each EMF was accumulated from both sides -> /2.
//  At faces with finer neighbors, the overlap seam (midpoint of fine subfaces) was
//  contributed by two fine blocks -> /2.
//  At edges, divide by nedge_fine_[eid] (the number of blocks at the finest level
//  touching that edge).
...
void CommChannel::AverageFluxBoundary(const NeighborConnectivity &nc) {
    ...
    if (n == BoundaryFace::inner_x1 || n == BoundaryFace::outer_x1) {
      ...
      if (nl == mylevel) {
        ...
        if (pmb->block_size.nx3 > 1) { // 3D
          ...
          e2(ks,js,i) *= 0.5; e2(ks+1,js,i) *= 0.5;
          e3(ks,js,i) *= 0.5; e3(ks,js+1,i) *= 0.5;
        }
      } else if (nl > mylevel) {
        // Finer: the midpoint seam between fine subfaces was double-counted -> /2
        if (pmb->block_size.nx3 > 1) { // 3D
  ...
  for (int n = 0; n < nedge; ++n) {
    ...
    if (n >= 0 && n < 4) { // x1x2 edges: e3
      int i = ((n & 1) == 0) ? is : ie + 1;
      int j = ((n & 2) == 0) ? js : je + 1;
      for (int k = ks; k <= ke; ++k)
        e3(k,j,i) *= div;
    } else if (n >= 4 && n < 8) { // x1x3 edges: e2
      int i = ((n & 1) == 0) ? is : ie + 1;
      int k = ((n & 2) == 0) ? ks : ke + 1;
      for (int j = js; j <= je; ++j)
        e2(k,j,i) *= div;
    } else if (n >= 8 && n < 12) { // x2x3 edges: e1
      int j = ((n & 1) == 0) ? js : je + 1;
      int k = ((n & 2) == 0) ? ks : ke + 1;
      for (int i = is; i <= ie; ++i)
        e1(k,j,i) *= div;
...
//----------------------------------------------------------------------------------------
//! \fn void CommChannel::StartReceivingFluxCorr(const NeighborConnectivity &nc)
//  \brief Post persistent receive requests for flux correction.
//
//  CC (OverwriteFromFiner): post receives from face neighbors with level > mylevel.
//  FC (AccumulateAverage): post receives from same-level face + edge neighbors
//  (where applicable) AND from finer face + edge neighbors.  Also sets
//  recv_flx_same_lvl_ = true to begin the same-level phase.
...
void CommChannel::StartReceivingFluxCorr(const NeighborConnectivity &nc) {
  ...
  for (int n = 0; n < nc.num_neighbors(); ++n) {
    ...
    if (spec_.flcor_mode == FluxCorrMode::OverwriteFromFiner) {
      ...
      // FC (AccumulateAverage): receive from face+edge neighbors that participate
      if (nb.ni.type == NeighborConnect::face
          || nb.ni.type == NeighborConnect::edge) {
        if (nb.snb.level > mylevel) {
          ...
          // Same level: face always, edge only if edge_flag_ says same-level-only
          if (nb.ni.type == NeighborConnect::face
              || (nb.ni.type == NeighborConnect::edge && edge_flag_[nb.eid])) {
            MPI_Start(&req_flcor_recv_[nb.bufid]);
          }
...
//----------------------------------------------------------------------------------------
//! \fn void CommChannel::ClearFluxCorr(const NeighborConnectivity &nc)
//  \brief Wait on outstanding flux correction sends and reset all flags.
//
//  Must be called after the flux correction cycle completes (both send and receive
//  sides finished).  Resets all flcor flags to waiting for the next cycle.

void CommChannel::ClearFluxCorr(const NeighborConnectivity &nc) {
  if (spec_.flcor_mode == FluxCorrMode::None) return;
  ...
  for (int n = 0; n < nc.num_neighbors(); ++n) {
    ...
    if (spec_.flcor_mode == FluxCorrMode::OverwriteFromFiner) {
      // CC: only face neighbors participate
      if (nb.ni.type == NeighborConnect::face) {
...
#ifdef MPI_PARALLEL
        if (nb.snb.rank != Globals::my_rank) {
          // Wait on send if we sent (fine->coarse direction)
          if (nb.snb.level < mylevel) {
      ...
      // FC (AccumulateAverage): face + edge neighbors participate
      if (nb.ni.type == NeighborConnect::face
          || nb.ni.type == NeighborConnect::edge) {
        flcor_recv_flag_[nb.bufid].store(BoundaryStatus::waiting,
                                         std::memory_order_relaxed);
...
#ifdef MPI_PARALLEL
        if (nb.snb.rank != Globals::my_rank) {
          // Wait on send for neighbors we actually sent to:
          //   - to coarser (fine->coarse)
          //   - same-level face
          //   - same-level edge with edge_flag_
          if (nb.snb.level < mylevel) {
            MPI_Wait(&req_flcor_send_[nb.bufid], MPI_STATUS_IGNORE);
          } else if (nb.snb.level == mylevel) {
            if (nb.ni.type == NeighborConnect::face
