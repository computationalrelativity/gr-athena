    ...
    : Coordinates(pmb, pin, coarse_flag)
{
  ...
  fd_is_defined = true;
  fd_cc =
    new FiniteDifference::Uniform(nc1, nc2, nc3, dx1v(0), dx2v(0), dx3v(0));

  fd_cx = new FiniteDifference::Uniform(
    cx_nc1, cx_nc2, cx_nc3, dx1v(0), dx2v(0), dx3v(0));

  fd_vc =
    new FiniteDifference::Uniform(nv1, nv2, nv3, dx1f(0), dx2f(0), dx3f(0));
...
#if FLUID_ENABLED
  adm_sqrt_detgamma.InitWithShallowSlice(pz4c->storage.aux_extended,
  ...
  for (int k = kl; k <= ku; ++k)
    for (int j = jl; j <= ju; ++j)
    {
      ...
      LinearAlgebra::SlicedVecMet3Contraction(
        w_util_d_, w_util_u_, gamma_dd_, il, iu);
      ...
      {
        LinearAlgebra::SlicedVecMet3Contraction(
          beta_d_, beta_u_, gamma_dd_, il, iu);
