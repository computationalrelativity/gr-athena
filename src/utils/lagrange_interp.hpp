...
class LagrangeInterp1D
{
        ...
        m_out_of_bounds(false)
  {
    ...
    m_point = std::lrint(
      std::floor((m_coord - m_origin) * m_delta_inv - 0.5 * (order - 1)));
#ifdef LOCALINTERP_SYMMETRIC
    if (m_mid_flag)
    {
      m_point -= 1;
  ...
  T eval_diff(
    ...
    int const stride) const
  {
    static_assert(compute_diff, "eval_diff() requires compute_diff=true");
    T out_lr = 0;
    for (int i = 0; i <= order; ++i)
    {
  ...
  void m_calc_coeff_lr(Real const* const xp, Real* const coeff) const
  {
#define TYPECASE(I0, TEST, I1, OP)    \
  ...
  void m_calc_coeff_rl(Real const* const xp, Real* const coeff) const
  {
    TYPECASE(order, >=, 0, --)
#undef TYPECASE
  }
  ...
  void m_calc_coeff_diff_lr(Real const* const xp,
                            Real const* const coeff,
                            Real* const coeff_diff) const
  {
#define TYPECASE(M0, TEST, M1, OP)    \
  ...
  void m_calc_coeff_diff_rl(Real const* const xp,
                            Real const* const coeff,
                            Real* const coeff_diff) const
  {
    TYPECASE(order, >=, 0, --)
...
// . the fastest running index in the data is that corrsponding to the first
// dimension . the stride used to access the data along the first dimension is
// 1
...
class LagrangeInterpND
{
      ...
      : m_out_of_bounds(false)
  {
    for (int d = 0; d < ndim; ++d)
    {
      ...
      m_coord[d]  = coord[d];
      mp_interp[d] =
        new (&m_interp_scratch[d][0]) LagrangeInterp1D<order, compute_diff>(
          m_origin[d], m_delta[d], m_siz[d], m_coord[d]);
  ...
  // Move constructor: copy POD members, then placement-new move-construct
  // each 1D interpolator into this object's own scratch buffers.
      ...
      : m_out_of_bounds(o.m_out_of_bounds)
  {
    ...
    for (int d = 0; d < ndim; ++d)
    {
      mp_interp[d] = new (&m_interp_scratch[d][0])
        LagrangeInterp1D<order, compute_diff>(std::move(*o.mp_interp[d]));
    }
  }
  ...
  void m_fill_stencil(T const* const gf) const
  {
    ...
    if constexpr (D == 0)
    {
      ...
      std::memcpy(
        &m_vals[0][0], &gf[gidx], mp_interp[0]->npoint() * sizeof(T));
    }
    else
    {
      for (m_eval_pos[D] = 0; m_eval_pos[D] < mp_interp[D]->npoint();
           ++m_eval_pos[D])
      {
        m_fill_stencil<T, NextStencil<ndim, D>::value, der>(gf);
        if constexpr (D - 1 == der)
        {
          m_vals[D][m_eval_pos[D]] =
            mp_interp[D - 1]->eval_diff(m_vals[D - 1], 1);
        }
        else
        {
