...
volatile sig_atomic_t signalflag[nsignal];
sigset_t mask;
...
int CheckSignalFlags()
{
  ...
  // timestep in main.cpp; i.e. if an issue prevents a process from reaching
  // the end of a cycle, the signals will never be handled by that process /
  // the solver may hang
  int ret = 0;
  sigprocmask(SIG_BLOCK, &mask, nullptr);
#ifdef MPI_PARALLEL
  int flags_buf[nsignal];
  for (int n = 0; n < nsignal; n++)
    flags_buf[n] = static_cast<int>(signalflag[n]);
  MPI_Allreduce(
    MPI_IN_PLACE, flags_buf, nsignal, MPI_INT, MPI_MAX, MPI_COMM_WORLD);
  for (int n = 0; n < nsignal; n++)
    signalflag[n] = static_cast<sig_atomic_t>(flags_buf[n]);
#endif
  for (int n = 0; n < nsignal; n++)
    ret += signalflag[n];
