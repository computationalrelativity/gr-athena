    ...
    trgs(trgs)
{
  ...
  if (multilevel)
  {
    ...
    Add(RECV_HYDFLX, NONE,
        &GRMHD_Z4c_Monolithic::ReceiveAndCorrectHydroFlux);
  }
  ...
  if (NSCALARS > 0)
  {
    if (multilevel)
    {
      ...
      Add(SEND_SCLRFLX, CALC_HYDSCLRFLX,
          &GRMHD_Z4c_Monolithic::SendScalarFlux);
      Add(RECV_SCLRFLX, NONE,
          &GRMHD_Z4c_Monolithic::ReceiveScalarFlux);
    }
  }
  ...
  Add(SRCTERM_HYD, ADD_FLX_DIV,
      &GRMHD_Z4c_Monolithic::AddSourceTermsHydro);

  // Magnetic field: EMF corner calc, flux-correction exchange, CT integration
  if (MAGNETIC_FIELDS_ENABLED)
  {
    Add(CALC_FLDFLX, CALC_HYDSCLRFLX,
        &GRMHD_Z4c_Monolithic::CalculateEMF);
    Add(SEND_FLDFLX, CALC_FLDFLX,
        &GRMHD_Z4c_Monolithic::SendFluxCorrectionEMF);
    Add(RECV_FLDFLX, NONE,
        &GRMHD_Z4c_Monolithic::ReceiveAndCorrectEMF);
    Add(INT_FLD, RECV_FLDFLX,
        &GRMHD_Z4c_Monolithic::IntegrateField);
  }
  ...
  Add(INIT_Z4C_DERIV, NONE,
      &GRMHD_Z4c_Monolithic::InitializeZ4cDerivatives);
  Add(CALC_Z4CRHS, INIT_Z4C_DERIV,
      &GRMHD_Z4c_Monolithic::CalculateZ4cRHS);
  Add(INT_Z4C, CALC_Z4CRHS,
      &GRMHD_Z4c_Monolithic::IntegrateZ4c);

  // Z4c ghost-zone exchange (send after integration, recv polls from NONE)
  Add(SEND_Z4C, INT_Z4C,
      &GRMHD_Z4c_Monolithic::SendZ4c);
  Add(RECV_Z4C, NONE,
      &GRMHD_Z4c_Monolithic::ReceiveZ4c);

  Add(SETB_Z4C, (RECV_Z4C | INT_Z4C),
      &GRMHD_Z4c_Monolithic::SetBoundariesZ4c);

  if (multilevel)
  {
    Add(PROLONG_Z4C, (SEND_Z4C | SETB_Z4C),
        &GRMHD_Z4c_Monolithic::Prolongation_Z4c);
    Add(PHY_BVAL_Z4C, PROLONG_Z4C,
        &GRMHD_Z4c_Monolithic::PhysicalBoundary_Z4c);
  }
  else
  {
    Add(PHY_BVAL_Z4C, SETB_Z4C,
        &GRMHD_Z4c_Monolithic::PhysicalBoundary_Z4c);
  }

  Add(ALG_CONSTR, PHY_BVAL_Z4C,
      &GRMHD_Z4c_Monolithic::EnforceAlgConstr);

  // Pre-compute conformal derivative 3D arrays from post-comm z4c.u
  Add(PREP_Z4C_DERIV, ALG_CONSTR,
      &GRMHD_Z4c_Monolithic::PrepareZ4cDerivatives);

  Add(Z4C_TO_ADM, PREP_Z4C_DERIV,
      &GRMHD_Z4c_Monolithic::Z4cToADM);

#if CCE_ENABLED
  Add(CCE_DUMP, Z4C_TO_ADM,
      &GRMHD_Z4c_Monolithic::CCEDump);
#endif
  ...
  Add(CONS2PRIMG, (PHY_BVAL_HYD | CONS2PRIMP),
      &GRMHD_Z4c_Monolithic::PrimitivesGhosts);

  // Re-couple ADM source terms (GetMatter)
  Add(UPDATE_SRC, CONS2PRIMG,
      &GRMHD_Z4c_Monolithic::UpdateSource);

  // Diagnostics (last substep only - guarded internally)
  Add(ADM_CONSTR, UPDATE_SRC,
      &GRMHD_Z4c_Monolithic::ADM_Constraints);
  Add(Z4C_WEYL, UPDATE_SRC,
      &GRMHD_Z4c_Monolithic::Z4c_Weyl);
  Add(USERWORK, UPDATE_SRC,
      &GRMHD_Z4c_Monolithic::UserWork);

  // Geometric - no data dependencies on evolved state
  Add(NEW_DT, NONE,
      &GRMHD_Z4c_Monolithic::NewBlockTimeStep);

  if (adaptive)
    Add(FLAG_AMR, NONE,
        &GRMHD_Z4c_Monolithic::CheckRefinement);

  // =========================================================================
  // Cleanup - wait on outstanding sends and reset channel flags
  // =========================================================================
  Add(CLEAR_Z4C, Z4C_TO_ADM,
      &GRMHD_Z4c_Monolithic::ClearZ4c);
  Add(CLEAR_MAININT, PHY_BVAL_HYD,
      &GRMHD_Z4c_Monolithic::ClearMainInt);
}
...
TaskStatus GRMHD_Z4c_Monolithic::IntegrateHydroScalars(
...
// ---------------------------------------------------------------------------
TaskStatus GRMHD_Z4c_Monolithic::CalculateHydroScalarFlux(
    MeshBlock *pmb, int stage)
{
  if (stage <= nstages)
  {
...
TaskStatus GRMHD_Z4c_Monolithic::SendFluxCorrectionHydro(
    MeshBlock *pmb, int stage)
{
...
TaskStatus GRMHD_Z4c_Monolithic::SendFluxCorrectionEMF(
    MeshBlock *pmb, int stage)
{
...
TaskStatus GRMHD_Z4c_Monolithic::ReceiveAndCorrectHydroFlux(
    MeshBlock *pmb, int stage)
{
...
TaskStatus GRMHD_Z4c_Monolithic::ReceiveAndCorrectEMF(
    MeshBlock *pmb, int stage)
{
  comm::CommRegistry *pcomm = pmb->pcomm;
  const comm::NeighborConnectivity &nc = pcomm->connectivity();
...
TaskStatus GRMHD_Z4c_Monolithic::SendScalarFlux(
    MeshBlock *pmb, int stage)
{
...
TaskStatus GRMHD_Z4c_Monolithic::ReceiveScalarFlux(
    MeshBlock *pmb, int stage)
{
  comm::CommRegistry *pcomm = pmb->pcomm;
  const comm::NeighborConnectivity &nc = pcomm->connectivity();
...
// ---------------------------------------------------------------------------
TaskStatus GRMHD_Z4c_Monolithic::AddFluxDivergenceHydroScalars(
    MeshBlock *pmb, int stage)
{
  if (stage <= nstages)
  {
...
// ---------------------------------------------------------------------------
TaskStatus GRMHD_Z4c_Monolithic::AddSourceTermsHydro(
    MeshBlock *pmb, int stage)
{
  if (stage <= nstages)
  {
...
TaskStatus GRMHD_Z4c_Monolithic::InitializeZ4cDerivatives(
    MeshBlock *pmb, int stage)
{
  if (stage <= nstages)
  {
...
TaskStatus GRMHD_Z4c_Monolithic::PhysicalBoundary_Z4c(MeshBlock *pmb, int stage)
{
  if (stage <= nstages)
  {
    comm::CommRegistry *pcomm = pmb->pcomm;
...
TaskStatus GRMHD_Z4c_Monolithic::PrepareZ4cDerivatives(
    MeshBlock *pmb, int stage)
{
  if (stage <= nstages)
  {
...
TaskStatus GRMHD_Z4c_Monolithic::SendHydro(MeshBlock *pmb, int stage)
{
  if (stage <= nstages)
  {
    // Group-level send packs all MainInt channels (hydro u, field b, scalars s).
    // Restriction into coarse buffers is handled internally for multilevel.
...
// use different CommGroups (FluxCorr vs MainInt).
...
TaskStatus GRMHD_Z4c_Monolithic::PrimitivesPhysical(
    MeshBlock *pmb, int stage)
{
  if (stage <= nstages)
  {
...
// ---------------------------------------------------------------------------
TaskStatus GRMHD_Z4c_Monolithic::SetBoundariesHydro(
    MeshBlock *pmb, int stage)
{
  if (stage <= nstages)
  {
...
TaskStatus GRMHD_Z4c_Monolithic::PhysicalBoundary_Hyd(
    MeshBlock *pmb, int stage)
{
  if (stage <= nstages)
  {
...
TaskStatus GRMHD_Z4c_Monolithic::PrimitivesGhosts(
    MeshBlock *pmb, int stage)
{
  if (stage <= nstages)
  {
