...
MatterTaskList::MatterTaskList(ParameterInput *pin, Mesh *pm) {
  ...
  if (integrator == "vl2") {
    ...
    msg << "### FATAL ERROR in MatterTaskList constructor" << std::endl
  ...
  if (cfl_number > cfl_limit
    ...
    std::cout << "### Warning in MatterTaskList constructor" << std::endl
...
void MatterTaskList::AddTask(const TaskID& id, const TaskID& dep) {
  ...
  if (id == UPDATE_MET) {
    task_list_[ntasks].TaskFunc=
        ...
        (&MatterTaskList::UpdateMetric);
    task_list_[ntasks].lb_time = false;
  } else if (id == UPDATE_SRC) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::UpdateSource);
    task_list_[ntasks].lb_time = false;
  } else if (id == CLEAR_ALLBND) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::ClearAllBoundary);
    task_list_[ntasks].lb_time = false;
  } else if (id == CALC_HYDFLX) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::CalculateHydroFlux);
    task_list_[ntasks].lb_time = true;
  } else if (id == CALC_FLDFLX) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::CalculateEMF);
    task_list_[ntasks].lb_time = true;
  } else if (id == SEND_HYDFLX) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::SendHydroFlux);
    task_list_[ntasks].lb_time = true;
  } else if (id == SEND_FLDFLX) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::SendEMF);
    task_list_[ntasks].lb_time = true;
  } else if (id == RECV_HYDFLX) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::ReceiveAndCorrectHydroFlux);
    task_list_[ntasks].lb_time = false;
  } else if (id == RECV_FLDFLX) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::ReceiveAndCorrectEMF);
    task_list_[ntasks].lb_time = false;
  } else if (id == INT_HYD) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::IntegrateHydro);
    task_list_[ntasks].lb_time = true;
  } else if (id == INT_FLD) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::IntegrateField);
    task_list_[ntasks].lb_time = true;
  } else if (id == SRCTERM_HYD) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::AddSourceTermsHydro);
    task_list_[ntasks].lb_time = true;
  } else if (id == SEND_HYD) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::SendHydro);
    task_list_[ntasks].lb_time = true;
  } else if (id == SEND_FLD) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::SendField);
    task_list_[ntasks].lb_time = true;
  } else if (id == RECV_HYD) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::ReceiveHydro);
    task_list_[ntasks].lb_time = false;
  } else if (id == RECV_FLD) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::ReceiveField);
    task_list_[ntasks].lb_time = false;
  } else if (id == SETB_HYD) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::SetBoundariesHydro);
    task_list_[ntasks].lb_time = true;
  } else if (id == SETB_FLD) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::SetBoundariesField);
    task_list_[ntasks].lb_time = true;
  } else if (id == SEND_HYDSH) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::SendHydroShear);
    task_list_[ntasks].lb_time = true;
  } else if (id == RECV_HYDSH) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::ReceiveHydroShear);
    task_list_[ntasks].lb_time = false;
  } else if (id == SEND_FLDSH) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::SendFieldShear);
    task_list_[ntasks].lb_time = true;
  } else if (id == RECV_FLDSH) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::ReceiveFieldShear);
    task_list_[ntasks].lb_time = false;
  } else if (id == SEND_EMFSH) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::SendEMFShear);
    task_list_[ntasks].lb_time = true;
  } else if (id == RECV_EMFSH) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::ReceiveEMFShear);
    task_list_[ntasks].lb_time = false;
  } else if (id == RMAP_EMFSH) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::RemapEMFShear);
    task_list_[ntasks].lb_time = true;
  } else if (id == PROLONG_HYD) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::Prolongation_Hyd);
    task_list_[ntasks].lb_time = true;
  } else if (id == CONS2PRIM) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::Primitives);
    task_list_[ntasks].lb_time = true;
  } else if (id == PHY_BVAL_HYD) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::PhysicalBoundary_Hyd);
    task_list_[ntasks].lb_time = true;
  } else if (id == USERWORK) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::UserWork);
    task_list_[ntasks].lb_time = true;
  } else if (id == NEW_DT) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::NewBlockTimeStep);
    task_list_[ntasks].lb_time = true;
  } else if (id == FLAG_AMR) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::CheckRefinement);
    task_list_[ntasks].lb_time = true;
  } else if (id == DIFFUSE_HYD) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::DiffuseHydro);
    task_list_[ntasks].lb_time = true;
  } else if (id == DIFFUSE_FLD) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::DiffuseField);
    task_list_[ntasks].lb_time = true;
  } else if (id == CALC_SCLRFLX) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::CalculateScalarFlux);
    task_list_[ntasks].lb_time = true;
  } else if (id == SEND_SCLRFLX) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::SendScalarFlux);
    task_list_[ntasks].lb_time = true;
  } else if (id == RECV_SCLRFLX) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::ReceiveScalarFlux);
    task_list_[ntasks].lb_time = false;
  } else if (id == INT_SCLR) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::IntegrateScalars);
    task_list_[ntasks].lb_time = true;
  } else if (id == SEND_SCLR) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::SendScalars);
    task_list_[ntasks].lb_time = true;
  } else if (id == RECV_SCLR) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::ReceiveScalars);
    task_list_[ntasks].lb_time = false;
  } else if (id == SETB_SCLR) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::SetBoundariesScalars);
    task_list_[ntasks].lb_time = true;
  } else if (id == DIFFUSE_SCLR) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::DiffuseScalars);
    task_list_[ntasks].lb_time = true;
   } else if (id == CLEAR_ALLBND) {
      task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
      ...
      task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::CalculateZ4cRHS);
      task_list_[ntasks].lb_time = true;
    } else if (id == INT_Z4C) {
      task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::IntegrateZ4c);
      task_list_[ntasks].lb_time = true;
    } else if (id == SEND_Z4C) {
      task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::SendZ4c);
      task_list_[ntasks].lb_time = true;
    } else if (id == RECV_Z4C) {
      task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::ReceiveZ4c);
      task_list_[ntasks].lb_time = false;
    } else if (id == SETB_Z4C) {
      task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::SetBoundariesZ4c);
      task_list_[ntasks].lb_time = true;
    } else if (id == PROLONG_Z4C) {
      task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::Prolongation_Z4c);
      task_list_[ntasks].lb_time = true;
    } else if (id == PHY_BVAL_Z4C) {
      task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::PhysicalBoundary_Z4c);
      task_list_[ntasks].lb_time = true;
    } else if (id == ALG_CONSTR) {
      task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::EnforceAlgConstr);
      task_list_[ntasks].lb_time = true;
    ...
    else if (id == MONOLITHIC)
    {
      task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&MatterTaskList::DebugMonolithic);
      task_list_[ntasks].lb_time = false;
    }
#endif // DBG_MONOLITHIC_MATTER
   else {
...
//WGC
  ...
  if (stage == 1)
  {
    ...
    if (MAGNETIC_FIELDS_ENABLED) { // MHD
      ...
      if (integrator == "ssprk5_4") {
        std::stringstream msg;
        msg << "### FATAL ERROR in MatterTaskList::StartupTaskList\n"
...
TaskStatus MatterTaskList::CalculateEMF(MeshBlock *pmb, int stage) {
  if (stage <= nstages) {
...
TaskStatus MatterTaskList::SendEMF(MeshBlock *pmb, int stage) {
...
TaskStatus MatterTaskList::ReceiveAndCorrectEMF(MeshBlock *pmb, int stage) {
...
TaskStatus MatterTaskList::IntegrateField(MeshBlock *pmb, int stage) {
  Field *pf = pmb->pfield;
...
TaskStatus MatterTaskList::AddSourceTermsHydro(MeshBlock *pmb, int stage) {
  return TaskStatus::next;
  Hydro *ph = pmb->phydro;
  Field *pf = pmb->pfield;
...
TaskStatus MatterTaskList::DiffuseField(MeshBlock *pmb, int stage) {
  Field *pf = pmb->pfield;
...
TaskStatus MatterTaskList::SendField(MeshBlock *pmb, int stage) {
  if (stage <= nstages) {
...
TaskStatus MatterTaskList::ReceiveField(MeshBlock *pmb, int stage) {
  bool ret;
  if (stage <= nstages) {
...
TaskStatus MatterTaskList::SetBoundariesField(MeshBlock *pmb, int stage) {
  if (stage <= nstages) {
...
TaskStatus MatterTaskList::SendHydroShear(MeshBlock *pmb, int stage) {
  if (stage <= nstages) {
...
TaskStatus MatterTaskList::ReceiveHydroShear(MeshBlock *pmb, int stage) {
  bool ret;
  ret = false;
  if (stage <= nstages) {
...
TaskStatus MatterTaskList::SendFieldShear(MeshBlock *pmb, int stage) {
  if (stage <= nstages) {
...
TaskStatus MatterTaskList::ReceiveFieldShear(MeshBlock *pmb, int stage) {
  bool ret;
  ret = false;
  if (stage <= nstages) {
...
TaskStatus MatterTaskList::SendEMFShear(MeshBlock *pmb, int stage) {
  pmb->pfield->fbvar.SendEMFShearingBoxBoundaryCorrection();
  return TaskStatus::success;
}


TaskStatus MatterTaskList::ReceiveEMFShear(MeshBlock *pmb, int stage) {
...
TaskStatus MatterTaskList::RemapEMFShear(MeshBlock *pmb, int stage) {
...
TaskStatus MatterTaskList::PhysicalBoundary_Hyd(MeshBlock *pmb, int stage) {
  Hydro *ph = pmb->phydro;
  PassiveScalars *ps = pmb->pscalars;
  BoundaryValues *pbval = pmb->pbval;
...
TaskStatus MatterTaskList::UserWork(MeshBlock *pmb, int stage) {
...
TaskStatus MatterTaskList::CheckRefinement(MeshBlock *pmb, int stage) {
  if (stage != nstages) return TaskStatus::success; // only do on last stage
...
TaskStatus MatterTaskList::CalculateScalarFlux(MeshBlock *pmb, int stage) {
  PassiveScalars *ps = pmb->pscalars;
  if (stage <= nstages) {
    if ((stage == 1) && (integrator == "vl2")) {
...
TaskStatus MatterTaskList::SendScalarFlux(MeshBlock *pmb, int stage) {
  pmb->pscalars->sbvar.SendFluxCorrection();
  return TaskStatus::success;
}


TaskStatus MatterTaskList::ReceiveScalarFlux(MeshBlock *pmb, int stage) {
...
TaskStatus MatterTaskList::IntegrateScalars(MeshBlock *pmb, int stage) {
  PassiveScalars *ps = pmb->pscalars;
  if (stage <= nstages) {
...
TaskStatus MatterTaskList::SendScalars(MeshBlock *pmb, int stage) {
  if (stage <= nstages) {
...
TaskStatus MatterTaskList::ReceiveScalars(MeshBlock *pmb, int stage) {
  bool ret;
  if (stage <= nstages) {
...
TaskStatus MatterTaskList::SetBoundariesScalars(MeshBlock *pmb, int stage) {
  if (stage <= nstages) {
...
TaskStatus MatterTaskList::DiffuseScalars(MeshBlock *pmb, int stage) {
  PassiveScalars *ps = pmb->pscalars;
  Hydro *ph = pmb->phydro;
  // return if there are no diffusion to be added
...
//printf("phybdyz4c\n");
  ...
  if (stage <= nstages) {
    ...
    // Scaled coefficient for RHS time-advance within stage
    Real dt = (stage_wghts[(stage-1)].beta)*(pmb->pmy_mesh->dt);
//TODO VC/CC issue here use correct boundary function:
/*
//only ever calls prefer_vc option
    if (PREFER_VC) {
...
#ifdef Z4C_ASSERT_FINITE
TaskStatus MatterTaskList::AssertFinite(MeshBlock *pmb, int stage) {
  if (stage != nstages) return TaskStatus::success; // only do on last stage
...
TaskStatus MatterTaskList::DebugMonolithic(MeshBlock *pmb, int stage)
{
  ...
  if (stage <= nstages)
  {
    ...
    // set up some constants for integration etc ------------------------------
    Real ave_wghts_1[3];
    ave_wghts_1[0] = 1.0;
    ave_wghts_1[1] = stage_wghts[stage-1].delta;
    ave_wghts_1[2] = 0.0;
    ...
    // IntegrateZ4c
    pz4c->WeightedAve(pz4c->storage.u1, pz4c->storage.u,
                      pz4c->storage.u2, ave_wghts_1);
    pz4c->WeightedAve(pz4c->storage.u, pz4c->storage.u1,
                      pz4c->storage.u2, ave_wghts_2);
    pz4c->AddZ4cRHS(pz4c->storage.rhs, stage_wghts[stage-1].beta,
                    pz4c->storage.u);
