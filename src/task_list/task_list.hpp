...
public:
  ...
  TaskStatus UserWork(MeshBlock *pmb, int stage);               // USERWORK     [x]
  // TaskStatus NewBlockTimeStep(MeshBlock *pmb, int stage);       // NEW_DT       [x]
  // TaskStatus AddToADMMatter(MeshBlock *pmb, int stage);         // ADD_ADM      [x]
  TaskStatus CalcClosure(MeshBlock *pmb, int stage);            // CALC_CL      [x]
  TaskStatus CalcOpacity(MeshBlock *pmb, int stage);            // CALC_OP      [x]
  TaskStatus CalcFiducialVelocity(MeshBlock *pmb, int stage);   // CALC_FV      [x]
  TaskStatus CalcFluxes(MeshBlock *pmb, int stage);             // CALC_FLX     [x]
  TaskStatus GRSources(MeshBlock *pmb, int stage);              // GR_SRC       [x]
  TaskStatus CalcUpdate(MeshBlock *pmb, int stage);             // CALC_UPDT    [x]
...
namespace M1IntegratorTaskNames {
  ...
  const TaskID NONE(0);
  const TaskID CLEAR_ALLBND(1);
  const TaskID ADD_ADM(2);
  const TaskID CALC_CL(3);
  const TaskID CALC_OP(4);
  const TaskID CALC_FV(5);
  const TaskID CALC_FLX(6);
  const TaskID GR_SRC(7);
  const TaskID CALC_UPDT(8);
