...
public:
  ...
  enum TaskStatus CalculateVwaveRHS(MeshBlock *pmb, int stage);
  ...
  enum TaskStatus VwaveIntegrate(MeshBlock *pmb, int stage);
  ...
  enum TaskStatus VwaveSend(MeshBlock *pmb, int stage);
  ...
  enum TaskStatus VwaveReceive(MeshBlock *pmb, int stage);
...
namespace HydroIntegratorTaskNames {
  ...
  const uint64_t SEND_HYDSH=1LL<<50;
  const uint64_t SEND_EMFSH=1LL<<51;
  const uint64_t SEND_FLDSH=1LL<<52;
  const uint64_t RECV_HYDSH=1LL<<53;
  const uint64_t RECV_EMFSH=1LL<<54;
  const uint64_t RECV_FLDSH=1LL<<55;
  const uint64_t RMAP_EMFSH=1LL<<56;

  const uint64_t DIFFUSE_HYD=1LL<<57;
  const uint64_t DIFFUSE_FLD=1LL<<58;
  const uint64_t CALC_DIFFUSIVITY=1LL<<59;
