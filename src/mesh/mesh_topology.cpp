...
//  \brief Implementations for mesh topology utilities: neighbor search, buffer
//  ID logic,
//         boundary flag parsing, and NeighborBlock::SetNeighbor.
//
//  Extracted from the former src/bvals/bvals_base.cpp and
//  src/bvals/utils/boundary_flag.cpp.
...
//----------------------------------------------------------------------------------------
// Static topology tables (zero-initialized by default for static storage
// duration)
...
void NeighborBlock::SetNeighbor(int irank,
                                int ilevel,
                                int igid,
                                int ilid,
                                int iox1,
...
)
{
  ...
  targetid  = itargetid;
  polar     = ipolar;
  neighbor_all_same_level =
    false;  // conservative default; set by SearchAndSetNeighbors
...
//----------------------------------------------------------------------------------------
// SearchAndSetNeighbors -- walks the MeshBlockTree to fill neighbor[],
// nneighbor, nblevel for a given MeshBlock.  block_bcs and loc are read from
// pmb->nc_ and pmb->loc. Friend access to NeighborConnectivity private members
// (neighbor_, nneighbor_, nblevel_).

void mesh_topology::SearchAndSetNeighbors(MeshBlock* pmb,
                                          MeshBlockTree& tree,
                                          int* ranklist,
                                          int* nslist)
{
  ...
  for (int n = -1; n <= 1; n += 2)
  {
    ...
    {  // neighbor at finer level
      ...
      for (int f2 = 0; f2 < nf2; f2++)
      {
        for (int f1 = 0; f1 < nf1; f1++)
        {
          ...
          neighbor[nneighbor].SetNeighbor(ranklist[fid],
                                          nlevel,
                                          fid,
                                          fid - nslist[ranklist[fid]],
                                          n,
                                          0,
                                          0,
                                          NeighborConnect::face,
                                          bid,
                                          tbid,
                                          false,
                                          f1,
    ...
    {  // same or coarser level
      ...
      neighbor[nneighbor].SetNeighbor(ranklist[nid],
                                      nlevel,
                                      nid,
                                      nid - nslist[ranklist[nid]],
                                      n,
                                      0,
                                      0,
                                      NeighborConnect::face,
  ...
  for (int n = -1; n <= 1; n += 2)
  {
    ...
    {  // same or coarser
      ...
      {
        if ((n == -1 &&
             nc.block_bcs_[BoundaryFace::inner_x2] == BoundaryFlag::polar) ||
            (n == 1 &&
             nc.block_bcs_[BoundaryFace::outer_x2] == BoundaryFlag::polar))
        {
          polar = true;
        }
      ...
      nc.block_bcs_[BoundaryFace::inner_x2] == BoundaryFlag::polar_wedge)
  {
    ...
    int num_north_polar_blocks =
      static_cast<int>(pmy_mesh->GetNrbx3() * (1 << level));
...
#if defined(DBG_NO_REF_NN_SAME_LEVEL)
  // Cache the tree-based "all neighbors same level" flag on the MeshBlock so
  // that NeighborBlocksSameLevel() (receiver side) returns the same predicate
  // as nb.neighbor_all_same_level (sender side).  The old list-based
  // computation could disagree due to edge/corner neighbor pruning, causing
  // MPI buffer size mismatch -> truncation -> hang.
  {
    MeshBlockTree* my_node = tree.FindMeshBlock(loc);
    pmb->all_neighbors_same_level_cached_ =
      (my_node != nullptr) ? my_node->all_neighbors_same_level_ : false;
  }
#endif
}
