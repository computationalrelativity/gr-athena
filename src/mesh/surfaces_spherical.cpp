...
SurfacesSpherical::SurfacesSpherical(Mesh* pm,
                                     ParameterInput* pin,
                                     const int par_ix)
    : Surfaces(pm, pin, par_ix)
{
  ...
  {
    psurf.push_back(
      new SurfaceSpherical(pm, pin, dynamic_cast<Surfaces*>(this), surf_ix));
  }
}
...
SurfaceSpherical::SurfaceSpherical(Mesh* pm,
                                   ParameterInput* pin,
                                   Surfaces* psurfs,
                                   const int surf_ix)
    : Surface(pm, pin, psurfs, surf_ix)
{
  ...
  aliases::AA nth =
    pin->GetOrAddRealArray(psurfs->par_block_name, "nth", -1, 0);
  aliases::AA nph =
    pin->GetOrAddRealArray(psurfs->par_block_name, "nph", -1, 0);

  if ((radii.GetSize() == 0) || (nth.GetSize() == 0) || (nph.GetSize() == 0))
  {
    std::ostringstream msg;
...
Real SurfaceSpherical::InterpolateAtPoint(aliases::AA& raw_cpt,
