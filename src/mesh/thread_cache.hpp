...
//  \brief Per-thread scratch data for eliminating redundant computation across
//  passes.
//
//  Currently holds:
//    1. Face-centered (FC) geometry fields cached during the high-order flux
//    pass
//       so that the low-order fallback pass (hybridization) can reuse them
//       without re-interpolating the spacetime metric to face centers.
//    2. Low-order fallback flux scratch arrays (lo_hflux, lo_sflux) used
//    during
...
//       M1's property-preservation hybridization - moved here from
//       per-MeshBlock M1::storage.flux_lo to save memory (~5 MB per MeshBlock
//       * nMeshBlocks
//       -> ~5 MB * nthreads).
//
...
struct ThreadCache
{
  ...
  enum FCGeomField
  {
    ...
    FC_GZZ,
    FC_SQRT_DETGAMMA,
    FC_GAMMA_UU_DIAG,  // only the diagonal component gamma^{dd} for flux
                       // direction d
    N_FC_GEOM_FIELDS   // = 12
  };
  ...
  AthenaArray<Real>
    lo_hflux[3];  // (NHYDRO,  ncells3, ncells2, ncells1+1) etc.
  AthenaArray<Real>
  ...
  // Dimensions: (ixn_Lab::N * N_GRPS * N_SPCS, ncells3, ncells2, ncells1+1)
  // etc.
  ...
  void AllocateLOFlux(int ncells1, int ncells2, int ncells3, bool f2, bool f3)
  {
    ...
    if (NHYDRO > 0)
    {
      ...
      lo_hflux[1] = AthenaArray<Real>(NHYDRO,
                                      ncells3,
                                      ncells2 + 1,
                                      ncells1,
                                      (f2 ? DS::allocated : DS::empty));
      lo_hflux[2] = AthenaArray<Real>(NHYDRO,
                                      ncells3 + 1,
                                      ncells2,
                                      ncells1,
    ...
    if (NSCALARS > 0)
    {
      ...
      lo_sflux[1] = AthenaArray<Real>(NSCALARS,
                                      ncells3,
                                      ncells2 + 1,
                                      ncells1,
                                      (f2 ? DS::allocated : DS::empty));
      lo_sflux[2] = AthenaArray<Real>(NSCALARS,
                                      ncells3 + 1,
                                      ncells2,
                                      ncells1,
                                      (f3 ? DS::allocated : DS::empty));
  ...
  //  \brief Write the 12 FC geometry fields for a single pencil (k,j) in
  //  direction dir.
  //
  //  dir: 0=X1, 1=X2, 2=X3 (i.e. ivx-1)
  //  ivx: 1=IVX, 2=IVY, 3=IVZ - used only to select which gamma^{dd} diagonal
  //  to store

  inline void StoreFCGeometry(int dir,
                              int k,
                              int j,
                              int il,
                              int iu,
...
#pragma omp simd
  ...
  //  \brief Read 12 primary fields from cache.

  inline void LoadFCGeometry(int dir,
                             int k,
                             int j,
                             int il,
                             int iu,
