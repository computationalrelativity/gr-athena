...
// ----------------------------------------------------------------------------
void Reconstruction::ReconstructPPMX1(AthenaArray<Real> &z,
...
#pragma omp simd
    for (int i=il; i<=iu; ++i) {
      ...
      q_ip1(n_tar,i) = z(n_src,k,j,i+1);
      q_ip2(n_tar,i) = z(n_src,k,j,i+2);
    }
...
void Reconstruction::ReconstructPPMX2(AthenaArray<Real> &z,
                                      AthenaArray<Real> &zl_,
                                      AthenaArray<Real> &zr_,
                                      const int n_tar,
                                      const int n_src,
...
#pragma omp simd
  for (int i=il; i<=iu; ++i) {
    ...
    q_jp1(n_tar,i) = z(n_src,k,j+1,i);
    q_jp2(n_tar,i) = z(n_src,k,j+2,i);
  }
...
void Reconstruction::ReconstructPPMX3(AthenaArray<Real> &z,
                                      AthenaArray<Real> &zl_,
                                      AthenaArray<Real> &zr_,
                                      const int n_tar,
                                      const int n_src,
...
#pragma omp simd
  for (int i=il; i<=iu; ++i) {
    ...
    q_kp1(n_tar,i) = z(n_src,k+1,j,i);
    q_kp2(n_tar,i) = z(n_src,k+2,j,i);
  }
