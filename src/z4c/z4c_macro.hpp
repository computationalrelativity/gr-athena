...
#define SW_CC_CX_VC(a, b, c) a
#elif defined(Z4C_CX_ENABLED)
#define SW_CC_CX_VC(a, b, c) b
#else
#define SW_CC_CX_VC(a, b, c) c
#endif

#if defined(Z4C_CC_ENABLED) || defined(Z4C_CX_ENABLED)
#define SW_CCX_VC(a, b) a
#else
#define SW_CCX_VC(a, b) b
#endif

// BD: allow replacement slots with function-like specification
#if defined(Z4C_CC_ENABLED)
#define FCN_CC_CX_VC(A, B, C) A
#elif defined(Z4C_CX_ENABLED)
#define FCN_CC_CX_VC(A, B, C) B
#else
#define FCN_CC_CX_VC(A, B, C) C
#endif

#define IX_IL pz4c->mbi.il

#define IX_IU pz4c->mbi.iu

#define IX_JL pz4c->mbi.jl

#define IX_JU pz4c->mbi.ju

#define IX_KL pz4c->mbi.kl

#define IX_KU pz4c->mbi.ku

#define GSIZEI (pz4c->mbi.ng)

#define GSIZEJ ((pz4c->mbi.f2) ? (pz4c->mbi.ng) : (0))

#define GSIZEK ((pz4c->mbi.f3) ? (pz4c->mbi.ng) : (0))
...
#define ILOOP1(i) _Pragma("omp simd") for (int i = IX_IL; i <= IX_IU; ++i)

// 1D loop over i on the whole block
#define GLOOP1(i) \
  _Pragma("omp simd") for (int i = IX_IL - GSIZEI; i <= IX_IU + GSIZEI; ++i)
...
#define NS_ILOOP1(i) for (int i = IX_IL; i <= IX_IU; ++i)

#define NS_ILOOP3(k, j, i) \
  ILOOP2(k, j)             \
  NS_ILOOP1(i)

#define NS_GLOOP1(i) for (int i = IX_IL - GSIZEI; i <= IX_IU + GSIZEI; ++i)

#define NS_GLOOP3(k, j, i) \
  GLOOP2(k, j)             \
  NS_GLOOP1(i)
