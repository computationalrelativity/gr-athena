...
Real Z4c::NewBlockTimeStep(void) {
  ...
  Real min_dt_parabolic = min_dt;
  Real min_dt_user = min_dt;
  ...
  if (pmb->pmy_mesh->UserTimeStep_ != nullptr) {
    min_dt_user = pmb->pmy_mesh->UserTimeStep_(pmb);
  ...
  pmb->new_block_dt_hyperbolic_ = min_dt;
