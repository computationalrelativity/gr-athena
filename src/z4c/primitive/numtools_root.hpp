...
class Root
{
  ...
  // argument is assumed to be the quantity of interest. All other arguments
  // are assumed to be constant parameters for the function. The root-finding
  // method is the Illinois variant of false position.
  //
  ...
  inline bool FalsePosition(Functor&& f,
                            Real& lb,
                            Real& ub,
                            Real& x,
                            Real tol,
  ...
  // argument is assumed to be the quantity of interest. All other arguments
  // are assumed to be constant parameters for the function. The root-finding
  // method is Chandrupatla's method, a simpler alternative to Brent's method
  ...
  template <class Functor, class... Types>
  inline bool Chandrupatla(Functor&& f,
                           Real& lb,
                           Real& ub,
                           Real& x,
                           Real tol,
                           Types... args)
  {
    ...
    do
    {
      ...
      // Calculate f at the prospective root
      ftest = f(x, args...);
      // Robust relative-x tolerance + functional tolerance.
      if (std::fabs(x - x1) <= tol * (std::fabs(x) + tol))
      {
        break;
      }
      if (std::fabs(ftest) <= tol)
   ...
   *
   * A safe Newton solve performs a Newton-Raphson solve, but it also brackets
   * the root using bisection to ensure that the result converges.
   *
   * \param[in]     f     The functor to find a root for. Its root function
   ...
   */
  template <class Functor, class... Types>
  inline bool NewtonSafe(Functor&& f,
                         Real& lb,
                         Real& ub,
                         Real& x,
                         Real tol,
                         Types... args)
  {
    ...
    do
    {
      ...
      f(fx, dfx, x, args...);
      // Functional tolerance: accept convergence when |f| is within tol.
      if (std::fabs(fx) <= tol)
      {
        break;
      }
  ...
  template <class T>
  T quadratic_interpolate(const T& a,
                          const T& b,
                          T const& d,
                          const T& fa,
                          const T& fb,
  ...
  T toms748_solve(F&& f, const T& ax, const T& bx, const T& tol, Types... args)
  {
    ...
    {
      // Robust relative-or-absolute width tolerance.  The original
      // form ``|a-b| <= tol * min(a,b)`` degenerated at min(a,b)=0
      // (common when the lower bracket is 0) and also failed to
      // terminate when |a|,|b| differed vastly.  We now normalise by
      // max(|a|,|b|) + tol so the criterion remains well-defined and
      // meaningful even at or near zero.
      return std::abs(a - b) <=
             tol * (std::max(std::abs(a), std::abs(b)) + tol);
    };
    ...
    // Early-exit on endpoint within functional tolerance (matches the
    // pre-checks used by FalsePosition / Chandrupatla / NewtonSafe).
    if (std::fabs(fa) <= tol)
    {
      f(a, args...);
      return a;
    ...
    if (tolf(a, b) || (std::fabs(fa) <= tol) || (std::fabs(fb) <= tol))
    {
      max_iter = 0;
      if (std::fabs(fa) <= tol)
        b = a;
      else if (std::fabs(fb) <= tol)
        a = b;
