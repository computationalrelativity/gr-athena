//! \file cold_eos_transition.cpp
//  \brief Implementation of ColdEOSTransition
...
void ColdEOSTransition::ReadColdSliceFromFile(std::string fname)
{
  ...
  // Read the density index to cut the Lorene table
  ...
  // accumulated on top of it; those datasets are optional and are lumped
  // into SCXA as bound light nuclei (Z/A ~ 0.5), consistent with
  // EOSCompOSE::ReadTableFromFile.
  ...
  if (ierr == 0)
  {
    ...
    {
      // convert from abundance to mass fraction
      m_table[index(ECY + SCXA, in)] += max(0.0, min(scratch[in] * 2.0, 1.0));
    }
  }
  ...
  for (int in = 0; in < m_np; ++in)
  {
    m_table[index(ECH, in)] =
      ...
      exp(m_table[index(ECLOGN, in)]);
  }

  //  fill dPdn: D0_x_2 gives dlogP/dlogn, convert with P/n
  ...
  for (int in = 0; in < m_np; ++in)
  {
    ...
    if (SumX <= 0.0)
    {
      ...
      {
        printf(
          "Warning: sum of mass fractions at index %d is %e, cannot "
          "normalize\n",
          in,
          SumX);
      }
