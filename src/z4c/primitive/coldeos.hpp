...
class ColdEOS : public ColdEOSPolicy
{
  ...
  using ColdEOSPolicy::DensityFromEnergy;
  ...
  //
  //  \param[in] rho  The density
  //  \return The derivative of the pressure wrt. the density according to the
  //  cold EOS slice.
  Real GetdPdrho(Real rho)
  {
    Real n = rho / GetBaryonMass();
    return dPdn(n) * eos_units->PressureConversion(*code_units) /
           GetBaryonMass();
  }
  ...
  {
    Real n = rho / GetBaryonMass();
    return Enthalpy(n) / mb * eos_units->EnergyConversion(*code_units) /
           eos_units->MassConversion(*code_units);
  }
  ...
  {
    Real n =
      DensityFromPressure(P * code_units->PressureConversion(*eos_units));
    return n * GetBaryonMass();
  }

  //! \fn Real GetDensityFromEnergy(Real E)
  //
  //  \param[in] E  The energy density
  //  \return The density according to the cold EOS slice.
  Real GetDensityFromEnergy(Real E)
  {
    Real n = DensityFromEnergy(E * code_units->EnergyConversion(*eos_units) *
                               code_units->DensityConversion(*eos_units));
    return n * GetBaryonMass();
  }
