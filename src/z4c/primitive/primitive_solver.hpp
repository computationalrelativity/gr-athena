...
class PrimitiveSolver
{
  ...
  class UpperRootFunctor
  {
                           ...
                           Real min_h)
    {
      ...
      // const Real drbarsq = rbsq*x*(1.0 + x) + (mu*rbsq + 2.0*(mu*rbsq +
      // rsq)*x)*dx;
      const Real drbarsq = rbsq * xsq + mu * rbsq * dx +
                           x * (rbsq + 2.0 * (mu * rbsq + rsq) * dx);
      f  = mu * dis - 1.0;
  ...
  class MuFromWFunctor
  {
                           ...
                           Real W)
    {
      ...
      const Real drbarsq = rbsq * xsq + mu * rbsq * dx +
                           x * (rbsq + 2.0 * (mu * rbsq + rsq) * dx);
      // const Real drbarsq = 2.0*rsq*dx + x*(1.0 + x)*rbsq + mu*dx*(1.0
      // + 2.0*x)*rbsq;
  ...
  class RootFunctor
  {
                           ...
                           Real* P)
    {
      ...
      const Real qbar =
        q - 0.5 * bsq - 0.5 * musq * xsq * std::fma(bsq, rsq, -rbsq);
      ...
      // NOTE: for extreme velocities, this alternative form of W may be more
      // accurate: Wsq = 1/(eps*(2 - eps)) = 1/(eps*(1 + v)), where eps = 1 -
      // v.
  ...
  Error CheckDensityValid(Real& mul,
                          Real& muh,
                          Real D,
                          Real bsq,
                          Real rsq,
  ...
  SolverResult ConToPrim(Real prim[NPRIM],
                         Real cons[NCONS],
                         Real b[NMAG],
                         Real g3d[NSPMETRIC],
                         Real g3u[NSPMETRIC]);
  ...
  Error PrimToCon(Real prim[NPRIM],
                  Real cons[NCONS],
                  Real b[NMAG],
                  Real g3d[NSPMETRIC]);
...
// CheckDensityValid {{{
template <typename EOSPolicy, typename ErrorPolicy>
inline Error PrimitiveSolver<EOSPolicy, ErrorPolicy>::CheckDensityValid(
  Real& mul,
  Real& muh,
  Real D,
  Real bsq,
  ...
  if (D > rho_max)
  {
    ...
    else
    {
      ...
      if (f < 0)
      {
        ...
        bool result =
          root.NewtonSafe(MuFromW, mulc, muhc, mu, 1e-10, bsq, rsq, rbsq, W);
        if (!result)
        {
...
// ConToPrim {{{
template <typename EOSPolicy, typename ErrorPolicy>
inline SolverResult PrimitiveSolver<EOSPolicy, ErrorPolicy>::ConToPrim(
  Real prim[NPRIM],
  Real cons[NCONS],
  Real b[NMAG],
  Real g3d[NSPMETRIC],
  Real g3u[NSPMETRIC])
{
  ...
  if (tighten_bracket)
  {
    ...
    if (rsqr > min_h * min_h)
    {
      ...
      Real mulc   = mul;
      Real mulh   = muh;
      bool result = root.NewtonSafe(
        UpperRoot, mulc, mulh, mu, 1e-10, bsqr, rsqr, rbsqr, min_h);
      ...
      {
        // To avoid problems with the case where the root and the upper bound
        // collide, we will perturb the bound slightly upward.
  ...
  if (validate_density)
  {
    ...
    // TODO: This is probably something that should be handled by the
    // ErrorPolicy.
  ...
  if (use_toms_748)
  {
    mu     = root.toms748_solve(RootFunction,
                            mul,
                            muh,
                            tol,
                            D,
  ...
  {
    result = root.FalsePosition(RootFunction,
                                mul,
                                muh,
                                mu,
                                tol,
  ...
  // only be trusted on single-thread benchmarks.
...
// PrimToCon {{{
template <typename EOSPolicy, typename ErrorPolicy>
inline Error PrimitiveSolver<EOSPolicy, ErrorPolicy>::PrimToCon(
  Real prim[NPRIM],
  Real cons[NCONS],
  Real bu[NMAG],
  Real g3d[NSPMETRIC])
