...
void EOSCompOSE::ReadTableFromFile(std::string fname) {
  #pragma omp critical (EOSCompose_ReadTable)
  {
    if (m_initialized==false) {
      ...
      // printf("Loading thread has table at %p\n",m_table);
      // printf("Loading thread has flag at %p\n",&m_initialized);
      ...
      // printf("loader has table at %p\n",m_table);
      // printf("loader has table[0,0,0,0] = %16e\n",m_table[0]);
  ...
  // printf("reader has table at %p\n",m_table);
  // printf("reader has table[0,0,0,0] = %16e\n",m_table[0]);
}
