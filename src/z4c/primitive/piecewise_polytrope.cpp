...
int PiecewisePolytrope::FindPiece(Real n) const
{
  ...
  if (!initialized)
  {
    throw std::runtime_error(
      "PiecewisePolytrope::FindPiece - EOS not initialized.");
  }
...
{
  return mb * n * (1.0 + eps_pieces[p]) +
         GetColdPressure(n, p) / (gamma_pieces[p] - 1.0);
}

Real PiecewisePolytrope::GetColdPressure(Real n, int p)
{
  return pressure_pieces[p] *
         std::pow((n / density_pieces[p]), gamma_pieces[p]);
}
...
Real PiecewisePolytrope::TemperatureFromEntropy(Real n, Real s, Real* Y)
{
  throw std::logic_error(
    "PiecewisePolytrope::TemperatureFromEntropy not currently implemented.");
}
...
void PiecewisePolytrope::PressureAndEnthalpy(Real n,
                                             Real T,
                                             Real* Y,
                                             Real* P,
                                             Real* h)
...
void PiecewisePolytrope::TemperaturePressureAndEnthalpyFromE(Real n,
                                                             Real e,
                                                             Real* Y,
                                                             Real* T,
                                                             Real* P,
...
void PiecewisePolytrope::PressureAndEnthalpyFromE(Real n,
                                                  Real e,
                                                  Real* Y,
                                                  Real* P,
                                                  Real* h,
...
{
  throw std::logic_error(
    "PiecewisePolytrope::FrYn not currently implemented.");
}
...
{
  throw std::logic_error(
    "PiecewisePolytrope::BaryonChemicalPotential not currently implemented.");
}

Real PiecewisePolytrope::ChargeChemicalPotential(Real n, Real T, Real* Y)
{
  throw std::logic_error(
    "PiecewisePolytrope::ChargeChemicalPotential not currently implemented.");
}

Real PiecewisePolytrope::ElectronLeptonChemicalPotential(Real n,
                                                         Real T,
                                                         Real* Y)
{
  throw std::logic_error(
    "PiecewisePolytrope::ElectronLeptonChemicalPotential not currently "
    "implemented.");
}
                                            ...
                                            int n)
{
  ...
  {
    printf(
      "PiecewisePolytrope: Invalid number of polytropes requested.");  // NOLINT
    return false;
  }
  ...
  for (int i = 1; i < n; i++)
  {
    if (densities[i] <= densities[i - 1])
    {
      ...
      // increasing.
      printf(
        "PiecewisePolytrope: Densities must be strictly increasing.");  // NOLINT
      return false;
    }
  }
  ...
  //
  // Note that we store densities 1 twice, because on the first segment we need
  // to write the pressure in terms of rho1 and not rho0 (which would give a
  // division by zero)
  ...
  // the only restriction on the temperature is that it needs to be
  // nonnegative.
  min_T = 0.0;
  ...
  // std::cout << "  P0 = " << P0 << "\n";
  // std::cout << "  mb = " << mb << "\n";
  // std::cout << "  gamma_thermal = " << gamma_thermal << "\n";
...
void PiecewisePolytrope::SetNSpecies(int n)
{
  if (n > MAX_SPECIES || n < 0)
  {
    throw std::out_of_range(
      "PiecewisePolytrope::SetNSpecies - n cannot exceed MAX_SPECIES.");
  }
  n_species = n;
}
