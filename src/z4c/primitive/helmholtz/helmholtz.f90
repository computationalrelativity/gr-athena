module helm_eos
   use, intrinsic :: iso_c_binding, only: c_char, c_bool, c_double, c_int
   use quintic_hermite
   implicit none
...
!
  ! routine read_table reads an electron helm free energy table
...
! open the file (use softlinks to input the desired table)
      ...
      open (unit=19, file=helm_table_path, status='old')
...
!       write(6,*) 'read dpdd table'
      ...
      ! read the electron continue hemical potential table
       do j=1,jmax
        do i=1,imax
...
!!$      ! other quantites such the normalized character emical potential eta (plus its
...
!!$
!!$      double precision sioncon,forth,forpi,asoli3,light2
...
!!$        x       = abar*abar*sqrt(abar) * deni/imb
!!$        s       = sioncon * temp
!!$        z       = x * s * sqrt(s)
!!$        y       = log(z)
!!$
...
!!$                  + 1.5d0 * kerg_mb * tempi*ytot1
!!$        x       = imb*kerg/abar
!!$        dsionda = (dpionda*deni + deionda)*tempi &
!!$                  + kerg_mb*ytot1*ytot1* (2.5d0 - y)
!!$        dsiondz = 0.0d0
!!$
...
!!$
!!$         y        = imb*ytot1*kt*(a1 + 0.25d0/plasg*(b1*x - c1/x))
!!$         decouldd = y * plasgdd
...
!!$         dpcouldz = y * decouldz
!!$
!!$         y        = -imb*kerg/(abar*plasg)*(0.75d0*b1*x+1.25d0*c1/x+d1)
!!$         dscouldd = y * plasgdd
!!$         dscouldt = y * plasgdt
...
!!$         scoul    = -imb/abar*kerg*(c2*x -a2*(b2-1.0d0)/b2*y)
!!$
...
!!$         decouldz = s * dpcouldz
!!$
!!$         s        = -imb*kerg/(abar*plasg)*(1.5d0*c2*x-a2*(b2-1.0d0)*y)
!!$         dscouldd = s * plasgdd
!!$         dscouldt = s * plasgdt
...
!!$      end
   ...
   subroutine bombproof(den, temp, ye, success)
      include 'implno.dek'
      include 'helm_table_storage.dek'
      ...
      if (temp .gt. t(jmax)) then
       write(6,'(1x, 5(a, 1pe18.10))') 'temp=',temp,' t(jmax)=',t(jmax)
       write(6,*) 'temp too hot, off grid'
       !success = .false.
       temp = t(jmax)
      end if
      if (temp .lt. t(1)) then
       write(6,'(1x, 5(a, 1pe18.10))') 'temp=',temp,' t(1)=',t(1)
       write(6,*) 'temp too cold, off grid'
       !success = .false.
       temp = t(1)
      end if
   ...
   subroutine helm_etot(den, temp, Abar, Zbar, &
     etot, success) bind(C, name="helm_etot")
      ...
      real(8) :: ye, din, deni, kt, erad, eion
      real(8), dimension(36) :: fi
      real(8) :: free, df_t, sele, eele, &
                 xt, xd, mxt, mxd, &
                 si0t, si1t, si2t, si0mt, si1mt, si2mt, &
                 si0d, si1d, si2d, si0md, si1md, si2md
      ...
      real(8) :: dsi0t, dsi1t, dsi2t, dsi0mt, dsi1mt, dsi2mt
      ...
      call bombproof(den, temp, ye, success)
      if (.not. success) then
        write (6, *) 'fail in helm_etot'
      end if
      din  = ye*den
      deni = 1.0d0/den
      kt   = kerg * temp

      erad = asol * temp * temp * temp * temp * deni
      eion = 1.5d0 * imb * den * kt * deni /abar
   ...
   end subroutine helm_etot

   subroutine helm_ptot(den, temp, Abar, Zbar, &
     ptot, success) bind(C, name="helm_ptot")
      include 'implno.dek'
      include 'const.dek'
      include 'helm_table_storage.dek'
   ...
   end subroutine helm_ptot

   subroutine helm_cs(den, temp, Abar, Zbar, &
     cs, success) bind(C, name="helm_cs")
      include 'implno.dek'
      include 'const.dek'
      include 'helm_table_storage.dek'
      ...
      real(8) :: free, df_d, df_t, df_dt, df_tt, &
                 pion, eion, prad, erad, &
                 dpiondt, dpiondd, deiondt, &
                 dpraddt, dpraddd, deraddt, &
                 pele, sele, eele, &
                 dpepdt, dsepdt, deepdt, dpepdd, &
                 pres, ener, &
                 dpresdd, dpresdt, denerdt, &
                 z, zz, zzi, chit, chid, x, gam1
      ...
      prad = asol/3.0d0 * temp * temp * temp * temp
      dpraddd = 0.0d0
      dpraddt = 4.0d0/3.0d0 * asol * temp * temp * temp
      ...
      ! floating point limit of the subtraction of two large terms.
      ! since dpresdd doesn't enter the maxwell relations at all, use the
      ! bicubic interpolation instead of the formally correct expression
      fi(1)  = dpdf(iat,jat)
      fi(2)  = dpdf(iat+1,jat)
      fi(3)  = dpdf(iat,jat+1)
      fi(4)  = dpdf(iat+1,jat+1)
      ...
      chit  = temp/pres * dpresdt
      chid  = dpresdd*zzi
      x     = zz * chit/(temp * denerdt)
      gam1  = chit*x + chid
      z     = 1.0d0 + (ener + clight*clight)*zzi
      cs    = clight * sqrt(gam1/z)
   end subroutine helm_cs
...
!!$
!!$      if (.not. has_mb) then
!!$         write(*,*) 'Error: baryon mass must be set before calleng Helmholtz EOS'
!!$         return
!!$      end if
!!$
...
!!$   end subroutine helm_eos_wrap
   ...
   subroutine read_helm_table(&
     helm_table_path, str_len&
   ) bind(C, name="read_helm_table")
      implicit none
      integer(c_int), intent(in) :: str_len
      character(kind=c_char), intent(in) :: helm_table_path(*)
      ...
      call read_table(fortran_string)
      table_read = .true.
   ...
   subroutine check_bounds(&
     rho_trans, temp_trans, ye_max, success&
      ...
      if (.not. table_read) then
         write (6, *) 'must read helm_table.dat before checking bounds'
         return
      end if

      if (temp_trans > t(jmax)) then
         write (6, *) 'temp_trans > temp_helm_max'
         write (6, *) 'temp_helm_max = ', t(jmax)
         return
      end if
