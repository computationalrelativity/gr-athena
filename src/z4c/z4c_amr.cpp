...
pin(pin)
{
  ...
  ref_method = pin->GetOrAddString("z4c_amr",
                                   "method",
                                   "Linf_box_in_box");
  ref_x1min = pin->GetOrAddReal("z4c_amr",
                                "x1min",
                                pin->GetReal("mesh","x1min")/2.);
  ref_x1max = pin->GetOrAddReal("z4c_amr",
                                "x1max",
                                pin->GetReal("mesh","x1max")/2.);
  ref_x2min = pin->GetOrAddReal("z4c_amr",
                                "x2min",
                                pin->GetReal("mesh","x2min")/2.);
  ref_x2max = pin->GetOrAddReal("z4c_amr",
                                "x2max",
                                pin->GetReal("mesh","x2max")/2.);
  ref_x3min = pin->GetOrAddReal("z4c_amr",
                                "x3min",
                                pin->GetReal("mesh","x3min")/2.);
  ref_x3max = pin->GetOrAddReal("z4c_amr",
                                "x3max",
                                pin->GetReal("mesh","x3max")/2.);
  ...
  ref_FD_r2_inn  = pin->GetOrAddReal("z4c_amr","FD_radius2_inn",
                                     ref_FD_r1_out );
...
int Z4c_AMR::FDErrorApprox(MeshBlock *pmb, Real dref_tol, Real ref_tol)
{
  ...
  {
    if (Verbose) printf(
                 "dref-tol < err < ref-tol: %e < %e < %e ==> nothing %s.\n",
...
int Z4c_AMR::LinfBoxInBox(MeshBlock *pmb)
{
  ...
  Real L = (pmb->pmy_mesh->mesh_size.x1max -
            pmb->pmy_mesh->mesh_size.x1min)/2. - par_b;
  Real xv[24];
...
int Z4c_AMR::FDTruncError(MeshBlock *pmb)
{
  ...
  {
    if (Verbose)
      printf("Mb_radius = %g ==> calling FD AMR for the ring = [%g,%g], "
             "tol=[%g,%g]\n",
