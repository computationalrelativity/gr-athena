...
// BD: TODO- generalize derivative control properly.

// print the results
// note: at 'if (Verbose)' when Verbose = 0, the if block is ignored by
// the compiler
#define Verbose (0)
    ...
    : pz4c(z4c), pin(pin)
{
  ...
  // available methods:
  // "Linf_box_in_box", "L2_sphere_in_sphere", and "fd_truncation_error"
  ref_method = pin->GetOrAddString("z4c_amr", "method", "Linf_box_in_box");
  ref_x1min =
    pin->GetOrAddReal("z4c_amr", "x1min", pin->GetReal("mesh", "x1min") / 2.);
  ref_x1max =
    pin->GetOrAddReal("z4c_amr", "x1max", pin->GetReal("mesh", "x1max") / 2.);
  ref_x2min =
    pin->GetOrAddReal("z4c_amr", "x2min", pin->GetReal("mesh", "x2min") / 2.);
  ref_x2max =
    pin->GetOrAddReal("z4c_amr", "x2max", pin->GetReal("mesh", "x2max") / 2.);
  ref_x3min =
    pin->GetOrAddReal("z4c_amr", "x3min", pin->GetReal("mesh", "x3min") / 2.);
  ref_x3max =
    pin->GetOrAddReal("z4c_amr", "x3max", pin->GetReal("mesh", "x3max") / 2.);

  // specify how to compute truncation error
  ref_deriv = pin->GetOrAddInteger("z4c_amr", "deriv_order", 7);
  ref_pow   = pin->GetOrAddInteger("z4c_amr", "deriv_power", 1);

  // ensure the grid-space doesn't fall below specific number for GW extraction
  // (can be an order of total mass)
  ref_gwh = pin->GetOrAddReal("z4c_amr", "gw_gridspace", dmax);

  // where to check the above criterion
  // (can be the largest gw extraction radius)
  ref_gwr = pin->GetOrAddReal("z4c_amr", "gw_radius", dmax);

  // one can specify different criteria for different annuli using finite
  // difference truncation error
  ref_FD_r1_inn = pin->GetOrAddReal("z4c_amr", "FD_radius1_inn", 0.);
  ref_FD_r1_out =
    pin->GetOrAddReal("z4c_amr",
                      "FD_radius1_out",
                      2.2 * pin->GetOrAddReal("problem", "par_b", 1.));
  ref_FD_r2_inn =
    pin->GetOrAddReal("z4c_amr", "FD_radius2_inn", ref_FD_r1_out);
  ref_FD_r2_out =
    pin->GetOrAddReal("z4c_amr",
                      "FD_radius2_out",
                      std::max(std::abs(ref_x3max - ref_x3min),
                               std::max(std::abs(ref_x1max - ref_x1min),
...
int Z4c_AMR::FDErrorApprox(MeshBlock* pmb, Real dref_tol, Real ref_tol)
{
  ...
  if (Verbose)
    sprintf(region,
            "[%0.1f,%0.1f]x[%0.1f,%0.1f]x[%0.1f,%0.1f]",
                                ...
                                const int p)
{
  ...
  Real derive_kji = 0.;
  const int npts =
    (IX_KU - IX_KL + 1) * (IX_JU - IX_KL + 1) * (IX_IU - IX_IL + 1);
...
// \brief returning the point-wise max error of a derivative of chi in a
// meshblock
//
                                  ...
                                  const int p)
{
  ...
  if (deriv_order == 7)
  {
    ...
    ILOOP3(k, j, i)
    {
      ...
      for (int a = 0; a < NDIM; ++a)
      {
        // derive_kji += std::pow(pz4c->fd->Dx_ho(a, z4c.chi(k,j,i)),p); // p
        // != 1
        derive_kji +=
          pz4c->fd->Dx_ho(a, z4c.chi(k, j, i));  // p = 1 (optimization)
      }
...
//----------------------------------------------------------------------------------------
// \!fn void Z4c:: FDTruncErrorChiLinfComponentWise(MeshBlock *const pmy_block,
// const int p)
// \brief returning the maximum truncation error of chi among x, y, and z
// directions and for all points in the given meshblock.
...
int Z4c_AMR::LinfBoxInBox(MeshBlock* pmb)
{
  ...
  // Initial distance between one of the punctures and the edge of the
  // full mesh, needed to calculate the box-in-box grid structure
  Real par_b = pin->GetOrAddReal("problem", "par_b", 1.);
  Real L =
    (pmb->pmy_mesh->mesh_size.x1max - pmb->pmy_mesh->mesh_size.x1min) / 2. -
    par_b;
  Real xv[24];
  ...
  Real x1sum_sup =
    (5 * pmb->block_size.x1max + 3 * pmb->block_size.x1min) / 8.;
  Real x1sum_inf =
    (3 * pmb->block_size.x1max + 5 * pmb->block_size.x1min) / 8.;
  Real x2sum_sup =
    (5 * pmb->block_size.x2max + 3 * pmb->block_size.x2min) / 8.;
  Real x2sum_inf =
    (3 * pmb->block_size.x2max + 5 * pmb->block_size.x2min) / 8.;
  Real x3sum_sup =
    (5 * pmb->block_size.x3max + 3 * pmb->block_size.x3min) / 8.;
  Real x3sum_inf =
    (3 * pmb->block_size.x3max + 5 * pmb->block_size.x3min) / 8.;
  ...
  for (int six = 0; six < pmb->pz4c->opt.sphere_zone_number; ++six)
  {
    ...
    if (pix != -1)
    {
      xyz_wz[0] = pmb->pmy_mesh->pz4c_tracker[pix]->GetPos(0);
      xyz_wz[1] = pmb->pmy_mesh->pz4c_tracker[pix]->GetPos(1);
      xyz_wz[2] = pmb->pmy_mesh->pz4c_tracker[pix]->GetPos(2);
    }
    else
    {
...
int Z4c_AMR::FDTruncError(MeshBlock* pmb)
{
  ...
  else if (ref_FD_r1_inn <= mb_radius && mb_radius <= ref_FD_r1_out)
  {
    if (Verbose)
      printf(
        "Mb_radius = %g ==> calling FD AMR for the ring = [%g,%g], "
        "tol=[%g,%g]\n",
        mb_radius,
        ref_FD_r1_inn,
        ref_FD_r1_out,
