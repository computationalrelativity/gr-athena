...
void Z4c::GetMatter(::AA& u_mat, ::AA& u_adm, ::AA& w, ::AA& r, ::AA& bb_cc)
{
  ...
  if (opt.Tmunuinterp == 0)
  {
    ...
    ILOOP2(k, j)
    {
...
#else
        Real const norm2_utilde =
          InnerProductSlicedVec3Metric(w_utilde_u, adm.g_dd, k, j, i);
...
#if MAGNETIC_FIELDS_ENABLED
        Real const r_sqrt_detgamma_i =
          OO(aux_extended.gs_sqrt_detgamma(k, j, i));
      ...
      for (int a = 0; a < NDIM; ++a)
      {
        ...
        {
          bi_u(a, i) = (bb_u(a, i) +
                        alpha_reg(i) * b0_u(i) * W(i) *
                          (v_u(a, i) - adm.beta_u(a, k, j, i) * oo_alpha(i))) /
                       W(i);
        }
      }
      ...
      for (int a = 0; a < NDIM; ++a)
      {
        for (int b = a; b < NDIM; ++b)
        {
          ILOOP1(i)
          {
            bsq(i) += ((a == b) ? 1.0 : 2.0) * bb_u(a, i) * bb_u(b, i) *
                      adm.g_dd(a, b, k, j, i) / SQR(W(i));
          }
        }
      }
#endif
      ...
      ILOOP1(i)
      {
        ...
        mat.rho(k, j, i) =
          (wb_fac - pb_sum - SQR(alpha_reg(i)) * SQR(b0_u(i)));

        mat.S_d(0, k, j, i) =
          wb_fac * v_d(0, i) - b0_u(i) * bi_d(0, i) * alpha_reg(i);
        mat.S_d(1, k, j, i) =
          wb_fac * v_d(1, i) - b0_u(i) * bi_d(1, i) * alpha_reg(i);
        mat.S_d(2, k, j, i) =
          wb_fac * v_d(2, i) - b0_u(i) * bi_d(2, i) * alpha_reg(i);

        mat.S_dd(0, 0, k, j, i) =
          (wb_fac * v_d(0, i) * v_d(0, i) + pb_sum * adm.g_dd(0, 0, k, j, i) -
           bi_d(0, i) * bi_d(0, i));
        mat.S_dd(0, 1, k, j, i) =
          (wb_fac * v_d(0, i) * v_d(1, i) + pb_sum * adm.g_dd(0, 1, k, j, i) -
           bi_d(0, i) * bi_d(1, i));
        mat.S_dd(0, 2, k, j, i) =
          (wb_fac * v_d(0, i) * v_d(2, i) + pb_sum * adm.g_dd(0, 2, k, j, i) -
           bi_d(0, i) * bi_d(2, i));
        mat.S_dd(1, 1, k, j, i) =
          (wb_fac * v_d(1, i) * v_d(1, i) + pb_sum * adm.g_dd(1, 1, k, j, i) -
           bi_d(1, i) * bi_d(1, i));
        mat.S_dd(1, 2, k, j, i) =
          (wb_fac * v_d(1, i) * v_d(2, i) + pb_sum * adm.g_dd(1, 2, k, j, i) -
           bi_d(1, i) * bi_d(2, i));
        mat.S_dd(2, 2, k, j, i) =
          (wb_fac * v_d(2, i) * v_d(2, i) + pb_sum * adm.g_dd(2, 2, k, j, i) -
           bi_d(2, i) * bi_d(2, i));
      }
#else
      ...
      for (int a = 0; a < NDIM; ++a)
      {
        ...
        for (int b = a; b < NDIM; ++b)
        {
          ...
          {
            mat.S_dd(a, b, k, j, i) =
              (w_hrho(i) * SQR(W(i)) * v_d(a, i) * v_d(b, i) +
