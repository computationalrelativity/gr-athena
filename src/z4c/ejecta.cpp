...
//  T_.InitWithShallowSlice(pmb->phydro->temperature, ITM, 1);
  ...
  for (int n=0; n<Z4c::N_ADM; ++n) {
    adm_[n].InitWithShallowSlice(pmb->pz4c->storage.adm, Z4c::I_ADM_gxx+n, 1);
  }

  for (int n=0; n<Z4c::N_Z4c; ++n) {
    z4c_[n].InitWithShallowSlice(pmb->pz4c->storage.u, Z4c::I_Z4c_chi+n, 1);
  }
...
//  For interp of VC/CX metric vars
  ...
  for (int i=0; i<ntheta; i++) {
    ...
    for (int j=0; j<nphi; j++) {
      ...
      pinterp3_cx = new LagrangeInterpND<2*NGHOST-1, 3>(origin_cx, delta_cx, size_cx, coord);
...
*/
      ...
      other[I_entropy](i,j) = pmb->peos->GetEOS().GetEntropyPerBaryon(npt,temppt,Ypt); 
