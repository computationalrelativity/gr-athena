...
//========================================================================================
//! \file awa_z4c.cpp
//  \brief implementation of functions in the Z4c class for initializing Apples
//  with Apples tests See  https://arxiv.org/abs/gr-qc/0305023
...
#define SINWAVE(a, dx, dy, x, y) \
  ((a) * std::sin(2. * M_PI * ((x) / (dx) - (y) / (dy))))
#define DSINWAVE(a, dx, dy, x, y) \
  (-(a) * 2. * M_PI * std::cos(2. * M_PI * ((x) / (dx) - (y) / (dy))))

// Gaussian profile for simple 3D gauge wave test
#define GAUSSIAN(a, d, x, y, z) \
  ((a) * std::exp(-(SQR(x) + SQR(y) + SQR(z)) / SQR(d)))

// Gaussian profile for 1d linear test
#define GAUSSIAN1(a, w, x) ((a) * std::exp(-SQR(x) / (2 * SQR(w))))
#define DGAUSSIAN1(a, w, x) \
  ((a * x / SQR(W)) * std::exp(-SQR(x) / (2 * SQR(w))))
...
void Z4c::ADMLinearWave1(AthenaArray<Real>& u_adm)
{
  ...
  // y = 0 and d_y should therefore be irrelevant (set to non-zero to prevent
  // nan)
  Real const d_y = 1;
...
void Z4c::ADMLinearWave2(AthenaArray<Real>& u_adm)
{
  ...
  GLOOP2(k, j)
  {
    ...
    for (int a = 0; a < NDIM - 1; ++a)
    {
      ...
      {
        adm.g_dd(a, a, k, j, i) +=
          0.5 * SINWAVE(amp, d_x, d_y, mbi.x1(i), mbi.x2(j));
      }
    }
    ...
    GLOOP1(i)
    {
      adm.g_dd(0, 1, k, j, i) =
        0.5 * SINWAVE(amp, d_x, d_y, mbi.x1(i), mbi.x2(j));
      adm.g_dd(2, 2, k, j, i) -= SINWAVE(amp, d_x, d_y, mbi.x1(i), mbi.x2(j));
    }
    ...
    for (int a = 0; a < NDIM - 1; ++a)
    {
      for (int b = a; b < NDIM - 1; ++b)
      {
        ...
        {
          adm.K_dd(a, b, k, j, i) =
            0.25 * SQRT2 * DSINWAVE(amp, d_x, d_y, mbi.x1(i), mbi.x2(j));
        }
      }
    }
    // K_zz
    ...
    {
      adm.K_dd(2, 2, k, j, i) =
        -0.5 * SQRT2 * DSINWAVE(amp, d_x, d_y, mbi.x1(i), mbi.x2(j));
    }
  }
}
...
void Z4c::ADMGaugeWave2(AthenaArray<Real>& u_adm)
{
  ...
  GLOOP2(k, j)
  {
    ...
    for (int a = 0; a < NDIM - 1; ++a)
    {
      ...
      {
        adm.g_dd(a, a, k, j, i) -=
          0.5 * SINWAVE(amp, d_y, d_y, mbi.x1(i), mbi.x2(j));
      }
    }
    ...
    GLOOP1(i)
    {
      ...
      adm.K_dd(2, 2, k, j, i) =
        -0.5 * SQRT2 * DSINWAVE(amp, d_y, d_y, mbi.x1(i), mbi.x2(j));
    }
  }
}
...
void Z4c::GaugeGaugeWave2(AthenaArray<Real>& u)
{
  ...
  GLOOP2(k, j)
  {
    GLOOP1(i)
    {
      // lapse
      z4c.alpha(k, j, i) =
        std::sqrt(1.0 - SINWAVE(amp, d_x, d_y, mbi.x1(i), mbi.x2(j)));
    }
  }
...
void Z4c::ADMPolarisedGowdy(AthenaArray<Real>& u_adm)
{
  ...
  GLOOP3(k, j, i)
  {
    ...
    // K_xx
    adm.K_dd(0, 0, k, j, i) =
      sign_K / 4. * pow_t_m_1_4 * std::exp(L / 4.) * (1. / t - dt_L);
