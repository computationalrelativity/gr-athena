...
class Z4c_AMR
{
  ...
  Real FDTruncErrorChiL2(MeshBlock* const pmb,
                         const int deriv_order,
                         const int p);

  // returning the max err of:
  // h^6 * ( (d^n fld/dx^n)^p + (d^n fld/dy^n)^p + (d^n fld/dz^n)^p )
  Real FDTruncErrorChiLinf(MeshBlock* const pmb,
                           const int deriv_order,
                           const int p);

  // returning the max err of:
  // h^6 * max{|(d^n fld/dx^n)^p|, |(d^n fld/dy^n)^p|, |(d^n fld/dz^n)^p|}
  Real FDTruncErrorChiLinfComponentWise(MeshBlock* const pmy_block,
                                        const int deriv_order,
                                        const int p);
  ...
  Real mb_radius;  // the length of the line from the origin to the meshblock's
                   // center
