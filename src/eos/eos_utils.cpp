...
bool EquationOfState::CanExcisePoint(const bool is_slice,
                                     AT_N_sca& alpha,
                                     AA& x1,
                                     AA& x2,
                                     AA& x3,
                                     ...
                                     const int k)
{
  ...
  if (ph->opt_excision.horizon_based || ph->opt_excision.hybrid_hydro)
  {
    ...
    for (auto pah_f : pm->pah_finder)
    {
      ...
      {
        const Real alpha_min__ = (ph->opt_excision.hybrid_fac_min_alpha *
                                  pm->global_extrema.min_adm_alpha);

        const bool can_excise =
          ((r_2 < SQR(horizon_radius)) && (alpha__ < alpha_min__));
...
bool EquationOfState::CanExcisePoint(Real& excision_factor,
                                     const bool is_slice,
                                     AT_N_sca& alpha,
                                     AA& x1,
                                     AA& x2,
      ...
      if (!is_admissible && (horizon_radius > max_horizon_radius))
      {
        if (ph->opt_excision.use_taper)
        {
          ...
          */
          ...
          const Real t_arg =
            (dt_response > 0)
              ? std::max(std::min(1.0, t_elapsed / dt_response), 0.0)
              : 1.0;
          ...
          // argument on [0,1]
          const Real f_arg =
            std::max(std::min(1.0, dist_ahf / horizon_radius), 0.0);
...
void EquationOfState::SanitizeLoopLimits(int& il,
                                         int& iu,
                                         int& jl,
                                         int& ju,
                                         int& kl,
...
void EquationOfState::GeometryToSlicedCC(geom_sliced_cc& gsc,
                                         const int k,
                                         const int j,
                                         const int il,
                                         const int iu,
...
#pragma omp simd
  ...
  else
  {
    ...
    sl_adm_sqrt_detgamma.InitWithShallowSlice(
      pz4c->storage.aux_extended, Z4c::I_AUX_EXTENDED_ms_sqrt_detgamma);
#endif  // FLUID_ENABLED
  }
...
#pragma omp simd
    ...
    {
      AT_N_sym sl_adm_gamma_uu;
      sl_adm_gamma_uu.InitWithShallowSlice(pz4c->storage.aux_extended,
                                           Z4c::I_AUX_EXTENDED_guxx);
      pco_gr->GetGeometricFieldCC(gamma_uu_, sl_adm_gamma_uu, k, j);
    }
#else
#pragma omp simd
    for (int i = il; i <= iu; ++i)
    {
      const Real oo_det_gamma_i = 1. / SQR(sqrt_det_gamma_(i));
      LinearAlgebra::Inv3Metric(oo_det_gamma_i,
                                gamma_dd_(0, 0, i),
                                gamma_dd_(0, 1, i),
                                gamma_dd_(0, 2, i),
                                gamma_dd_(1, 1, i),
...
#pragma omp simd
    for (int i = il; i <= iu; ++i)
    {
      const Real det_gamma_i    = LinearAlgebra::Det3Metric(gamma_dd_, i);
      sqrt_det_gamma_(i)        = std::sqrt(det_gamma_i);
      const Real oo_det_gamma_i = 1. / det_gamma_i;
      LinearAlgebra::Inv3Metric(oo_det_gamma_i,
                                gamma_dd_(0, 0, i),
                                gamma_dd_(0, 1, i),
                                gamma_dd_(0, 2, i),
                                     ...
                                     int i)
{
  ...
  if (w < 1.0)
  {
    ...
    const Real r   = std::sqrt(gxx * SQR(x) + gyy * SQR(y) + gzz * SQR(z) +
                             2.0 * (gxy * x * y + gxz * x * z + gyz * y * z));
    const Real v_r = (prim(IVX, k, j, i) * x + prim(IVY, k, j, i) * y +
                      prim(IVZ, k, j, i) * z) /
                     r;

    const Real activation_fac =
      (hyd_der_ms(IX_HU_d_0, k, j, i) < -1.0 && v_r > 0) ? (1.0 - w) : 0.0;
  ...
  else
  {
    ...
    cons_scalar(SCXN, k, j, i) =
      prim_scalar(SCXN, k, j, i) * cons(IDN, k, j, i);
    cons_scalar(SCXP, k, j, i) =
      prim_scalar(SCXP, k, j, i) * cons(IDN, k, j, i);
    cons_scalar(SCXA, k, j, i) =
      prim_scalar(SCXA, k, j, i) * cons(IDN, k, j, i);
    cons_scalar(SCXH, k, j, i) =
      prim_scalar(SCXH, k, j, i) * cons(IDN, k, j, i);
  }
...
#pragma omp simd
  for (int i = il; i <= iu; ++i)
  {
    ...
    hyd_der_ms(IX_U_d_0, k, j, i) =
      ...
      (bx * vWy + by * vWx) * gxy + (bx * vWz + bz * vWx) * gxz +
      (by * vWz + bz * vWy) * gyz;
    ...
    const Real oo_sqrt_det_gamma = OO(gsc.sqrt_det_gamma_(i));

    fld_der_ms(IX_b0, k, j, i) =
      oo_sqrt_det_gamma * (W / alp) *
      (gxx * pf->bcc(IB1, k, j, i) * v_u_x +
       gyy * pf->bcc(IB2, k, j, i) * v_u_y +
       gzz * pf->bcc(IB3, k, j, i) * v_u_z +
       gxy * (pf->bcc(IB1, k, j, i) * v_u_y + pf->bcc(IB2, k, j, i) * v_u_x) +
       gxz * (pf->bcc(IB1, k, j, i) * v_u_z + pf->bcc(IB3, k, j, i) * v_u_x) +
       gyz * (pf->bcc(IB2, k, j, i) * v_u_z + pf->bcc(IB3, k, j, i) * v_u_y));
    ...
    fld_der_ms(IX_MAG, k, j, i) =
      fld_der_ms(IX_B2, k, j, i) / cons(IDN, k, j, i);
    fld_der_ms(IX_BET, k, j, i) =
      fld_der_ms(IX_b2, k, j, i) / (2.0 * prim(IPR, k, j, i));
    fld_der_ms(IX_MRI, k, j, i) =
      fld_der_ms(IX_b_U_3, k, j, i) /
      (std::sqrt(rho) * hyd_der_ms(IX_OM, k, j, i));
    fld_der_ms(IX_ALF, k, j, i) =
      std::sqrt(fld_der_ms(IX_B2, k, j, i) / (4.0 * M_PI * rho));
...
bool EquationOfState::NeighborsEncloseValue(const AA& src,
                                            const int n,
                                            const int k,
                                            const int j,
                                            const int i,
...
void EquationOfState::NearestNeighborSmooth(AA& tar,
                                            const AA& src,
                                            const int kl,
                                            const int ku,
                                            const int jl,
                                            ...
                                            const Real sigma_frac)
{
  ...
  const int max_neighbors = (2 * num_neighbors + 1) * (2 * num_neighbors + 1) *
                            (2 * num_neighbors + 1);
  ...
  for (int kk = -num_neighbors; kk <= num_neighbors; ++kk)
    for (int jj = -num_neighbors; jj <= num_neighbors; ++jj)
      for (int ii = -num_neighbors; ii <= num_neighbors; ++ii)
      {
        if (!keep_base_point && ii == 0 && jj == 0 && kk == 0)
          continue;
        ...
        if ((i_ix < 0) || (i_ix >= pmb->ncells1))
          continue;
        if ((j_ix < 0) || (j_ix >= pmb->ncells2))
          continue;
        if ((k_ix < 0) || (k_ix >= pmb->ncells3))
          continue;
        if (!mask(k_ix, j_ix, i_ix))
          continue;
  ...
  if (use_hybrid_mean_median)
  {
    ...
    for (int m = 0; m < count - 1; ++m)
      for (int n2 = m + 1; n2 < count; ++n2)
        if (neighbor_vals[m] > neighbor_vals[n2])
          std::swap(neighbor_vals[m], neighbor_vals[n2]);
  ...
  const Real alpha)
{
  ...
  if (count == 0)
    return src(n, k, j, i);  // fallback if no valid neighbors
