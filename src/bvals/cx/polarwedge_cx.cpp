...
//----------------------------------------------------------------------------------------
//! \fn void CellCenteredXBoundaryVariable::PolarWedgeInnerX2(
//          Real time, Real dt, int il, int iu, int jl, int kl, int ku, int ngh)
//  \brief polar wedge boundary conditions, inner x2 boundary

void CellCenteredXBoundaryVariable::PolarWedgeInnerX2(
    Real time, Real dt, int il, int iu, int jl, int kl, int ku, int ngh) {
  for (int n=0; n<=nu_; ++n) {
...
#pragma omp simd
        for (int i=il; i<=iu; ++i) {
          (*var_cx)(n,k,jl-j,i) = sign * (*var_cx)(n,k,jl+j-1,i);
        }
      }
    }
  }
...
#pragma omp simd
        for (int i=il; i<=iu; ++i) {
          (*var_cx)(n,k,ju+j,i) = sign * (*var_cx)(n,k,ju-j+1,i);
        }
      }
    }
  }
