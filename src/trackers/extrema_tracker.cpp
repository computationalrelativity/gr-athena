...
ExtremaTracker::ExtremaTracker(Mesh* pmesh, ParameterInput* pin, int res_flag)
    : pmesh(pmesh), ndim(pmesh->ndim)
{
  N_tracker = pin->GetOrAddInteger("trackers_extrema", "N_tracker", 0);

  // New-style specification
  use_new_style =
    pin->GetOrAddBoolean("trackers_extrema", "use_new_style", false);
  ...
  {
    ns_control_fields =
      pin->GetOrAddStringArray("trackers_extrema", "control_field", "", 0);
  ...
  if (N_tracker > 0)
  {
    ...
    {
      output_filename = pin->GetOrAddString(
        "trackers_extrema", "filename", "trackers_extrema");
      ...
      {
        control_field_name =
          pin->GetOrAddString("trackers_extrema", "control_field", "none");
      }
...
{
  update_max_step_factor =
    pin->GetOrAddReal("trackers_extrema", "update_max_step_factor", 1.);

  update_strategy =
    pin->GetOrAddInteger("trackers_extrema", "update_strategy", 0);

  if (use_new_style)
  {
    bool param_well_formed = true;

    AthenaArray<int> ref_level_ =
      pin->GetOrAddIntegerArray("trackers_extrema", "ref_level", 1);
    AthenaArray<int> ref_type_ =
    ...
    if constexpr (NDIM >= 3)
    {
      c_x3_ = pin->GetOrAddRealArray("trackers_extrema", "ini_x3", 0);
      param_well_formed = param_well_formed && (c_x3_.GetSize() == N_tracker);
    }
    if constexpr (NDIM >= 2)
    {
      c_x2_ = pin->GetOrAddRealArray("trackers_extrema", "ini_x2", 0);
      param_well_formed = param_well_formed && (c_x2_.GetSize() == N_tracker);
    }
    if constexpr (NDIM >= 1)
    {
      c_x1_ = pin->GetOrAddRealArray("trackers_extrema", "ini_x1", 0);
      param_well_formed = param_well_formed && (c_x1_.GetSize() == N_tracker);
    }
    ...
    AA_B ef = pin->GetOrAddBooleanArray(
      "trackers_extrema", "evaluate_fields", false, N_tracker);
  ...
  else
  {
    for (int n = 1; n <= N_tracker; ++n)
    {
      ...
      ref_level(n - 1) =
        pin->GetOrAddInteger("trackers_extrema", "ref_level_" + n_str, 1);

      ref_type(n - 1) =
        pin->GetOrAddInteger("trackers_extrema", "ref_type_" + n_str, 0);

      // 0: to the point; values larger take radii
      if (ref_type(n - 1) >= 1)
      {
        ref_zone_radius(n - 1) =
          pin->GetOrAddReal("trackers_extrema", "ref_zone_radius_" + n_str, 1);
      }

      // minima / maxima? by default min.
      minima(n - 1) =
        pin->GetOrAddBoolean("trackers_extrema", "minima_" + n_str, true);

      if constexpr (NDIM >= 3)
      {
        c_x3(n - 1) =
          pin->GetOrAddReal("trackers_extrema", "ini_" + n_str + "_x3", 0);
      }
      if constexpr (NDIM >= 2)
      {
        c_x2(n - 1) =
          pin->GetOrAddReal("trackers_extrema", "ini_" + n_str + "_x2", 0);
      }
      if constexpr (NDIM >= 1)
      {
        c_x1(n - 1) =
          pin->GetOrAddReal("trackers_extrema", "ini_" + n_str + "_x1", 0);
      }
    }
  }
}
...
void ExtremaTracker::ReduceTracker()
{
...
#ifdef MPI_PARALLEL
  {
    ...
    std::vector<Real> dx_buf(
      buf_sz);  // N_tracker is typically very small (1-4)
...
void ExtremaTracker::EvolveTracker()
{
...
#ifdef MPI_PARALLEL
  {
    ...
    MPI_Bcast(
      bcast_buf.data(), buf_sz, MPI_ATHENA_REAL, rank_root, MPI_COMM_WORLD);
...
ExtremaTrackerLocal::ExtremaTrackerLocal(MeshBlock* pmb, ParameterInput* pin)
    : pmy_block(pmb),
      ptracker_extrema(pmb->pmy_mesh->ptracker_extrema),
...
void ExtremaTrackerLocal::TreatCentreIfLocalMember()
{
  ...
  for (int n = 1; n <= ptracker_extrema->N_tracker; ++n)
  {
    ...
    const bool is_contained =
      pmy_block->PointContained(ptracker_extrema->c_x1(n - 1),
...
void ExtremaTrackerLocal::UpdateLocStepByControlFieldTimeStep(const int n)
{
  ...
  else if constexpr (NDIM == 1)
  {
    ...
    offset_ix_x1 = IxExtremaOffset(n,
...
void ExtremaTrackerLocal::UpdateLocStepByControlFieldQuadInterp(const int n)
{
  ...
  if constexpr (NDIM == 3)
  {
    ...
    const Real ds_interp[3] = { ds[0] / interp_ds_fac,
                                ds[1] / interp_ds_fac,
                                ds[2] / interp_ds_fac };
    ...
    do
    {
      ...
      ++iter;
    } while ((iter < iter_max) && ((std::abs(cdx_st[0]) > tol_ds) ||
  ...
  else if constexpr (NDIM == 2)
  {
    ...
    const Real ds_interp[2] = { ds[0] / interp_ds_fac, ds[1] / interp_ds_fac };
...
int ExtremaTrackerLocal::LocateCentrePhysicalIndex(const int n, const int axis)
{
  // n starts at 1
  ...
  int idx =
    std::lower_bound(&((*x)(idx_l)), &((*x)(idx_u)), src_val) - &((*x)(idx_l));
  idx += idx_l;
...
bool ExtremaTrackerLocal::IsOrdPhysical(const int axis, const Real x)
{
  ...
  {
    case 2:
      return ((mbi->x3(mbi->kl - 1) <= x) && (x <= mbi->x3(mbi->ku + 1)));
    case 1:
      return ((mbi->x2(mbi->jl - 1) <= x) && (x <= mbi->x2(mbi->ju + 1)));
    case 0:
      return ((mbi->x1(mbi->il - 1) <= x) && (x <= mbi->x1(mbi->iu + 1)));
    default:
      std::cout << "ExtremaTrackerLocal requires 0<=axis<3" << std::endl;
  }
