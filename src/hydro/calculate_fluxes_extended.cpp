    ...
    switch (ivx)
    {
      case 1:
      {
        for (int i = il; i <= iu; ++i)
        {
          ...
          mal(i) =
            std::max(std::abs(lambda(NHYDRO - 1, k, j, i + di)), mal(i));
        }
      ...
      case 2:
      {
        for (int i = il; i <= iu; ++i)
        {
          ...
          mal(i) =
            std::max(std::abs(lambda(NHYDRO - 1, k, j + dj, i)), mal(i));
        }
        break;
      }
      case 3:
      {
        for (int i = il; i <= iu; ++i)
        {
          ...
          mal(i) =
            std::max(std::abs(lambda(NHYDRO - 1, k + dk, j, i)), mal(i));
        }
        break;
      }
    }
...
#pragma omp simd
  ...
  for (int i = il; i <= iu; ++i)
  {
    W_(i) = std::sqrt(1. + InnerProductVecMetric(w_util_u, gamma_dd, k, j, i));
  }
...
#pragma omp simd
  ...
  {
    f(IDN, k, j, i) =
      u(IDN, k, j, i) * alpha(k, j, i) *
      (w_v_u_(ivx - 1, i) - beta_u(ivx - 1, k, j, i) / alpha(k, j, i));
    f(IEN, k, j, i) =
      u(IEN, k, j, i) * alpha(k, j, i) *
        (w_v_u_(ivx - 1, i) - beta_u(ivx - 1, k, j, i) / alpha(k, j, i)) +
...
#pragma omp simd
  ...
  for (int i = il; i <= iu; ++i)
  {
    const Real norm2_utilde =
      InnerProductSlicedVec3Metric(w_util_u_, gamma_dd, k, j, i);
...
#pragma omp simd
  for (int i = il; i <= iu; ++i)
  {
    ...
    const Real T = pmb->peos->GetEOS().GetTemperatureFromP(n, w_p(k, j, i), Y);

    w_hrho_(i) = w_rho(k, j, i) * pmb->peos->GetEOS().GetEnthalpy(n, T, Y);

    Real cs  = pmb->peos->GetEOS().GetSoundSpeed(n, T, Y);
    Real cs2 = cs * cs;
    if (pmb->peos->restrict_cs2)
    {
      cs2 = std::min(cs2, pmb->peos->max_cs2);
    }

    Eigenvalues::HydroEigenvalues(cs2,
                                  w_v_u_(ivx - 1, i),
                                  w_norm2_v_(i),
                                  alpha(k, j, i),
                                  beta_u(ivx - 1, k, j, i),
...
#pragma omp simd
  for (int i = il; i <= iu; ++i)
  {
    ...
    lambda(1, k, j, i) =
      (alpha(k, j, i) * w_v_u_(ivx - 1, i) - beta_u(ivx - 1, k, j, i));
