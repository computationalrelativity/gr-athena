...
//    Returning alpha(i), beta_u(a,i) gamma_dd(a,b,i)
      ...
      AthenaTensor<Real, TensorSymm::NONE, NDIM, 0> lambda_p, lambda_m;
  ...
  vcbeta_x.InitWithShallowSlice(pmy_block->pz4c->storage.u,Z4c::I_Z4c_betax,1);
      ...
      lambda_p.NewAthenaTensor(nn1);
      lambda_m.NewAthenaTensor(nn1);
...
*/
      ...
      for (int i = il; i <= iu; ++i){
        ...
        // Calculate extremal wavespeed
         #pragma omp simd
      for (int i = il; i <= iu; ++i){
        Real lambda_ms = std::min(lambda_m_l(i), lambda_m_r(i));
        Real lambda_ps = std::max(lambda_p_l(i), lambda_p_r(i));
        lambda_p(i) = std::max(0.0, lambda_ps);
        lambda_m(i) = std::min(0.0, lambda_ms);
        }
      ...
      for (int i = il; i <= iu; ++i){
          flux(n,k,j,i) =
              ((lambda_p(i)*flux_l(n, i) - lambda_m(i)*flux_r(n, i))
               + lambda_p(i)*lambda_m(i)*(cons_r(n, i) - cons_l(n, i)))/(lambda_p(i) - lambda_m(i));
          }
        }
