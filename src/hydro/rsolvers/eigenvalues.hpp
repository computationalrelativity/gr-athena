...
//
// Inputs:
//   peos:    pointer to EquationOfState (for EOS access and speed-clamping
//   params) n:       baryon number density T:       temperature vi: Eulerian
//   3-velocity component v^i in the normal direction v2:      norm-squared of
//   Eulerian 3-velocity  gamma_{ab} v^a v^b alpha:   lapse betai:   shift
//   component beta^i in the normal direction gammaii: inverse 3-metric
//   component gamma^{ii} in the normal direction prim_scalar: passive scalar
//   array (particle fractions Y)
// Outputs:
//   plambda_plus:  most positive eigenvalue
//   plambda_minus: most negative eigenvalue

inline void HydroEigenvalues(EquationOfState* peos,
                             Real n,
                             Real T,
                             Real vi,
                             Real v2,
                             ...
                             Real prim_scalar[NSCALARS])
{
  ...
  Real root_1 =
    alpha * (vi * (1.0 - cs_2) + cs * sqrt_term) / (1.0 - v2 * cs_2) - betai;
  Real root_2 =
    alpha * (vi * (1.0 - cs_2) - cs * sqrt_term) / (1.0 - v2 * cs_2) - betai;
...
inline void MHDEigenvalues(EquationOfState* peos,
                           Real n,
                           Real T,
                           Real bsq,
                           Real vi,
