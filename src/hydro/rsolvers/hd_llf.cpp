...
#pragma omp simd
  for (int i = il; i <= iu; ++i)
  {
    ...
    cons_l_(IEN, i) = sqrt_detgamma_(i) * (w_hrho_l_(i) * SQR(W_l_(i)) -
                                           w_rho_l_(i) * W_l_(i) - w_p_l_(i));
  }
...
#pragma omp simd
    for (int i = il; i <= iu; ++i)
    {
      ...
      cons_l_(IVX + a, i) =
        sqrt_detgamma_(i) * (w_hrho_l_(i) * W_l_(i) * w_util_d_l_(a, i));
    }
  }
...
#pragma omp simd
  for (int i = il; i <= iu; ++i)
  {
    ...
    // tau = (rho * h = ) wgas * gamma_lorentz**2 - rho * gamma_lorentz - p
    cons_r_(IEN, i) = sqrt_detgamma_(i) * (w_hrho_r_(i) * SQR(W_r_(i)) -
                                           w_rho_r_(i) * W_r_(i) - w_p_r_(i));
  }
...
#pragma omp simd
    for (int i = il; i <= iu; ++i)
    {
      // S_i = wgas * gamma_lorentz**2 * v_i = wgas * gamma_lorentz * u_i
      cons_r_(IVX + a, i) =
        sqrt_detgamma_(i) * (w_hrho_r_(i) * W_r_(i) * w_util_d_r_(a, i));
    }
  }
