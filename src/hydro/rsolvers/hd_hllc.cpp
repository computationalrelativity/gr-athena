...
#pragma omp simd
  ...
  for (int i = il; i <= iu; ++i)
  {
    ...
    const Real a_norm = a_q / coeff_scale;
    const Real b_norm = b_q / coeff_scale;
    const Real c_norm = c_q / coeff_scale;
    const Real coeff_tol = 1e-12;

    if (std::abs(a_norm) > coeff_tol) {
      // Quadratic branch
      Real disc = b_norm * b_norm - 4.0 * a_norm * c_norm;
      const Real disc_scale = b_norm * b_norm + 4.0 * std::abs(a_norm * c_norm);
      ...
      if (b_norm >= 0.0)
        lambda_c(i) = (-b_norm - std::sqrt(disc)) / (2.0 * a_norm);
      else
        lambda_c(i) = -2.0 * c_norm / (b_norm - std::sqrt(disc));
    } else if (std::abs(b_norm) > coeff_tol) {
      // Linear branch: a_norm ~ 0 but b_norm is resolved
      lambda_c(i) = -c_norm / b_norm;
    } else {
      // Both a_norm and b_norm are unresolved -- fall back to LLF
      ++hllc_nfallback_;
      for (int n = 0; n < NHYDRO; ++n)
        flux(n, k, j, i) = 0.5 *
          ((flux_l_(n, i) + flux_r_(n, i)) -
