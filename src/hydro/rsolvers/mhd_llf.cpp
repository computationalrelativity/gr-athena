...
//   Harm cf. LLFNonTransforming() in llf_rel.cpp
...
#pragma omp simd
  ...
  LinearAlgebra::SlicedVecMet3Contraction(
    w_util_d_l_, w_util_u_l_, gamma_dd_, il, iu);

  LinearAlgebra::SlicedVecMet3Contraction(
    w_util_d_r_, w_util_u_r_, gamma_dd_, il, iu);
...
#pragma omp simd
  ...
  else
  {
    for (int i = il; i <= iu; ++i)
    {
      const Real norm2_utilde_l =
        InnerProductSlicedVec3Metric(w_util_u_l_, gamma_dd_, i);

      const Real norm2_utilde_r =
        InnerProductSlicedVec3Metric(w_util_u_r_, gamma_dd_, i);
...
#pragma omp simd
  ...
  // lower idx
  LinearAlgebra::SlicedVecMet3Contraction(beta_d_, beta_u_, gamma_dd_, il, iu);

  LinearAlgebra::SlicedVecMet3Contraction(
    w_v_d_l_, w_v_u_l_, gamma_dd_, il, iu);
...
#pragma omp simd
  for (int i = il; i <= iu; ++i)
  {
    ...
    else
    {
      ...
      Real csl = peos->GetEOS().GetSoundSpeed(nl__, Tl__, Yl__);
      Real csr = peos->GetEOS().GetSoundSpeed(nr__, Tr__, Yr__);
      cs2l     = csl * csl;
      cs2r     = csr * csr;
...
#pragma omp simd
  for (int i = il; i <= iu; ++i)
  {
    ...
    flux_l_(IBZ, i) = (cons_l_(IBZ, i) * alpha_w_vtil_u_l_(ivx - 1, i) -
                       q_scB_u_l_(ivx - 1, i) * sqrt_detgamma_(i) *
    ...
    flux_r_(IBZ, i) = (cons_r_(IBZ, i) * alpha_w_vtil_u_r_(ivx - 1, i) -
                       q_scB_u_r_(ivx - 1, i) * sqrt_detgamma_(i) *
...
#pragma omp simd
    for (int i = il; i <= iu; ++i)
    {
      ...
      for (int I = 0; I < N_BCPT; ++I)
      {
        ...
        {
          flx__[I] =
            0.5 * ((flux_l_(IBY + I, i) + flux_r_(IBY + I, i)) -
      ...
      wct(k, j, i) = GetWeightForCT(
        flux(IDN, k, j, i), prim_l_(IDN, i), prim_r_(IDN, i), dxw_(i), dt);
    }
  }
  else
  {
#pragma omp simd
    for (int i = il; i <= iu; ++i)
    {
      ey(k, j, i) = -0.5 * (flux_l_(IBY, i) + flux_r_(IBY, i) -
                            lambda(i) * (cons_r_(IBY, i) - cons_l_(IBY, i)));
      ez(k, j, i) = 0.5 * (flux_l_(IBZ, i) + flux_r_(IBZ, i) -
