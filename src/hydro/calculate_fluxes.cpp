  ...
  if (pmb->pmy_mesh->f2)
  {
    ...
    for (int k=kl; k<=ku; ++k)
    {
      ...
      if (pr->xorder_fallback)
      {
        ...
        FallbackInadmissiblePrimitiveX2_(wl_, wr_, r_wl_, r_wr_, il, iu);
      }
      else
      {
#if USETM
        ...
        pr->ReconstructPassiveScalarsX2_(ReconstructionVariant::donate,
                                         ps->r, rl_, rr_,
                                         k, jl-1, il, iu);
        FloorPrimitiveX2_(wl_, wr_, rl_, rr_, k, jl-1, il, iu);
#else
        FloorPrimitiveX2_(wl_, wr_, k, jl-1, il, iu);
#endif
      }
      ...
      for (int j=jl; j<=ju; ++j)
      {
        ...
        if (pr->xorder_fallback)
        {
          ...
          FallbackInadmissiblePrimitiveX2_(wlb_, wr_, r_wlb_, r_wr_, il, iu);
        }
        else
        {
#if USETM
  ...
  if (pmb->pmy_mesh->f3)
  {
    ...
    { // this loop ordering is intentional
      ...
      if (pr->xorder_fallback)
      {
        ...
        FallbackInadmissiblePrimitiveX3_(wl_, wr_, r_wl_, r_wr_, il, iu);
      }
      else
      {
#if USETM
        ...
        pr->ReconstructPassiveScalarsX3_(ReconstructionVariant::donate,
                                         ps->r, rl_, rr_,
                                         kl-1, j, il, iu);
        FloorPrimitiveX3_(wl_, wr_, rl_, rr_, kl-1, j, il, iu);
#else
      ...
      for (int k=kl; k<=ku; ++k)
      {
        ...
        if (pr->xorder_fallback)
        {
          ...
          FallbackInadmissiblePrimitiveX3_(wlb_, wr_, r_wlb_, r_wr_, il, iu);
        }
        else
        {
#if USETM
          ...
          pr->ReconstructPassiveScalarsX3_(ReconstructionVariant::donate,
                                           ps->r, rlb_, rr_,
                                           k, j, il, iu);
          FloorPrimitiveX3_(wlb_, wr_, rlb_, rr_, k, j, il, iu);
#else
          FloorPrimitiveX3_(wlb_, wr_, k, j, il, iu);
#endif
        }
                               ...
                               AthenaArray<Real> &bcc, const int order)
{
  ...
  for (int k=kl; k<=ku; ++k) {
    for (int j=jl; j<=ju; ++j) {
      ...
      if (order == 1) {
        pmb->precon->DonorCellX1(k, j, is-1, ie+1, pmb->phydro->w_init, bcc, wl_, wr_);
      } else if (order == 2) {
        pmb->precon->PiecewiseLinearX1(k, j, is-1, ie+1, pmb->phydro->w_init, bcc, wl_, wr_);
      } else if (order ==3 || order==4) {
...
#if USETM
          pmb->peos->ApplyPrimitiveFloors(wl_, pmb->pscalars->r, k, j, i); // TODO PH: Fix
...
#pragma omp simd
  ...
  if (pmb->pmy_mesh->f2) {
    ...
    for (int k=kl; k<=ku; ++k) {
      ...
      if (order == 1) {
        pmb->precon->DonorCellX2(k, js-1, il, iu, pmb->phydro->w_init, bcc, wl_, wr_);
      } else if (order == 2) {
        pmb->precon->PiecewiseLinearX2(k, js-1, il, iu, pmb->phydro->w_init, bcc, wl_, wr_);
      } else if (order ==3 || order == 4){
      ...
      for (int j=js; j<=je+1; ++j) {
        ...
        if (order == 1) {
          pmb->precon->DonorCellX2(k, j, il, iu, pmb->phydro->w_init, bcc, wlb_, wr_);
        } else if (order == 2) {
          pmb->precon->PiecewiseLinearX2(k, j, il, iu, pmb->phydro->w_init, bcc, wlb_, wr_);
        } else if (order == 3 || order ==4 ){
...
#pragma omp simd
  ...
  if (pmb->pmy_mesh->f3) {
    ...
    for (int j=jl; j<=ju; ++j) { // this loop ordering is intentional
      ...
      if (order == 1) {
        pmb->precon->DonorCellX3(ks-1, j, il, iu, pmb->phydro->w_init, bcc, wl_, wr_);
      } else if (order == 2) {
        pmb->precon->PiecewiseLinearX3(ks-1, j, il, iu, pmb->phydro->w_init, bcc, wl_, wr_);
      } else if (order == 3 || order == 4 ) {
      ...
      for (int k=ks; k<=ke+1; ++k) {
        ...
        if (order == 1) {
          pmb->precon->DonorCellX3(k, j, il, iu, pmb->phydro->w_init, bcc, wlb_, wr_);
        } else if (order == 2) {
          pmb->precon->PiecewiseLinearX3(k, j, il, iu, pmb->phydro->w_init, bcc, wlb_, wr_);
        } else if (order == 3 || order == 4){
