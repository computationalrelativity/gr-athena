...
// TODO WC: check mbi.nn? is number of vertices!
  int sz = n[0] * n[1] * n[2];
...
// TODO work out what to do with psi(4)
...
//TODO work out what to do with psi4?
...
//TODO WC: check that rns produces beta_u not beta_d
  }
...
//check that CLOOP3 does the right thing
  GCLOOP3(k,j,i){
    flat_ix = i + n[0]*(j + n[1]*k);
...
*/
    if(rho[flat_ix] > rho_atm){
        ...
        w_init(IVX,k,j,i) = w1(IVX,k,j,i) = w(IVX,k,j,i) = ux[flat_ix]  - v_pert*std::cos(M_PI*r/(2.0*8.0))*x[i]/r;
        w_init(IVY,k,j,i) = w1(IVY,k,j,i) = w(IVY,k,j,i) = uy[flat_ix]- v_pert*std::cos(M_PI*r/(2.0*8.0))*y[j]/r;
        w_init(IVZ,k,j,i) = w1(IVZ,k,j,i) = w(IVZ,k,j,i) = uz[flat_ix]- v_pert*std::cos(M_PI*r/(2.0*8.0))*z[k]/r;
    } else{
