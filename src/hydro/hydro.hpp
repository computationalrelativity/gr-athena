...
class Hydro
{
  ...
  AA derived_ms;
  // storage for derived quantities for internal usage
  // (HydroInternalDerivedIndex);
  AA derived_int;
  ...
  struct ixn_cons
  {
    ...
    static constexpr char const* const names[] = {
      "hydro.cons.D",     "hydro.cons.S_d_1", "hydro.cons.S_d_2",
      "hydro.cons.S_d_3", "hydro.cons.tau",
    };
  };
  ...
  struct ixn_prim
  {
    ...
    static constexpr char const* const names[] = {
      "hydro.prim.rho",      "hydro.prim.util_u_1", "hydro.prim.util_u_2",
      "hydro.prim.util_u_3", "hydro.prim.p",
    };
  };
  ...
  void CheckStateWithFluxDivergenceDMP(const Real wght,
                                       AA& u,
                                       AA& u_old,
                                       AA& s,
                                       AA& s_old,
  ...
  void HybridizeFluxes(AA (&hflux)[3],
                       AA (&sflux)[3],
                       AA (&lo_hflux)[3],
                       AA (&lo_sflux)[3],
                       const AA_B& mask);
