...
// TODO: I need to define this to be able to assign it to a gsl_function
...
// TODO: Check what is this function for
  ...
  CLOOP3(k,j,i) {
    ...
    for (int ig = 0; ig < nspecies*ngroups; ++ig) {
      ...
      calc_closure_pt(iteration, pmb, i, j, k, ig,
		      ...
		      W, u_u, v_d, proj_ud, vec.E(k,j,i,ig), F_d,
		      &rad.chi(k,j,i,ig), P_dd);
      unpack_P_dd(P_dd,
		  &rad.P_dd(0,0,k,j,i,ig), &rad.P_dd(0,1,k,j,i,ig), &rad.P_dd(0,2,k,j,i,ig),
		  &rad.P_dd(1,1,k,j,i,ig), &rad.P_dd(1,2,k,j,i,ig),
		  &rad.P_dd(2,2,k,j,i,ig));
      
      for (int a = 0; a < NDIM; ++a) {
	      for (int b = a; b < a; ++b) {
	        assert(isfinite(rad.P_dd(a,b,k,j,i,ig)));
	      }
      }
      
      // J
      // TODO: Check this E
      assemble_rT(n_d, lab.E(k,j,i,ig), F_d, P_dd, T_dd);
      
      rad.J(k,j,i,ig) = calc_J_from_rT(T_dd, u_u);      

      assert(isfinite(rad.J(k,j,i,ig)));
      ...
      unpack_H_d(H_d,
		    &rad.Ht(k,j,i,ig),
		    &rad.H(1,k,j,i,ig),&rad.H(2,k,j,i,ig),&rad.H(3,k,j,i,ig));

      assert(isfinite(rad.Ht(k,j,i,ig)));
      for (int a = 0; a < NDIM; ++a) {
	      assert(isfinite(rad.H(a,k,j,i,ig)));
      }
      ...
      rad.nnu(k,j,i,ig) = vec.N(k,j,i,ig)/Gamma;
