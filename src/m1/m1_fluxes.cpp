...
void M1::CalcFluxes(AthenaArray<Real> & u, AthenaArray<Real> & u_rhs)
{
  ...
  for (int ig = 0; ig < ngroups*nspecies; ++ig) {
    rhs[PINDEX1D(ig, 0)] = &vec_rhs.N  (   0,0,0, ig);
    rhs[PINDEX1D(ig, 1)] = &vec_rhs.F_d(0, 0,0,0, ig);
    rhs[PINDEX1D(ig, 2)] = &vec_rhs.F_d(1, 0,0,0, ig);
    rhs[PINDEX1D(ig, 3)] = &vec_rhs.F_d(2, 0,0,0, ig);
    rhs[PINDEX1D(ig, 4)] = &vec_rhs.E  (   0,0,0, ig);
  }
  ...
  for (int dir = 0; dir < NDIM; ++dir) {
	...
	for (__k = 0; __k < lsh[2]; ++__k) {
	  ...
	  for (int ig = 0; ig < ngroups*nspecies; ++ig) {
	    ...
	    pack_P_dd(beta_u(1), beta_u(2), beta_u(3),
		      rad.P_dd(0,0,k,j,i,ig), rad.P_dd(0,1,k,j,i,ig), rad.P_dd(1,1,k,j,i,ig),
	    ...
	    Real const Gamma = compute_Gamma(fidu.Wlorentz(k,j,i), v_u,
					     rad.J(k,j,i,ig), vec.E(k,j,i,ig), F_d);

	    // Note that nnu is densitized here
	    Real const nnu = vec.N(k,j,i,ig)/Gamma;

	    cons[GFINDEX1D(__k, ig, 0)] = vec.N(k,j,i,ig);
	    cons[GFINDEX1D(__k, ig, 1)] = vec.F_d(0,k,j,i,ig);
	    cons[GFINDEX1D(__k, ig, 2)] = vec.F_d(1,k,j,i,ig);
	    cons[GFINDEX1D(__k, ig, 3)] = vec.F_d(2,k,j,i,ig);
	    cons[GFINDEX1D(__k, ig, 4)] = vec.E(k,j,i,ig);
	    ...
	    flux[GFINDEX1D(__k, ig, 4)] =
	      calc_E_flux(alpha(), beta_u, vec.E(k,j,i,ig), F_u, dir+1);
	    ...
	    Real const lam[3] = {
	      ...
	      - alpha()*fx - beta_u(dir+1),
	      alpha()*vec.E(k,j,i,ig)*ffx - beta_u(dir+1),
	    };
	...
	for (__k = M1_NGHOST-1; __k < lsh[2]-M1_NGHOST; ++__k) {
	  ...
	  for (int ig = 0; ig < ngroups*nspecies; ++ig) {
	    ...
	    Real avg_abs_1 = rmat.abs_1(k,j,i,ig);
	    Real avg_scat_1 = rmat.scat_1(k,j,i,ig);
	    
	    index[2]++;
	    
	    avg_abs_1 += rmat.abs_1(k,j,i,ig);
	    avg_scat_1 += rmat.scat_1(k,j,i,ig);
