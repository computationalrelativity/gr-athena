                    ...
                    const int i)
{
  ...
  if (w_rho < pm1.opt_solver.eql_rho_min || w_T < pm1.opt_solver.eql_t_min)
  {
    return;
  }
  ...
  for (int ix_g = 0; ix_g < pm1.N_GRPS; ++ix_g)
    for (int ix_s = 0; ix_s < pm1.N_SPCS; ++ix_s)
    {
      ...
      sc_E(k, j, i) = ONE_3RD * sc_J(k, j, i) * (4.0 * W2 - 1.0);

      for (int a = 0; a < N; ++a)
      {
        sp_F_d(a, k, j, i) =
          4.0 * ONE_3RD * W2 * pm1.fidu.sp_v_d(a, k, j, i) * sc_J(k, j, i);
      }
      ...
      const Real Gamma__ = W / sc_J(k, j, i) * (sc_E(k, j, i) - dotFv__);
...
void SetEquilibrium_n_nG(M1& pm1,
                         const Real dt,
                         ...
                         const int i)
{
  ...
  bool opt_flux_lo_fallback_E  = pm1.opt.flux_lo_fallback_E;
  bool opt_flux_lo_fallback_nG = pm1.opt.flux_lo_fallback_nG;
  ...
  if (pm1.opt_solver.equilibrium_evolve)
  {
    ...
    {
      // Sources::SetMatterSourceZero(S, k, j, i);
      // Integrators::Explicit::StepExplicit_E_F_d(pm1, dt, C, P, I, S, k, j,
      // i);
      ...
      Assemble::Frames::ToFiducialExpansionCoefficients(
        pm1, J_0, H_n, H_v, H_F, C.sc_chi, C.sc_E, C.sp_F_d, k, j, i);
      ...
      Closures::EddingtonFactors::ThickLimit(CL_C.sc_xi(k, j, i),
                                             CL_C.sc_chi(k, j, i));

      // (sc_J, st_H_d) -> (sc_E, sp_F_d) reduces to:
      sc_E(k, j, i) = ONE_3RD * J_0 * (4.0 * W2 - 1.0) + 2.0 * W * H_n__;

      Real dotFv(0.0);
      for (int a = 0; a < N; ++a)
      {
        sp_F_d(a, k, j, i) =
          4.0 * ONE_3RD * W2 * pm1.fidu.sp_v_d(a, k, j, i) * J_0;
        sp_F_d(a, k, j, i) +=
          W * (H_n__ * pm1.fidu.sp_v_d(a, k, j, i) + sp_H_d_(a, i));
  ...
  if (pm1.opt_solver.equilibrium_src_E_F_d)
  {
    ...
    {
      Assemble::Frames::sources_sc_E_sp_F_d(pm1,
                                            S.sc_E,
                                            S.sp_F_d,
                                            C.sc_chi,
                                            C.sc_E,
  ...
  Real sc_Gam__ = Assemble::Frames::ToFiducial(
    pm1, sc_J, st_H_u, sc_n, sc_chi, sc_E, sp_F_d, sc_nG, k, j, i);
  ...
  */
  ...
  {
    sc_avg_nrg(k, j, i) =
      (pm1.eql.sc_J(ix_g, ix_s)(k, j, i) / pm1.eql.sc_n(ix_g, ix_s)(k, j, i));
  ...
  if (pm1.opt_solver.equilibrium_src_nG)
  {
    ...
    else
    {
      Assemble::Frames::sources_sc_nG(
        pm1, S.sc_nG, C.sc_n, C.sc_eta_0, C.sc_kap_a_0, k, j, i);
    }
  }
  else
  {
...
void SetEquilibrium_E_F_d_n_nG(M1& pm1,
                               const Real dt,
                               Update::StateMetaVector& C,
                               Update::StateMetaVector& P,
                               Update::StateMetaVector& I,
                               ...
                               const int i)
{
  ...
  Real eq_sc_Gam__ =
    pm1.fidu.sc_W(k, j, i) / sc_J(k, j, i) * (sc_E(k, j, i) - dotFv);
  ...
  // std::printf("%.3e %.3e\n", std::abs(1 - sc_Gam__ / eq_sc_Gam__),
  // sc_Gam__);
...
void MapReferenceEquilibrium(M1& pm1,
                             M1::vars_Eql& eq,
                             Update::StateMetaVector& C,
                             const int k,
                             const int j,
                             const int i)
{
  ...
  // (sc_J, st_H_d) -> (sc_E, sp_F_d) reduces to:
  C.sc_E(k, j, i) = ONE_3RD * C.sc_J(k, j, i) * (4.0 * W2 - 1.0);

  for (int a = 0; a < N; ++a)
  {
    C.sp_F_d(a, k, j, i) =
      4.0 * ONE_3RD * W2 * pm1.fidu.sp_v_d(a, k, j, i) * C.sc_J(k, j, i);
  }
  ...
  const Real Gamma__ = W / C.sc_J(k, j, i) * (C.sc_E(k, j, i) - dotFv__);
  ...
  pm1.radmat.sc_avg_nrg(C.ix_g, C.ix_s)(k, j, i) =
    W / C.sc_nG(k, j, i) * (C.sc_E(k, j, i) - dotFv__);
}
