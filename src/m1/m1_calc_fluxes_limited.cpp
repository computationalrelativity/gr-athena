                            ...
                            AT_C_sca& Flux)
{
  switch (dir)
  {
    case 0:
    {
      ...
      if (!pm1->opt.flux_limiter_multicomponent && pm1->opt.flux_limiter_nn)
      {
        CC_GLOOP3(k, j, i)
        {
      ...
      ReconstructLimitedFluxX1(
        pm1, q, F, xi, kap_a, kap_s, lambda, flux_limiter, Flux);
      break;
    }
    case 1:
    {
  ...
  switch (dir)
  {
    case 0:
    {
      for (int a = 0; a < N; ++a)
      {
        ...
        ReconstructLimitedFluxX1(
          pm1, q_c, F_c, xi, kap_a, kap_s, lambda, flux_limiter, Flux_c);
      }
      break;
    }
    case 1:
                 ...
                 const Real& qp2)
{
  ...
  switch (flx_var)
  {
    ...
    case (M1::opt_flux_variety::HybridizeMinMod):
    {
      ...
      // const Real Phi = std::max(0.0, MinMod2(d_ql * oo_d_qc, d_qr *
      // oo_d_qc)); return 1 - Phi;
      Real Phi = 0.0;
      if ((d_qlc > 0) && (d_qcr > 0))
      {
    ...
    case (M1::opt_flux_variety::HybridizeMinModA):
    {
      ...
      const Real A =
        (((d_qlc < 0) && (d_qcr < 0))) ? 1.0 : std::min(1.0, OO(kap_fac));
    ...
    case (M1::opt_flux_variety::HybridizeMinModC):
    {
      ...
      const Real A = (((d_qlc < 0) && (d_qcr < 0))) ? 1.0 : xi_avg;
                   ...
                   const AT_C_sca& kap_s)
{
  if (pm1->opt.flux_limiter_nn)
  {
    ...
    for (int i = M1_IX_IL - M1_FSIZEI; i <= M1_IX_IU + M1_FSIZEI + 1; ++i)
    {
      ...
      flux_limiter(0, k, j, i - 1) =
        std::max(th_imh, flux_limiter(0, k, j, i - 1));
      flux_limiter(0, k, j, i + 0) =
        std::max(th_imh, flux_limiter(0, k, j, i + 0));
    }
  }
  else
  {
    // SIMD-safe: each iteration writes only to flux_limiter(0,k,j,i).
    M1_LLOOP3_1(k, j, i)
    {
      const Real hyb_fac =
        FluxLimiter(pm1->opt.flux_variety,
                   ...
                   const AT_C_sca& kap_s)
{
  M1_LLOOP3_2(k, j, i)
  {
    ...
    if (pm1->opt.flux_limiter_nn)
    {
      flux_limiter(1, k, j - 1, i) =
        std::max(th_jmh, flux_limiter(1, k, j - 1, i));
    }
    flux_limiter(1, k, j + 0, i) =
      std::max(th_jmh, flux_limiter(1, k, j + 0, i));
  }
}
                   ...
                   const AT_C_sca& kap_s)
{
  M1_LLOOP3_3(k, j, i)
  {
    ...
    if (pm1->opt.flux_limiter_nn)
    {
      flux_limiter(2, k - 1, j, i) =
        std::max(th_kmh, flux_limiter(2, k - 1, j, i));
    }
    flux_limiter(2, k + 0, j, i) =
      std::max(th_kmh, flux_limiter(2, k + 0, j, i));
  }
}
