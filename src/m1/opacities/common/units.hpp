#ifndef M1_OPACITIES_COMMON_UNITS_HPP
#define M1_OPACITIES_COMMON_UNITS_HPP

//! \file units.hpp
//  \brief contains unit definitions, conversion, and constants.
//
...
namespace M1::Opacities::Common::Units
{
...
struct UnitSystem
{
  ...
  inline constexpr Real OpacityConversion(UnitSystem& b) const
  {  // number per length
  ...
  inline constexpr Real NumberDensityConversion(UnitSystem& b) const
  ...
  inline constexpr Real EnergyDensityConversion(UnitSystem& b) const
  ...
  inline constexpr Real NumberRateConversion(UnitSystem& b) const
  {  // number per unit time per unit volume
    return (time * POW3(length)) / (b.time * POW3(b.length));
  }

  inline constexpr Real EnergyRateConversion(UnitSystem& b) const
  {  // energy per unit time per unit volume
    return (b.energy * time * POW3(length)) /
           (energy * b.time * POW3(b.length));
  }
  //! \}
};
...
static UnitSystem GeometricSolar{
  ...
  POW3(CGS.G / (CGS.c * CGS.c)) *
    SQR(CGS.Msun / (CGS.c)),  // pressure, Msun^-2
...
#endif  // M1_OPACITIES_COMMON_UNITS_HPP
