...
M1::M1(MeshBlock* pmb, ParameterInput* pin)
    : pm1(this),
      ...
      fluxes{ { N_GRPS, N_SPCS }, { N_GRPS, N_SPCS }, { N_GRPS, N_SPCS } },
      fluxes_lo{ { N_GRPS, N_SPCS }, { N_GRPS, N_SPCS }, { N_GRPS, N_SPCS } },
      lab{ { N_GRPS, N_SPCS }, { N_GRPS, N_SPCS }, { N_GRPS, N_SPCS } },
      rhs{ { N_GRPS, N_SPCS }, { N_GRPS, N_SPCS }, { N_GRPS, N_SPCS } },
      lab_aux{ // {N_GRPS,N_SPCS},
               { N_GRPS, N_SPCS },
               { N_GRPS, N_SPCS }
      },
             ...
             { N_GRPS, N_SPCS } }
{
  ...
  // CC only (no CX/VC variant). Has flux correction (OverwriteFromFiner).
  {
    ...
    // Parity: per group-species block of 5, {E(scalar), F_{x,y,z}(vector),
    // nG(scalar)}.
    for (int gs = 0; gs < N_GS; ++gs)
    {
      spec.component_groups.push_back({ comm::GeomType::Scalar, 1 });  // E
      spec.component_groups.push_back(
        { comm::GeomType::Vector, 3 });  // F_{x,y,z}
      spec.component_groups.push_back({ comm::GeomType::Scalar, 1 });  // nG
    }
    // Flux correction: fine-restricted fluxes overwrite coarse at AMR
    // interfaces.
  ...
  {
    storage_eql.NewAthenaArray(ixn_Eql::N * N_GS, mbi.nn3, mbi.nn2, mbi.nn1);
    SetVarAliasesEql(storage_eql, eql);
  }

  // storage for misc. quantities ---------------------------------------------
  if ((opt_solver.src_lim >= 0) || (opt_solver.full_lim >= 0))
  {
  ...
  ev_strat.masks.excised.NewAthenaArray(mbi.nn3, mbi.nn2, mbi.nn1);
  ...
  {
    ev_strat.masks.flux_limiter.NewAthenaArray(N, mbi.nn3, mbi.nn2, mbi.nn1);
  ...
  if (opt.flux_lo_fallback)
  {
    ...
    if (opt.flux_lo_fallback_species)
    {
      ...
      ev_strat.masks.compute_point.NewAthenaArray(
        N_SPCS, mbi.nn3, mbi.nn2, mbi.nn1);
    }
    else
    {
  ...
  */
  ...
  std::call_once(gsl_err_handler_flag, []() { gsl_set_error_handler_off(); });

  gsl_brent_solver = gsl_root_fsolver_alloc(gsl_root_fsolver_brent);

  gsl_newton_solver = gsl_root_fdfsolver_alloc(gsl_root_fdfsolver_newton);

  // Pre-allocate multiroot solvers for implicit integrators
  gsl_hybrids_solver =
    gsl_multiroot_fsolver_alloc(gsl_multiroot_fsolver_hybrids, GSL_N_SYS);
  gsl_hybridsj_solver =
    gsl_multiroot_fdfsolver_alloc(gsl_multiroot_fdfsolver_hybridsj, GSL_N_SYS);
...
void M1::SetVarAliasesFluxes(AA (&u_flux)[M1_NDIM], vars_Flux& fluxes)
{
  for (int ix_g = 0; ix_g < N_GRPS; ++ix_g)
    for (int ix_s = 0; ix_s < N_SPCS; ++ix_s)
      ...
      {
        SetVarAlias(
          fluxes.sc_E, u_flux, ix_g, ix_s, ix_f, ixn_Lab::E, ixn_Lab::N);
        SetVarAlias(
          fluxes.sp_F_d, u_flux, ix_g, ix_s, ix_f, ixn_Lab::F_x, ixn_Lab::N);
        SetVarAlias(
          fluxes.sc_nG, u_flux, ix_g, ix_s, ix_f, ixn_Lab::nG, ixn_Lab::N);
      }
}
...
void M1::SetVarAliasesSource(AthenaArray<Real>& sources, vars_Source& src)
{
  for (int ix_g = 0; ix_g < N_GRPS; ++ix_g)
    for (int ix_s = 0; ix_s < N_SPCS; ++ix_s)
    {
      ...
      SetVarAlias(
        src.sp_F_d, sources, ix_g, ix_s, ixn_Src::sp_F_0, ixn_Src::N);
    }
}
...
void M1::SetVarAliasesLabAux(AthenaArray<Real>& u, vars_LabAux& lab_aux)
{
  for (int ix_g = 0; ix_g < N_GRPS; ++ix_g)
    for (int ix_s = 0; ix_s < N_SPCS; ++ix_s)
    {
      ...
      SetVarAlias(
        lab_aux.sc_chi, u, ix_g, ix_s, ixn_Lab_aux::chi, ixn_Lab_aux::N);
      SetVarAlias(
        lab_aux.sc_xi, u, ix_g, ix_s, ixn_Lab_aux::xi, ixn_Lab_aux::N);
    }
}
...
void M1::SetVarAliasesRadMat(AthenaArray<Real>& u, vars_RadMat& radmat)
{
  for (int ix_g = 0; ix_g < N_GRPS; ++ix_g)
    for (int ix_s = 0; ix_s < N_SPCS; ++ix_s)
    {
      SetVarAlias(radmat.sc_eta_0, u, ix_g, ix_s, ixn_RaM::eta_0, ixn_RaM::N);
      SetVarAlias(
        radmat.sc_kap_a_0, u, ix_g, ix_s, ixn_RaM::kap_a_0, ixn_RaM::N);

      SetVarAlias(radmat.sc_eta, u, ix_g, ix_s, ixn_RaM::eta, ixn_RaM::N);
      SetVarAlias(radmat.sc_kap_a, u, ix_g, ix_s, ixn_RaM::kap_a, ixn_RaM::N);
      SetVarAlias(radmat.sc_kap_s, u, ix_g, ix_s, ixn_RaM::kap_s, ixn_RaM::N);
...
void M1::SetVarAliasesDiag(AthenaArray<Real>& u, vars_Diag& rdiag)
{
  for (int ix_g = 0; ix_g < N_GRPS; ++ix_g)
    ...
    {
      SetVarAlias(
        rdiag.sc_radflux_0, u, ix_g, ix_s, ixn_Diag::radflux_0, ixn_Diag::N);
      SetVarAlias(
        rdiag.sc_radflux_1, u, ix_g, ix_s, ixn_Diag::radflux_1, ixn_Diag::N);

      SetVarAlias(rdiag.sc_y, u, ix_g, ix_s, ixn_Diag::y, ixn_Diag::N);
      SetVarAlias(rdiag.sc_z, u, ix_g, ix_s, ixn_Diag::z, ixn_Diag::N);
    }
}
...
void M1::StatePrintPoint(const std::string& tag,
                         const int ix_g,
                         const int ix_s,
                         const int k,
                         const int j,
...
#pragma omp critical
  {
    ...
    lab_aux.sc_chi(ix_g, ix_s)
      .PrintPoint("lab_aux.sc_chi(ix_g,ix_s)", k, j, i);
    lab_aux.sc_xi(ix_g, ix_s).PrintPoint("lab_aux.sc_xi(ix_g,ix_s)", k, j, i);

    // std::cout << "sym2===============: " << "\n";
    // lab_aux.sp_P_dd(ix_g,ix_s).PrintPoint("lab_aux.sp_P_dd(ix_g,ix_s)",
    // k,j,i);
    ...
    const Real kap_as = (radmat.sc_kap_a(ix_g, ix_s)(k, j, i) +
