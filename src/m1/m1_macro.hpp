...
// Indicial magic -------------------------------------------------------------
#define M1_IX_IL pm1->mbi.il

#define M1_IX_IU pm1->mbi.iu

#define M1_IX_JL pm1->mbi.jl

#define M1_IX_JU pm1->mbi.ju

#define M1_IX_KL pm1->mbi.kl

#define M1_IX_KU pm1->mbi.ku

// if positivity preservation utilized, extend here masking loops
#define M1_MSIZEI pm1->opt.flux_lo_fallback

#define M1_MSIZEJ ((pm1->mbi.f2) ? (pm1->opt.flux_lo_fallback) : (0))

#define M1_MSIZEK ((pm1->mbi.f3) ? (pm1->opt.flux_lo_fallback) : (0))

#define M1_FSIZEI (1 + pm1->opt.flux_lo_fallback)

#define M1_FSIZEJ ((pm1->mbi.f2) ? (1 + pm1->opt.flux_lo_fallback) : (0))

#define M1_FSIZEK ((pm1->mbi.f3) ? (1 + pm1->opt.flux_lo_fallback) : (0))

// if positivity preservation utilized, extend here reconstruction loops
#define M1_RSIZEI pm1->opt.flux_lo_fallback

#define M1_RSIZEJ ((pm1->mbi.f2) ? (pm1->opt.flux_lo_fallback) : (0))

#define M1_RSIZEK ((pm1->mbi.f3) ? (pm1->opt.flux_lo_fallback) : (0))

#define M1_GSIZEI (pm1->mbi.ng)

#define M1_GSIZEJ ((pm1->mbi.f2) ? (pm1->mbi.ng) : (0))

#define M1_GSIZEK ((pm1->mbi.f3) ? (pm1->mbi.ng) : (0))
...
#define M1_ILOOP1(i) \
  _Pragma("omp simd") for (int i = M1_IX_IL; i <= M1_IX_IU; ++i)

// 1D loop over i for mask nodes
#define M1_MLOOP1(i)                                     \
  _Pragma("omp simd") for (int i = M1_IX_IL - M1_MSIZEI; \
                           i <= M1_IX_IU + M1_MSIZEI;    \
                           ++i)

// 1D loop over i over flux-salient nodes
#define M1_FLOOP1(i)                                      \
  _Pragma("omp simd") for (int i = M1_IX_IL - M1_FSIZEI;  \
                           i <= M1_IX_IU + M1_FSIZEI + 1; \
                           ++i)

// 1D loop over i on the whole block
#define M1_GLOOP1(i)                                     \
  _Pragma("omp simd") for (int i = M1_IX_IL - M1_GSIZEI; \
                           i <= M1_IX_IU + M1_GSIZEI;    \
                           ++i)
...
#define M1_RLOOP1_1(i)                                    \
  _Pragma("omp simd") for (int i = M1_IX_IL - M1_RSIZEI;  \
                           i <= M1_IX_IU + M1_RSIZEI + 1; \
                           ++i)
...
// ============================================================================
// No-SIMD (NS) variants of the loop macros.
//
// Use these when the loop body contains operations that are incompatible with
// #pragma omp simd, such as scalar reductions without a reduction clause,
// nearest-neighbour reads that conflict with writes in the same i-loop, or
// complex control flow (break, assert, printf, ...).
// ============================================================================

// --- interior (I) ---
#define M1_NS_ILOOP1(i) for (int i = M1_IX_IL; i <= M1_IX_IU; ++i)

#define M1_NS_ILOOP3(k, j, i) \
  M1_ILOOP2(k, j)             \
  M1_NS_ILOOP1(i)

// --- flux-salient (F) ---
#define M1_NS_FLOOP1(i) \
  for (int i = M1_IX_IL - M1_FSIZEI; i <= M1_IX_IU + M1_FSIZEI + 1; ++i)

#define M1_NS_FLOOP3(k, j, i) \
  M1_FLOOP2(k, j)             \
  M1_NS_FLOOP1(i)

// --- limiting mask (L), direction 1: i-1/2 ---
#define M1_NS_LLOOP1_1(i) \
  for (int i = M1_IX_IL - M1_FSIZEI; i <= M1_IX_IU + M1_FSIZEI + 1; ++i)
#define M1_NS_LLOOP3_1(k, j, i) \
  M1_LLOOP2_1(k, j)             \
  M1_NS_LLOOP1_1(i)

// --- limiting mask (L), direction 2: j-1/2 ---
#define M1_NS_LLOOP1_2(i) \
  for (int i = M1_IX_IL - M1_FSIZEI; i <= M1_IX_IU + M1_FSIZEI; ++i)
#define M1_NS_LLOOP3_2(k, j, i) \
  M1_LLOOP2_2(k, j)             \
  M1_NS_LLOOP1_2(i)

// --- limiting mask (L), direction 3: k-1/2 ---
#define M1_NS_LLOOP1_3(i) \
  for (int i = M1_IX_IL - M1_FSIZEI; i <= M1_IX_IU + M1_FSIZEI; ++i)
#define M1_NS_LLOOP3_3(k, j, i) \
  M1_LLOOP2_3(k, j)             \
  M1_NS_LLOOP1_3(i)
