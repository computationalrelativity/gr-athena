...
//
// SB Constants should be taken from Lorene's "unites" so to ensure
// consistency.
...
Real const athenaB =
  (1.0 / athenaL / std::sqrt(eps0 * G_grav / (c_light * c_light)));
...
#else
Real const c_light =
  Lorene::Unites::c_si;  // 2.99792458E+8 ;	 ///< Velocity of light [m/s]
Real const nuc_dens = Lorene::Unites::rhonuc_si;  // Nuclear density as used in
                                                  // Lorene units [kg/m^3]
Real const G_grav =
  Lorene::Unites::g_si;  // gravitational constant [m^3/kg/s^2]
Real const M_sun = Lorene::Unites::msol_si;  // solar mass [kg]
// const Real mev_si = Lorene::Unites::mev_si // 1.602176634E-13 ;   ///< One
// MeV [J]
const Real m_u_si =
  Lorene::Unites::m_u_si;  // 1.6605390666E-27 ;  ///< atomic mass unit [kg]
...
Real const athenaT = athenaL / c_light;
Real const athenaB =
  (1.0 / athenaL / std::sqrt(eps0 * G_grav / (c_light * c_light)));  // 1 Tesla
#endif
...
void Mesh::InitUserMeshData(ParameterInput* pin)
{
  ...
  */
  ...
  std::string fn_ini_data =
    pin->GetOrAddString("problem", "filename", "resu.d");
  ...
  // Get the separation
  bns = new Lorene::Bin_NS(1, crd, crd, crd, fn_ini_data.c_str());
  sep = bns->dist;
  // sep in km
  delete bns;
  delete[] crd;
  ...
  bns =
    new Lorene::Bin_NS(npt_interp, xcrd_p, ycrd, zcrd, fn_ini_data.c_str());
  ...
  bns =
    new Lorene::Bin_NS(npt_interp, xcrd_m, ycrd, zcrd, fn_ini_data.c_str());
  ...
  {
    printf(
      "Warning: Internal energy in Lorene data and eos do not match "
...
#pragma omp critical
  {
    ...
    bns =
      new Lorene::Bin_NS(npoints_gs, xx_gs, yy_gs, zz_gs, fn_ini_data.c_str());
    if (!verbose)
    {
      std::cout.rdbuf(cur_buf);
    }
    ...
    if (verbose)
    {
      ...
      // Converting this to be in terms of the mass density rho = n m_nucleon
      // gets changes n_nuc to rho_nuc: K_SI(rho) = K_LORENE c^2 /
      // rho_nuc^(gamma-1) In SI units P has units of M / (L T^2) and rho has
      // units of M/L^3 thus K_SI has units of (L^3/M)^Gamma M/(L T^2). In
      // Cactus units P and rho have the same units thus K_Cactus is unitless.
    ...
    bns =
      new Lorene::Bin_NS(npoints_cc, xx_cc, yy_cc, zz_cc, fn_ini_data.c_str());
    ...
    < < < < < < < HEAD for (int k = kl; k <= ku;
                            ++k) for (int j = jl; j <= ju;
                                      ++j) for (int i = il; i <= iu; ++i)
    {
#if defined(USE_COMPOSE_EOS) || defined(USE_TABULATED_EOS) || \
  defined(USE_TRANSITION_EOS)
      // Lorene is using the atomic mass unit as reference mass so the density
      // has to be converted
...
>>>>>>> bugfix/zap
    ...
    Real vsq =
      (2.0 * (v_u_x * v_u_y * bns->g_xy[I] + v_u_x * v_u_z * bns->g_xz[I] +
              v_u_y * v_u_z * bns->g_yz[I]) +
       v_u_x * v_u_x * bns->g_xx[I] + v_u_y * v_u_y * bns->g_yy[I] +
