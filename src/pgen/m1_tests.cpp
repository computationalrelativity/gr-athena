  ...
  for (int ix_g = 0; ix_g < pm1->N_GRPS; ++ix_g)
    for (int ix_s = 0; ix_s < pm1->N_SPCS; ++ix_s)
    {
      ...
      M1_GLOOP3(k, j, i)
      {
        ...
        sp_F_d(0, k, j, i) =
          4.0 * ONE_3RD * J * SQR(W) * pm1->fidu.sp_v_d(0, k, j, i);
      }
                      ...
                      int ngh)
{
  ...
  M1::M1::vars_Lab U{ { pm1->N_GRPS, pm1->N_SPCS },
      ...
      for (int k = kl; k <= ku; ++k)
        for (int j = jl; j <= ju; ++j)
        {
          for (int i = 1; i <= ngh; ++i)
          {
            ...
            U_F_d(0, k, j, il - i) =
              (std::abs(pm1->mbi.x2(j)) <= 1.1) ? 1.0 : 0;
          }
...
void InitM1SphereRadAbs(MeshBlock* pmb, ParameterInput* pin)
{
  ...
  for (int ix_g = 0; ix_g < pm1->N_GRPS; ++ix_g)
    for (int ix_s = 0; ix_s < pm1->N_SPCS; ++ix_s)
    {
      ...
      M1_GLOOP3(k, j, i)
      {
        if ((SQR(pm1->mbi.x1(i)) + SQR(pm1->mbi.x2(j)) +
...
void InitM1ValueInject(MeshBlock* pmb, ParameterInput* pin)
{
  ...
  for (int a = 0; a < M1::N; ++a)
    for (int b = a; b < M1::N; ++b)
    {
      ...
      pm1->geom.sp_K_dd(a, b, K, J, I) =
        -0.1 * pm1->geom.sp_g_dd(a, b, K, J, I);
    }
  ...
  for (int c = 0; c < M1::N; ++c)
    for (int a = 0; a < M1::N; ++a)
      for (int b = a; b < M1::N; ++b)
      {
        pm1->geom.sp_dg_ddd(c, a, b, K, J, I) =
          1.2 + 10 * c + (a == b) + 0.1 * (a + b);
      }
...
void SetBHMask(MeshBlock* pmb, ParameterInput* pin)
{
  ...
  M1_GLOOP3(k, j, i)
  {
    ...
    pm1->MaskSet(SQR(x1) + SQR(x2) + SQR(x3) > SQR(R_BH), k, j, i);
  }
}
        ...
        for (int j = jl; j <= ju; ++j)
        {
          for (int i = 1; i <= ngh; ++i)
          {
            if (pm1->MaskGet(k, j, il - i) &&
                ...
                (pm1->mbi.x2(j) <= beam_y + beam_A) &&
                (pm1->mbi.x3(k) >= -beam_A) && (pm1->mbi.x3(k) <= +beam_A))
            {
              ...
              {
                beta2 +=
                  sp_beta_d(a, k, j, il - i) * sp_beta_u(a, k, j, il - i);
              }
              ...
              const Real fac_x1 =
                U_E(k, j, il - i) / sp_g_dd(0, 0, k, j, il - i) *
                (-sp_beta_d(0, k, j, il - i) +
                 std::sqrt(SQR(sp_beta_d(0, k, j, il - i)) -
                           sp_g_dd(0, 0, k, j, il - i) *
                             (beta2 - 0.99 * SQR(sc_alpha(k, j, il - i)))));
              ...
              for (int a = 0; a < M1::N; ++a)
              {
                ...
                {
                  U_F_d(a, k, j, il - i) +=
                    sp_F_u_(b, il - i) * sp_g_dd(a, b, k, j, il - i);
                }
              }
            }
            else
...
void Mesh::InitUserMeshData(ParameterInput* pin)
{
  ...
  {
    EnrollUserBoundaryFunction(BoundaryFace::inner_x1, Kerr::BCInnerX1);

    Kerr_AMR_sc_E_threshold =
      pin->GetOrAddReal("problem", "AMR_sc_E_threshold", 0);
    Kerr_AMR_target_level =
      pin->GetOrAddInteger("problem", "AMR_target_level", 0);
  }
