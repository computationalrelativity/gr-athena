...
#define SGRID_errorexits(s, t) SGRID_errorexits(__FILE__, __LINE__, (s), (t))
  ...
  int SGRID_extrstr_before_after_EQ(const char* str,
                                    char* before,
                                    char* after);
  int SGRID_fscanline(FILE* in, char* str);
  int SGRID_extrstr_before_after(const char* str,
                                 char* before,
                                 char* after,
                                 char z);
  int SGRID_find_before_after(const char* str,
                              char* before,
                              char* after,
                              const char* z);
  int SGRID_pfind_before_after(const char* str,
                               int p,
                               char* before,
                               char* after,
                               const char* z);
  ...
  int SGRID_DNSdata_Interpolate_ADMvars_to_xyz(double xyz[3],
                                               double* vars,
                                               int init);
}
...
enum
{
  ...
  idvar_VRy,
  idvar_VRz,
  idvar_NDATAMAX,  // TODO in NMESH this is 23, but they are 20, and only 20
                   // used...
};
...
void Mesh::InitUserMeshData(ParameterInput* pin)
{
  ...
  */
  ...
  {
    printf(
      "Warning: Internal energy in SGRID data and eos do not match "
    ...
    for (int k = 0; k < mbi->nn3; ++k)
      for (int j = 0; j < mbi->nn2; ++j)
        for (int i = 0; i < mbi->nn1; ++i)
        {
          Real zb = mbi->x3(k);
          Real yb =
            mbi->x2(j) * s180;  // multiply by -1 if 180 degree rotation
          Real xb     = mbi->x1(i) * s180;
          Real xs     = xb + sgrid_x_CM;  // shift x-coord
          Real xyz[3] = { xs, yb, zb };

          // Interpolate
          // This call is supposed to be threadsafe, it contains an OMP
          // Critical
    ...
    for (int k = kl; k <= ku; ++k)
      for (int j = jl; j <= ju; ++j)
        for (int i = il; i <= iu; ++i)
        {
          ...
          Real yb =
            pcoord->x2v(j) * s180;  // multiply by -1 if 180 degree rotation
          ...
          // Lorentz factor
          const Real vsq = (2.0 * (v_u_x * v_u_y * IDvars[idvar_gxy] +
...
//  \brief Initialize libsgrid: alloc mem, build the command and read
//  checkpoint This code is adapted from W.Tichy NMESH example
void DNS_init_sgrid(ParameterInput* pin)
{
  const int level_l = 0;
  ...
  const Real Interpolate_max_xyz_diff =
    pin->GetOrAddReal("problem", "Interpolate_max_xyz_diff", 0.);
  std::string outdir = pin->GetOrAddString("problem", "outdir", "SGRID");
  ...
  // MPIrank);
  std::sprintf(
    sgridoutdir, "%s/sgrid_level_%d_proc_%d", outdir.c_str(), level_l, myrank);
  std::sprintf(sgridoutdir_previous,
               "%s/sgrid_level_%d_proc_%d_previous",
               outdir.c_str(),
               level_l,
               myrank);
  ...
  if (stringptr == NULL)
  {  // no / found in DNSdataReader_sgrid_datadir
...
int DNS_parameters(ParameterInput* pin)
{
  ...
  std::snprintf(
    datadir, STRLEN - 1, "%s", pin->GetString("problem", "datadir").c_str());
  ...
  if (ret == EOF)
  {
    ...
    rewind(fp1);
    j =
      DNS_position_fileptr_after_str(fp1, "NS data properties (time = 0):\n");
    if (verbose)
  ...
  if ((strcmp(EoS_type, "PwP") == 0) || (strcmp(EoS_type, "pwp") == 0))
  {
    ...
    if (verbose)
    {
      ...
      std::printf("kappa     = %s\n", strkappa);
      std::printf(
        "Note: n_list contains the polytropic indices n,\n"
  ...
  /*
    ...
    Gets(Par("EoS_tab1d_load_file"))); printf("Make sure
    PwP_init_from_parameters gives a compatible EoS!!!\n");
