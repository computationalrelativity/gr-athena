...
Field::Field(MeshBlock* pmb, ParameterInput* pin)
    : pmy_block(pmb),
      b(pmb->ncells3, pmb->ncells2, pmb->ncells1),
                                   ...
                                   : AthenaArray<Real>::DataStatus::empty))
{
  ...
  // meshes.
  {
    ...
    amr.label           = "field_b";
    amr.face_var        = &b;
    amr.coarse_face_var = &coarse_b_;
    amr.nvar            = 3;
    amr.sampling        = comm::Sampling::FC;
    amr.group           = comm::AMRGroup::Main;
    amr.prolong_op      = comm::ProlongOp::FaceSharedMinmod;
    amr.restrict_op     = comm::RestrictOp::AreaWeightedFace;
  ...
  // Parity handled by hardcoded signs in FC physical BC functions, not
  // component_groups.
  {
    comm::CommSpec spec;
    spec.label        = "field_b";
