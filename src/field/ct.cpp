...
//  AthenaArray<Real> &area = face_area_, &len = edge_length_, &len_p1 = edge_length_p1_;
  Real area, x1l, x2l, x3l;
// TODO dynamical metric - remove length weightings - assuming Efield made from densitised flux and Bfield itself densitised
  //---- update B1
  for (int k=ks; k<=ke; ++k) {
    for (int j=js; j<=je; ++j) {
...
#pragma omp simd
        ...
        x1l = pmb->pcoord->dx1v(i);
        x2l = pmb->pcoord->dx2v(j);
        x3l = pmb->pcoord->dx3v(k);
        area = x2l*x3l;
          b_out.x1f(k,j,i) -=
//              (wght/area(i))*(len_p1(i)*e3(k,j+1,i) - len(i)*e3(k,j,i));
              (wght/x2l)*(e3(k,j+1,i) - e3(k,j,i));
        }
...
#pragma omp simd
      for (int i=is; i<=ie; ++i) {
        ...
        x3l = pmb->pcoord->dx3v(k);
        area = x1l*x3l;
        b_out.x2f(k,j,i) += (wght/x1l)*(e3(k,j,i+1) - e3(k,j,i));
      }
      if (pmb->block_size.nx3 > 1) {
...
#pragma omp simd
      for (int i=is; i<=ie; ++i) {
        ...
        x3l = pmb->pcoord->dx3v(k);
        area = x1l*x2l;
        b_out.x3f(k,j,i) -= (wght/x1l)*(e2(k,j,i+1) - e2(k,j,i));
      }
      if (pmb->block_size.nx2 > 1) {
//        pmb->pcoord->Edge1Length(k,j  ,is,ie,len);
