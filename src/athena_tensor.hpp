...
//
//  Convention: indices a,b,c,d are tensor indices. Indices n,i,j,k are grid
//  indices.
...
// tensor storage: Grid = field over a grid (AthenaArray), Point = single point
// (std::array)
enum class TensorStorage
{
  Grid,   // heap-allocated grid field (default, original behavior)
  Point,  // stack-allocated single-point tensor (replaces TensorPointwise)
};
...
// Data is stored in a flat array; accessor helpers convert multi-indices to
// flat offsets.
template <int N>
struct IdxMapResult
{
  ...
  else if constexpr (sym == TensorSymm::SYM2)
  {
    for (int a = 0; a < ndim; ++a)
      for (int b = 0; b < ndim; ++b)
        ...
        {
          r.data[a * ndim * ndim + b * ndim + c] = r.ndof++;
          r.data[a * ndim * ndim + c * ndim + b] =
            r.data[a * ndim * ndim + b * ndim + c];
        }
  }
    ...
    for (int a = 0; a < ndim; ++a)
      for (int b = 0; b < ndim; ++b)
        for (int c = 0; c < ndim; ++c)
          ...
          {
            r.data[a * N3 + b * N2 + c * ndim + d] = r.ndof++;
            r.data[a * N3 + b * N2 + d * ndim + c] =
              r.data[a * N3 + b * N2 + c * ndim + d];
          }
  }
  else if constexpr (sym == TensorSymm::ISYM2)
  {
...
template <typename T,
          ...
          int ndim,
          int rank,
          TensorStorage storage = TensorStorage::Grid>
class AthenaTensor;

//----------------------------------------------------------------------------------------
// rank 0 AthenaTensor: scalar fields (grid storage)
template <typename T, TensorSymm sym, int ndim>
class AthenaTensor<T, sym, ndim, 0, TensorStorage::Grid>
{
  public:
  // xtors: -------------------------------------------------------------------
  AthenaTensor()
  {
  }  // NewAthenaTensor needed for alloc.
  ...
  void PrintPoint(const std::string& name, const int i)
  {
    std::cout << name << "(" << i << ") = ";
...
//----------------------------------------------------------------------------------------
// rank 1 AthenaTensor: vector and co-vector fields (grid storage)
template <typename T, TensorSymm sym, int ndim>
class AthenaTensor<T, sym, ndim, 1, TensorStorage::Grid>
{
  public:
  // xtors: -------------------------------------------------------------------
  AthenaTensor()
  ...
  // - non-singleton
  void slice(const int a, AthenaTensor<T, TensorSymm::NONE, ndim, 0>& tar)
  {
    tar.InitWithShallowSlice(data_, ndim + 1, a);
  }

  // operators to access the data
  T& operator()(int const a, int const i)
  {
    return data_(a, i);
  }
  T& operator()(int const a, int const j, int const i)
  {
    return data_(a, j, i);
  }
  T& operator()(int const a, int const k, int const j, int const i)
  {
  ...
  T operator()(int const a, int const i) const
  {
    return data_(a, i);
  }
  T operator()(int const a, int const j, int const i) const
  {
    return data_(a, j, i);
  }
  T operator()(int const a, int const k, int const j, int const i) const
  {
    return data_(a, k, j, i);
  }
  T operator()(int const a,
  ...
  // array functions that initialize a tensor with shallow slice from an array
  void InitWithShallowSlice(AthenaArray<T>& src, const int indx)
  {
    data_.InitWithShallowSlice(src, indx, ndof());
  }
...
//----------------------------------------------------------------------------------------
// rank 2 AthenaTensor, e.g., the metric or the extrinsic curvature (grid
// storage)
template <typename T, TensorSymm sym, int ndim>
class AthenaTensor<T, sym, ndim, 2, TensorStorage::Grid>
{
  static constexpr auto idxmap_result_ =
    tensor_detail::compute_idxmap_rank2<sym, ndim>();
  ...
  // operators to access the data
  T& operator()(int const a, int const b, int const i)
  {
    return data_(idxmap(a, b), i);
  }
  T& operator()(int const a, int const b, int const j, int const i)
  {
  ...
  T operator()(int const a, int const b, int const i) const
  {
    return data_(idxmap(a, b), i);
  }
  T operator()(int const a, int const b, int const j, int const i) const
  {
    return data_(idxmap(a, b), j, i);
  }
...
//----------------------------------------------------------------------------------------
// rank 3 AthenaTensor, e.g., Christoffel symbols (grid storage)
template <typename T, TensorSymm sym, int ndim>
class AthenaTensor<T, sym, ndim, 3, TensorStorage::Grid>
{
  static constexpr auto idxmap_result_ =
    tensor_detail::compute_idxmap_rank3<sym, ndim>();
  ...
  // operators to access the data
  T& operator()(int const a, int const b, int const c, int const i)
  {
  ...
  T operator()(int const a, int const b, int const c, int const i) const
  {
    return data_(idxmap(a, b, c), i);
  }
  T operator()(int const a,
...
//----------------------------------------------------------------------------------------
// rank 4 AthenaTensor, e.g., mixed derivatives of the metric (grid storage)
template <typename T, TensorSymm sym, int ndim>
class AthenaTensor<T, sym, ndim, 4, TensorStorage::Grid>
{
  static constexpr auto idxmap_result_ =
    tensor_detail::compute_idxmap_rank4<sym, ndim>();
  ...
  static constexpr int idxmap(int const a,
                              ...
                              int const d)
  {
    return idxmap_result_
      .data[a * ndim * ndim * ndim + b * ndim * ndim + c * ndim + d];
  }
  static constexpr int ndof()
  {
    return idxmap_result_.ndof;
...
//========================================================================================
// Point-storage specializations: stack-allocated tensors at a single point
//
// These replace the former TensorPointwise class (utils/tensor.hpp).
// Storage is std::array<T, ndof> - no heap allocation, no New/Delete needed.
// operator() takes only tensor indices (no grid indices).
//========================================================================================

//----------------------------------------------------------------------------------------
// rank 0 AthenaTensor: scalar at a point
template <typename T, TensorSymm sym, int ndim>
class AthenaTensor<T, sym, ndim, 0, TensorStorage::Point>
{
  public:
  AthenaTensor()                               = default;
  ~AthenaTensor()                              = default;
  AthenaTensor(AthenaTensor const&)            = default;
  AthenaTensor& operator=(AthenaTensor const&) = default;
...
//----------------------------------------------------------------------------------------
// rank 1 AthenaTensor: vector / co-vector at a point
template <typename T, TensorSymm sym, int ndim>
class AthenaTensor<T, sym, ndim, 1, TensorStorage::Point>
{
  public:
  AthenaTensor()                               = default;
  ~AthenaTensor()                              = default;
  ...
  private:
  std::array<T, ndim> data_{};
};

//----------------------------------------------------------------------------------------
// rank 2 AthenaTensor at a point, e.g., metric, extrinsic curvature
template <typename T, TensorSymm sym, int ndim>
class AthenaTensor<T, sym, ndim, 2, TensorStorage::Point>
{
  static constexpr auto idxmap_result_ =
    tensor_detail::compute_idxmap_rank2<sym, ndim>();
  ...
  private:
  std::array<T, idxmap_result_.ndof> data_{};
};

//----------------------------------------------------------------------------------------
// rank 3 AthenaTensor at a point, e.g., Christoffel symbols, metric
// derivatives
template <typename T, TensorSymm sym, int ndim>
class AthenaTensor<T, sym, ndim, 3, TensorStorage::Point>
{
  static constexpr auto idxmap_result_ =
    tensor_detail::compute_idxmap_rank3<sym, ndim>();
...
//----------------------------------------------------------------------------------------
// rank 4 AthenaTensor at a point, e.g., second derivatives of the metric
template <typename T, TensorSymm sym, int ndim>
class AthenaTensor<T, sym, ndim, 4, TensorStorage::Point>
{
  static constexpr auto idxmap_result_ =
    tensor_detail::compute_idxmap_rank4<sym, ndim>();
