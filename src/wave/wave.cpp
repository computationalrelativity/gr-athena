...
#if WAVE_CC_ENABLED
#define WAVE_SW_CC_CX_VC(a, b, c) a
#elif WAVE_CX_ENABLED
#define WAVE_SW_CC_CX_VC(a, b, c) b
#else
#define WAVE_SW_CC_CX_VC(a, b, c) c
#endif

#if WAVE_CC_ENABLED || WAVE_CX_ENABLED
#define WAVE_SW_CCX_VC(a, b) a
#else
#define WAVE_SW_CCX_VC(a, b) b
#endif
                  ...
                  AthenaArray<Real>() }
{
  ...
  // set up sampling
  //---------------------------------------------------------------------------
  mbi.x1.InitWithShallowSlice(
    WAVE_SW_CCX_VC(pco->x1v, pco->x1f), 1, 0, mbi.nn1);
  mbi.x2.InitWithShallowSlice(
    WAVE_SW_CCX_VC(pco->x2v, pco->x2f), 1, 0, mbi.nn2);
  mbi.x3.InitWithShallowSlice(
    WAVE_SW_CCX_VC(pco->x3v, pco->x3f), 1, 0, mbi.nn3);

  // sizes are the same in either case
  mbi.dx1.InitWithShallowSlice(
    WAVE_SW_CCX_VC(pco->dx1v, pco->dx1f), 1, 0, mbi.nn1);
  mbi.dx2.InitWithShallowSlice(
    WAVE_SW_CCX_VC(pco->dx2v, pco->dx2f), 1, 0, mbi.nn2);
  mbi.dx3.InitWithShallowSlice(
    WAVE_SW_CCX_VC(pco->dx3v, pco->dx3f), 1, 0, mbi.nn3);
  //---------------------------------------------------------------------------
  ...
  // debug control
  debug_inspect_error =
    pin->GetOrAddBoolean("wave", "debug_inspect_err", false);
  debug_abort_threshold = pin->GetOrAddReal(
    "wave", "debug_abort_threshold", std::numeric_limits<Real>::max());

  // Additional boundary condition control
  std::string boundary_type =
    pin->GetOrAddString("wave", "boundary_type", "none");
    ...
    else if (pmb->pmy_mesh->ndim == 2)
    {
      ...
      for (int n = 1; n <= N_; ++n)
      {
        ...
        for (int m = 1; m <= M_; ++m)
        {
          std::string m_str = std::to_string(m);
          A_(m - 1, n - 1) =
            pin->GetOrAddReal("wave", "A_" + m_str + n_str, 0.);
          B_(m - 1, n - 1) =
            pin->GetOrAddReal("wave", "B_" + m_str + n_str, 0.);
        }
      }
    }
    else
      ...
      for (int o = 1; o <= O_; ++o)
      {
        ...
        for (int n = 1; n <= N_; ++n)
        {
          ...
          for (int m = 1; m <= M_; ++m)
          {
            std::string m_str = std::to_string(m);
            A_(m - 1, n - 1, o - 1) =
              pin->GetOrAddReal("wave", "A_" + m_str + n_str + o_str, 0.);
            B_(m - 1, n - 1, o - 1) =
              pin->GetOrAddReal("wave", "B_" + m_str + n_str + o_str, 0.);
          }
        }
      }
    }
