...
class Params:
  ...
  def __init__(self, args):
    ...
    self.analysis = (
        args.a
    ) ## what kind of analysis we want, L2 norm, derivative, plot, etc.
    self.coord_1d = (
        args.x
    ) ## the fixed coordinate along which the txt_1d(?) plot(s) is plotted

    self.findiff_ord = args.d ## finite difference order for FinDiff
    self.findiff_acc = (
        _findiff_acc ## finite difference accuracy here for FinDiff
    )
...
def L2(params, db, mbs, slice, file):
  db[params.output_field +
     "_L2"] = np.zeros(shape=db[params.output_field].shape)
  ...
  for mb in mbs.keys():
    ...
    if v_L2 == 0:
      print(
          "NOTE: in meshblock = {} L2-norm({}) = 0! We set it to {:e}.".format(
              mb, params.output_field, small_norm))
      v_L2 = small_norm
...
def Abs(params, db, mbs, slice, file):
  db[params.output_field +
     "_abs"] = np.zeros(shape=db[params.output_field].shape)
...
class Region:
  ...
  def MeshBlockByRadius(self, db):
    ...
    if len(mbs) == 0:
      raise Exception("No meshblock found for radius <= {}!".format(
          self.radius))
    return mbs
...
class Slice:
  ...
  coordv = (
      None ## normal coordinate to the plane, "x1v","x2v", "x3v" (center)
  )
  ...
  def __init__(self, params):
    ...
    if cs[0] == "z" or cs[0] == "Z":
      ...
      raise Exception(
          "could not pars '{}'! Set -c arg to, e.g., z=0.1.".format(cut))
  ...
  def SliceMeshBlock(self, params, db, vol):
    ...
    if len(self.mbs) == 0:
      raise Exception("could not find any {} = {} slice.".format(
          self.coordv, self.slice_val))
...
class Files:
  ...
  def __init__(self, params):
    ## get all files
    files = glob.glob("{}/*{}{}".format(params.hdf5_dir,
                                        params.hdf5_prefix,
                                        "*" + params.hdf5_suffix))
    if len(files) == 0:
      raise Exception("could not find any file '{}*{}' in:\n{}!".format(
          params.hdf5_prefix, params.hdf5_suffix, params.hdf5_dir))
...
class Plot:
  ...
  def __init__(self, params, db, mbs, slice, file):
    if params.out_format == "txt":
      ...
      if params.analysis == "grid_space":
        self.plot_radius_grid_space_txt(
            params,
            db,
            mbs,
            slice,
  ...
  def plot_2d_txt(self, params, db, mbs, slice, cycle, type, output):
    ...
    if type == "L2":
      ...
      txt_file.write('# "time = {}"\n'.format(cycle))
      txt_file.write("{} {}\n".format(params.resolution,
                                      L2_avg(params, db, mbs)))
      txt_file.close()
  ...
  def plot_radius_grid_space_txt(self,
                                 params,
                                 db,
                                 mbs,
                                 slice,
  ...
  def plot_1d_txt_mb(self,
    ...
    ## plot along x
      ...
      raise Exception("No such slice {} or kind {}!".format(
          slice.slice_dir, kind))
...
class Analysis:
  ...
  def __init__(self, params, db, mbs, slice, file):
    ...
    ## calc. derivative
      ...
      params.output_field = "d^{0}/dX^{0} ({1})".format(params.findiff_ord,
                                                        params.field_name)
  ...
  def derivative(self, params, db, mbs, slice, file):
    ...
    db[params.output_field] = np.zeros(shape=db[params.field_name].shape)
    db[params.output_field +
       "without_h"] = np.zeros(shape=db[params.field_name].shape)
    db[params.output_field +
       "grid-space"] = np.zeros(shape=db[params.field_name].shape)
...
if __name__ == "__main__":
  ...
  p = argparse.ArgumentParser(
      description="Plotting errors in a BBH Athena++ run.")
  ...
  p.add_argument(
      ...
      type=int,
      default=128,
      help="resolution of the run, e.g., 128, 96,...",
  )
  p.add_argument(
      "-p",
      type=str,
      required=True,
      help="hdf5 prefix, e.g., 'z4c_z' or 'adm'.",
  )
  p.add_argument(
      "-f",
      type=str,
      default="txt",
      help="output format = {pdf,png,txt,txt1d,txt1d_mbx,txt1d_mby,txt2d_mb}.",
  )
  p.add_argument(
      "-n",
      type=str,
      default="z4c.chi",
      help="field name, e.g., z4c.chi, con.H.",
  )
  p.add_argument(
      "-c",
      type=str,
      default="z=0.0",
      help="clipping/cutting of the 3D grid, e.g., z=0.",
  )
  ...
  p.add_argument(
      ...
      type=float,
      default=5.0,
      help="select all meshblocks whose radii are <= this value.",
  )
  p.add_argument(
      "-a",
      type=str,
  ...
  p.add_argument(
      ...
      type=float,
      default=1,
      help="derivative power. The results are powered to the m-th.",
  )
  p.add_argument(
      "-x",
      type=float,
      default=0,
      help="the fixed coordinate along which the txt_1d(?) plot(s) is plotted",
  )
